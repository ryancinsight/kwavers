...
fn test_plane_wave_creation() {
    let wave = KwaveAnalyticalPlaneWave::new(1e5, 1e6, SOUND_SPEED_WATER_SIM, [1.0, 0.0, 0.0], 0.0)
        .unwrap();
...
fn test_plane_wave_direction_normalization() {
    let wave = KwaveAnalyticalPlaneWave::new(1e5, 1e6, SOUND_SPEED_WATER_SIM, [3.0, 4.0, 0.0], 0.0)
        .unwrap();
    let norm =
...
fn test_error_metrics_phase_shifted() {
    let grid = Grid::new(32, 32, 32, 1e-3, 1e-3, 1e-3).unwrap();
    let wave1 =
        KwaveAnalyticalPlaneWave::new(1e5, 1e6, SOUND_SPEED_WATER_SIM, [1.0, 0.0, 0.0], 0.0)
            .unwrap();
    let wave2 =
        KwaveAnalyticalPlaneWave::new(1e5, 1e6, SOUND_SPEED_WATER_SIM, [1.0, 0.0, 0.0], PI / 4.0)
            .unwrap();
