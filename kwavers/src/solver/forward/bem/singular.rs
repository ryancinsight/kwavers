...
mod tests {
    ...
    fn test_duffy_closer_to_analytical_than_naive() {
        ...
        let qpoints = crate::solver::forward::bem::quadrature::TRIANGLE_3PT;
        let (area, _) = crate::solver::forward::bem::quadrature::triangle_area_normal(p0, p1, p2);
        let naive_sum: f64 = qpoints
            .iter()
            .map(|qp| {
                let r =
                    crate::solver::forward::bem::quadrature::map_to_triangle(qp.bary, p0, p1, p2);
                let dx = r[0];
                let dy = r[1];
