...
/// Preconditioned GMRES(restart) for dense real linear systems A·x = b.
///
/// Uses left Jacobi (diagonal) preconditioning: M = diag(A), solve `M⁻¹A x = M⁻¹b`.
/// This rescales rows so the diagonal is 1, improving condition number.
///
/// # Algorithm (Saad & Schultz 1986, §2)
///
/// Restarted GMRES with Modified Gram-Schmidt Arnoldi and Givens rotations
/// for the least-squares subproblem. Memory usage: O(N·restart) per outer restart.
///
/// ## Convergence guarantee
///
/// In exact arithmetic, GMRES converges in at most N outer iterations (each of
/// length `restart`). With diagonal preconditioning, convergence is faster when
/// diagonal dominance is high (common in BEM H matrices: H[i,i] ≈ 0.5).
///
/// # Arguments
...
///
/// # Errors
/// Returns `KwaversError::Numerical(NumericalError::ConvergenceFailed)` if
/// convergence is not achieved within `max_iter * restart` iterations.
///
/// # References
...
pub fn solve_gmres(
    ...
    // If A[i,i] ≈ 0, fall back to 1.0 (no scaling for that row).
    let precond: Vec<f64> = (0..n)
        .map(|i| {
            let aii = a[[i, i]];
            if aii.abs() < 1e-300 { 1.0 } else { 1.0 / aii }
        })
        .collect();
    ...
    let pb_norm = {
        let mut s = 0.0_f64;
        for i in 0..n { s += (precond[i] * rhs[i]).powi(2); }
        s.sqrt()
    };
    if pb_norm < 1e-300 {
    ...
    for outer in 0..max_iter {
        ...
        for i in 0..n { r[i] = precond[i] * (rhs[i] - ax[i]); }
        ...
        // Givens rotation factors for incremental QR of H
        let mut c_rot = vec![0.0_f64; m]; // cosines
        let mut s_rot = vec![0.0_f64; m]; // sines
        // RHS for least-squares: g = [β, 0, 0, ..., 0]
        let mut g = Array1::<f64>::zeros(m + 1);
        g[0] = beta;

        // v[:,0] = r / ‖r‖
        for i in 0..n { v[[i, 0]] = r[i] / beta; }

        let mut j_end = m;
        for j in 0..m {
            // w = M⁻¹ · A · v[:,j]   (preconditioned matvec)
            let v_j: Array1<f64> = v.column(j).to_owned();
            let av_j = a.dot(&v_j);
            let mut w = Array1::<f64>::zeros(n);
            for i in 0..n { w[i] = precond[i] * av_j[i]; }
            ...
            // New Givens rotation G_j to zero out h[j+1, j]
            let denom = (h[[j, j]].powi(2) + h[[j + 1, j]].powi(2)).sqrt();
            if denom < 1e-300 {
                // Breakdown: residual is already zero
                j_end = j + 1;
                break;
            }
            c_rot[j] = h[[j, j]] / denom;
            ...
            // Next basis vector (only if h_jp1 > 0 and space remains)
            if h_jp1 > 1e-300 && j + 1 < m {
                for i in 0..n { v[[i, j + 1]] = w[i] / h_jp1; }
            }

            // Convergence check: current residual norm is |g[j+1]|
        ...
        // Solve upper triangular H[0..j_end, 0..j_end] · y = g[0..j_end]
        // (back substitution)
        let mut y = Array1::<f64>::zeros(j_end);
        for i in (0..j_end).rev() {
            y[i] = g[i];
            for k in (i + 1)..j_end { y[i] -= h[[i, k]] * y[k]; }
            if h[[i, i]].abs() > 1e-300 {
                y[i] /= h[[i, i]];
            }
        }
        ...
        for k in 0..j_end {
            ...
            for i in 0..n { x[i] += y[k] * v_k[i]; }
        }

        // Final convergence check after update
        ...
        let _ = outer; // suppress warning in final iteration
    }

    // Final residual for error reporting
    let r_final: Array1<f64> = rhs - &a.dot(&x);
...
mod tests {
    ...
    ///
    /// Reference: Saad & Schultz (1986), Theorem 2.1 — exact convergence in ≤N steps.
    #[test]
    fn test_gmres_matches_direct_small_system() {
        // Diagonally dominant 5×5 system: A·x = b with known solution x* = [1,2,3,4,5]
        let n = 5usize;
        let mut a = ndarray::Array2::<f64>::zeros((n, n));
        // Set diagonal-dominant entries
        for i in 0..n {
            a[[i, i]] = 10.0 + i as f64;
            if i > 0 { a[[i, i-1]] = -1.0; }
        ...
        let max_err = x_gmres.iter().zip(x_true.iter())
            .map(|(a, b)| (a - b).abs())
            .fold(0.0_f64, f64::max);
        assert!(max_err < 1e-10, "GMRES solution error too large: {max_err:.3e}");
    }

    /// GMRES must converge within N iterations for an N×N non-singular system.
    ///
    /// Theorem (Saad & Schultz 1986): GMRES converges in ≤N steps in exact arithmetic.
    #[test]
    fn test_gmres_converges_within_n_iters() {
        let n = 8usize;
        // Build a tridiagonal system that GMRES solves in few iterations
        let mut a = ndarray::Array2::<f64>::zeros((n, n));
        for i in 0..n {
            a[[i, i]] = 4.0;
            if i > 0     { a[[i, i-1]] = -1.0; }
        ...
        // Solve with restart=n (full GMRES) — must converge in 1 outer restart
        ...
        assert!(res_norm / rhs_norm < 1e-10,
            "GMRES residual too large: rel={:.3e}", res_norm / rhs_norm);
    }

    /// GMRES with non-converging tolerance must return ConvergenceFailed error.
    #[test]
    fn test_gmres_nonconvergence_returns_error() {
        // Singular (all-zeros) system — cannot converge
        let n = 3usize;
        let a = ndarray::Array2::<f64>::zeros((n, n)); // singular
        let rhs = Array1::from_elem(n, 1.0);           // inconsistent rhs
