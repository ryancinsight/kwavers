...
impl BurtonMillerAssembler {
    ...
    pub(super) fn greens_function_double_normal_derivative(
        ...
        let cos_nx = rhat[2].mul_add(
            normal_x[2],
            rhat[0].mul_add(normal_x[0], rhat[1] * normal_x[1]),
        );
        let cos_ny = rhat[2].mul_add(
            normal_y[2],
            rhat[0].mul_add(normal_y[0], rhat[1] * normal_y[1]),
        );
        let nxny = normal_x[2].mul_add(
            normal_y[2],
    ...
    pub(super) fn triangle_area(&self, p1: [f64; 3], p2: [f64; 3], p3: [f64; 3]) -> f64 {
        ...
        0.5 * cross[2]
            .mul_add(cross[2], cross[1].mul_add(cross[1], cross[0].powi(2)))
            .sqrt()
    }
}
