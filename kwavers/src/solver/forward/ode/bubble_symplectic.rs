...
//! - `test_hamiltonian_no_drift`: H oscillation (max-min)/H₀ < 1% over 1000 periods
//! - `test_yoshida4_order`: Convergence order 4.0 ± 10%
...
const YOSHIDA_W1: f64 = 1.351_207_191_959_657_7;
const YOSHIDA_W2: f64 = -1.702_414_383_919_315_3;
...
// ─── Core step functions (free functions for use without the wrapper struct) ──
...
pub fn stormer_verlet_step(
    ...
    let v_half = state.wall_velocity + 0.5 * dt * accel_0;
    ...
    let r_new = state.radius + dt * v_half;
    ...
    state.wall_velocity = v_half + 0.5 * dt * accel_1;
