...
pub fn acoustic_energy_density(
    ...
    let kinetic = 0.5 * density * velocity.2.mul_add(velocity.2, velocity.1.mul_add(velocity.1, velocity.0.powi(2)));
...
pub fn acoustic_intensity(pressure: f64, velocity: (f64, f64, f64)) -> f64 {
    ...
    flux_z.mul_add(flux_z, flux_y.mul_add(flux_y, flux_x.powi(2))).sqrt()
}
