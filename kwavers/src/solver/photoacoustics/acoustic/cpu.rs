            ...
            for i in 0..nx {
                ...
                for j in 0..ny {
                    ...
                    for k in 0..nz {
                        ...
                        let laplacian = (workspace.current[[ip, j, k]] - 2.0 * center
                            + workspace.current[[im, j, k]])
                            * inv_dx2
                            + (workspace.current[[i, jp, k]] - 2.0 * center
                                + workspace.current[[i, jm, k]])
                                * inv_dy2
                            + (workspace.current[[i, j, kp]] - 2.0 * center
                                + workspace.current[[i, j, km]])
                                * inv_dz2;
                        workspace.next[[i, j, k]] =
                            2.0 * center - workspace.previous[[i, j, k]] + c2_dt2 * laplacian;
                    }
                }
            }
