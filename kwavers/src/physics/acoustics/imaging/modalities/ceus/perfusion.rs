...
//!
//! **Advection discretisation — first-order upwind (CFL-stable)**
//!
//! For the x-component (analogous for y, z):
//! ```text
//! u_x ≥ 0:  ∂C/∂x ≈ (C[i,j,k] − C[i−1,j,k]) / dx   (backward difference)
//! u_x < 0:  ∂C/∂x ≈ (C[i+1,j,k] − C[i,  j,k]) / dx  (forward difference)
//! ```
//! CFL stability requires `|u| dt / dx ≤ 1`; caller is responsible for
//! choosing dt accordingly.  A scratch buffer is used so that all reads
//! are from the state at time n (not the partially-updated state at n+1).
//!
//! **Boundary conditions**
//!
//! - i=0 (inflow face, u_x ≥ 0): Dirichlet — `C[0,j,k] = inflow_concentration`
//! - i=nx−1 (outflow, u_x ≥ 0): zero-gradient (upwind naturally gives this)
//! - All other faces: zero-gradient (no flux through walls)
//!
//! ## References
//!
...
impl PerfusionModel {
    ...
    pub fn update_concentration(
        ...
        // (smallest dx gives most conservative rate estimate)
        let dx = self.dx;
        let dy = self.dy;
        let dz = self.dz;
        let k_perf = self.permeability / dx; // [1/s]
                ...
                for k in 0..nz {
                    ...
                    let dc_dx = if vx >= 0.0 {
                        if i > 0 {
                            ...
                            0.0 // left boundary: upwind difference not available; zero-flux
                        }
                    } else if i + 1 < nx {
                        (c_old[[i + 1, j, k]] - c_old[[i, j, k]]) / dx
                    } else {
                        0.0 // right boundary: zero-gradient outflow
                    };
