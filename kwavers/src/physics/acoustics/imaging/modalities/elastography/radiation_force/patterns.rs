...
impl MultiDirectionalPush {
    ...
    pub fn focused_pattern(roi_center: [f64; 3], roi_size: [f64; 3], density: usize) -> Self {
        ...
        for i in 0..nx {
            for j in 0..ny {
                for k in 0..nz {
                    ...
                    let distance = (z - roi_center[2])
                        .mul_add(
                            z - roi_center[2],
                            (y - roi_center[1])
                                .mul_add(y - roi_center[1], (x - roi_center[0]).powi(2)),
                        )
                        .sqrt();
        ...
        if pushes.len() > density {
            pushes.sort_by(|a, b| {
                let dist_a = (a.location[2] - roi_center[2])
                    .mul_add(
                        a.location[2] - roi_center[2],
                        (a.location[1] - roi_center[1]).mul_add(
                            a.location[1] - roi_center[1],
                            (a.location[0] - roi_center[0]).powi(2),
                        ),
                    )
                    .sqrt();
                let dist_b = (b.location[2] - roi_center[2])
                    .mul_add(
                        b.location[2] - roi_center[2],
                        (b.location[1] - roi_center[1]).mul_add(
                            b.location[1] - roi_center[1],
                            (b.location[0] - roi_center[0]).powi(2),
                        ),
                    )
                    .sqrt();
