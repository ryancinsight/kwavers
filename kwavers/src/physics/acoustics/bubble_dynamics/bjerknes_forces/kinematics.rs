...
mod tests {
    ...
    fn predict_bubble_motion_zero_initial_velocity_matches_kinematics() {
        ...
        assert!((disp - expected_displacement).abs() < 1e-30,
            "displacement: expected {expected_displacement:.4e}, got {disp:.4e}");
        assert!((vel - expected_velocity).abs() < 1e-30,
    ...
    fn predict_bubble_motion_rejects_invalid_inputs() {
        let c = calc();
        assert!(c.predict_bubble_motion(0.0, 1e-9, 0.0, 1e-7).is_err(),
            "zero radius must be rejected");
        assert!(c.predict_bubble_motion(1e-6, 1e-9, 0.0, 0.0).is_err(),
    ...
    fn coalescence_probability_zero_when_too_far() {
        ...
        assert_eq!(c.coalescence_probability(far, 1.0), 0.0,
