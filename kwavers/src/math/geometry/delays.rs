...
mod tests {
    ...
    fn focus_phase_delays_are_non_negative_and_symmetric() {
        ...
        let delays =
            focus_phase_delays(&positions, [0.0, 0.0, 0.05], MHZ_TO_HZ, SOUND_SPEED_TISSUE).unwrap();
    ...
    fn plane_wave_broadside_has_zero_phase_delays_for_x_array() {
        ...
        let delays =
            plane_wave_phase_delays(&positions, [0.0, 0.0, 1.0], MHZ_TO_HZ, SOUND_SPEED_TISSUE).unwrap();
        for &delay in delays.iter() {
