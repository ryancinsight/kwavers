//! Physical constants for acoustic simulations
//!
//! This module provides a single source of truth (SSOT) for all physical constants
//! used throughout the simulation, eliminating magic numbers and ensuring consistency.

pub mod acoustic_parameters;
pub mod cavitation;
pub mod chemistry;
pub mod ct_acoustics;
pub mod fundamental;
pub mod hounsfield;
pub mod implants;
pub mod medical;
pub mod numerical;
pub mod optical;
pub mod state_dependent;
pub mod thermodynamic;
pub mod tissue_acoustics;
pub mod tissue_thermal;
pub mod water;

// ============================================================================
// EXPLICIT RE-EXPORTS (Core Physical Constants API)
// ============================================================================
// Re-export the most commonly used constants. For specialized constants,
// import directly from submodules: `use crate::core::constants::cavitation::BLAKE_THRESHOLD;`

// Fundamental physical constants (simulation defaults, universal physics, elastic)
pub use fundamental::{
    ACOUSTIC_ABSORPTION_TISSUE, ACOUSTIC_IMPEDANCE_TISSUE_NOMINAL, ATMOSPHERIC_PRESSURE,
    AVOGADRO, BOLTZMANN, BOND_TRANSFORM_FACTOR,
    DENSITY_TISSUE, DENSITY_WATER_37C, DENSITY_WATER_NOMINAL,
    GAS_CONSTANT, GRAVITY, LAME_TO_STIFFNESS_FACTOR,
    OPTICAL_ABSORPTION_TISSUE_NIR, PLANCK, REDUCED_SCATTERING_TISSUE_NIR,
    SOUND_SPEED_AIR, SOUND_SPEED_TISSUE, SOUND_SPEED_WATER, SOUND_SPEED_WATER_37C,
    SOUND_SPEED_WATER_SIM, SPEED_OF_LIGHT, SYMMETRY_TOLERANCE,
};

// Tissue-specific acoustic properties (SSOT: tissue_acoustics)
pub use tissue_acoustics::{
    ACOUSTIC_ABSORPTION_BLOOD, ACOUSTIC_ABSORPTION_BLOOD_PLASMA, ACOUSTIC_ABSORPTION_BRAIN,
    ACOUSTIC_ABSORPTION_BRAIN_GRAY, ACOUSTIC_ABSORPTION_BRAIN_WHITE,
    ACOUSTIC_ABSORPTION_CORTICAL_BONE, ACOUSTIC_ABSORPTION_CSF, ACOUSTIC_ABSORPTION_FAT,
    ACOUSTIC_ABSORPTION_KIDNEY_CORTEX, ACOUSTIC_ABSORPTION_LIVER, ACOUSTIC_ABSORPTION_MUSCLE,
    ACOUSTIC_ABSORPTION_MUSCLE_TRANSVERSE, ACOUSTIC_ABSORPTION_SKULL_BULK,
    ACOUSTIC_ABSORPTION_SKULL_CORTICAL_MIN, ACOUSTIC_ABSORPTION_SKULL_MIN,
    ACOUSTIC_ABSORPTION_SKULL_RANGE, ACOUSTIC_ABSORPTION_URINE, ACOUSTIC_ABSORPTION_WHOLE_BLOOD,
    B_OVER_A_AIR, B_OVER_A_BLOOD, B_OVER_A_BONE, B_OVER_A_BRAIN, B_OVER_A_BREAST_GLAND,
    B_OVER_A_CSF, B_OVER_A_FAT, B_OVER_A_KIDNEY, B_OVER_A_LIVER, B_OVER_A_LUNG,
    B_OVER_A_MINERAL_OIL, B_OVER_A_MUSCLE, B_OVER_A_NANOPARTICLE_SUSPENSION, B_OVER_A_SKIN,
    B_OVER_A_SOFT_TISSUE, B_OVER_A_URINE, B_OVER_A_WATER, B_OVER_A_WATER_37C,
    DENSITY_AIR, DENSITY_BLOOD, DENSITY_BRAIN, DENSITY_BREAST_FAT, DENSITY_BREAST_GLAND,
    DENSITY_CARTILAGE, DENSITY_CSF, DENSITY_FAT, DENSITY_KIDNEY, DENSITY_KIDNEY_MEDULLA,
    DENSITY_LIVER, DENSITY_MICROBUBBLE_SUSPENSION, DENSITY_MINERAL_OIL, DENSITY_MUSCLE,
    DENSITY_ULTRASOUND_GEL, DENSITY_URINE,
    SOFT_TISSUE_ABSORPTION_POWER_Y, SOUND_SPEED_BLOOD, SOUND_SPEED_BRAIN,
    SOUND_SPEED_BRAIN_GRAY_MATTER, SOUND_SPEED_BREAST_GLAND, SOUND_SPEED_CARTILAGE,
    SOUND_SPEED_CSF, SOUND_SPEED_FAT,
    SOUND_SPEED_KIDNEY, SOUND_SPEED_KIDNEY_MEDULLA, SOUND_SPEED_LIVER, SOUND_SPEED_LUNG,
    SOUND_SPEED_MINERAL_OIL, SOUND_SPEED_MUSCLE, SOUND_SPEED_NANOPARTICLE_SUSPENSION,
    SOUND_SPEED_ULTRASOUND_GEL, SOUND_SPEED_URINE,
    WATER_ABSORPTION_ALPHA_0_DB_CM_MHZ2, WATER_ABSORPTION_POWER_Y,
};

// CT / Hounsfield-unit thresholds and skull density model (SSOT: ct_acoustics)
pub use ct_acoustics::{
    DENSITY_SKULL_CORTICAL_RANGE, DENSITY_SKULL_MIN,
    HU_ABDOMEN_BODY_THRESHOLD, HU_BONE_THRESHOLD, HU_BRAIN_BODY_THRESHOLD, HU_SKULL_RANGE,
    SOUND_SPEED_SOFT_TISSUE_MAX,
};

// Water properties (most common medium)
pub use cavitation::{
    GAS_DIFFUSION_COEFFICIENT_AIR, GAS_DIFFUSION_COEFFICIENT_TISSUE, SURFACE_TENSION_TISSUE,
    SURFACE_TENSION_WATER, VAPOR_PRESSURE_WATER, VISCOSITY_AIR, VISCOSITY_WATER,
};
pub use fundamental::{BULK_MODULUS_WATER, C_WATER, DENSITY_WATER, WATER_TAIT_B, WATER_TAIT_N};

// Cavitation control limits
pub use cavitation::{INITIAL_BUBBLE_RADIUS, MAX_DUTY_CYCLE, MAX_RADIUS, MIN_DUTY_CYCLE, MIN_RADIUS};

// Acoustic parameters (most commonly used)
pub use acoustic_parameters::{
    AIR_POLYTROPIC_INDEX, BA_RATIO_TISSUE, BA_RATIO_WATER,
    BLOOD_PLASMA_VISCOSITY_37C, BLOOD_VISCOSITY_37C, DB_TO_NP,
    DENSITY_SKULL, DENSITY_SKULL_SUTURE, DENSITY_SKULL_TRABECULAR,
    DIAGNOSTIC_FREQ_MAX, DIAGNOSTIC_FREQ_MIN, NP_TO_DB,
    NONLINEARITY_COEFFICIENT_TISSUE, NONLINEARITY_COEFFICIENT_WATER,
    REFERENCE_FREQUENCY_HZ, REFERENCE_FREQUENCY_TISSUE_HZ,
    SAMPLING_FREQUENCY_DEFAULT, SHEAR_SPEED_SKULL_CORTICAL, SHEAR_SPEED_SKULL_TRABECULAR,
    SKULL_ATTENUATION_MARSAC_MAX_NP_PER_M_MHZ,
    SOUND_SPEED_SKULL_CORTICAL, SOUND_SPEED_SKULL_SUTURE, SOUND_SPEED_SKULL_TRABECULAR,
    VISCOSITY_PARENCHYMAL_TISSUE, VISCOSITY_SOFT_TISSUE,
    WATER_ABSORPTION_ALPHA_0, WATER_ABSORPTION_POWER, WATER_NONLINEARITY_B_A,
    WATER_VAPOR_PRESSURE_20C, WATER_VISCOSITY_20C,
};

// Thermodynamic constants
pub use thermodynamic::{
    BLOOD_TEMPERATURE_C, BODY_TEMPERATURE_C, BODY_TEMPERATURE_K,
    HEAT_CAPACITY_RATIO_DIATOMIC, HEAT_CAPACITY_RATIO_MONATOMIC,
    GRUNEISEN_SOFT_TISSUE, GRUNEISEN_SOFT_TISSUE_TEMP_COEFF,
    GRUNEISEN_WATER_20C, GRUNEISEN_WATER_37C,
    GRUNEISEN_WATER_TEMP_COEFF, GRUNEISEN_WATER_T_REF_C, H_VAP_WATER_100C,
    KELVIN_OFFSET_C, M_WATER, P_CRITICAL_WATER, P_TRIPLE_WATER,
    ROOM_TEMPERATURE_C, ROOM_TEMPERATURE_K, SPECIFIC_HEAT_WATER,
    SPECIFIC_HEAT_WATER_37C, THERMAL_CONDUCTIVITY_WATER, THERMAL_CONDUCTIVITY_WATER_37C,
    T_BOILING_WATER, T_CRITICAL_WATER, T_TRIPLE_WATER,
};

// Tissue-specific thermal properties (SSOT: tissue_thermal)
pub use tissue_thermal::{
    DC_DT_SOFT_TISSUE, DRHO_DT_SOFT_TISSUE, RHO_C_SOFT_TISSUE, THERMAL_EXPANSION_SOFT_TISSUE,
    SPECIFIC_HEAT_BLOOD, SPECIFIC_HEAT_BONE, SPECIFIC_HEAT_TISSUE,
    SPECIFIC_HEAT_BLOOD_PLASMA, SPECIFIC_HEAT_BRAIN_GRAY, SPECIFIC_HEAT_BRAIN_WHITE,
    SPECIFIC_HEAT_CSF, SPECIFIC_HEAT_MINERAL_OIL, SPECIFIC_HEAT_ULTRASOUND_GEL, SPECIFIC_HEAT_URINE,
    THERMAL_CONDUCTIVITY_BLOOD, THERMAL_CONDUCTIVITY_BRAIN, THERMAL_CONDUCTIVITY_BRAIN_GRAY,
    THERMAL_CONDUCTIVITY_CSF, THERMAL_CONDUCTIVITY_FAT, THERMAL_CONDUCTIVITY_KIDNEY,
    THERMAL_CONDUCTIVITY_LIVER, THERMAL_CONDUCTIVITY_MINERAL_OIL, THERMAL_CONDUCTIVITY_MUSCLE,
    THERMAL_CONDUCTIVITY_SKULL, THERMAL_CONDUCTIVITY_TISSUE, THERMAL_CONDUCTIVITY_ULTRASOUND_GEL,
    THERMAL_CONDUCTIVITY_URINE,
    THERMAL_DIFFUSIVITY_BLOOD, THERMAL_DIFFUSIVITY_TISSUE, THERMAL_DIFFUSIVITY_WATER,
};

// Medical constants (FDA limits)
pub use medical::{
    FDA_DERATING_FACTOR, ISPPA_LIMIT, ISPTA_LIMIT, MI_CAVITATION_BOWEL, MI_CAVITATION_BRAIN,
    MI_CAVITATION_FETAL, MI_CAVITATION_LUNG, MI_CAVITATION_OPHTHALMIC, MI_CAVITATION_SOFT_TISSUE,
    MI_LIMIT_BOWEL, MI_LIMIT_BRAIN, MI_LIMIT_FETAL, MI_LIMIT_LUNG, MI_LIMIT_OPHTHALMIC,
    IEC_TISSUE_ABSORPTION_DB_CM_MHZ, MI_LIMIT_SOFT_TISSUE, THERMAL_DOSE_REFERENCE_TEMP_C,
    IEC_TIB_DIVISOR, IEC_TIS_DIVISOR, THERMAL_DOSE_COAGULATION_CEM43,
    THERMAL_DOSE_DAMAGE_THRESHOLD_CEM43, THERMAL_DOSE_DIAGNOSTIC_SAFETY_CEM43,
    THERMAL_DOSE_PROTEIN_DENATURATION_CEM43,
    THERMAL_DOSE_R_ABOVE_43C, THERMAL_DOSE_R_BELOW_43C, THERMAL_DOSE_THRESHOLD,
    TI_LIMIT_SOFT_TISSUE, BLOOD_PERFUSION_RATE, TISSUE_PERFUSION_RATE, BLOOD_PERFUSION_RATE_BRAIN,
};

// Numerical constants
pub use numerical::{
    ABSORPTION_SINGULARITY_THRESHOLD, BYTES_PER_F64, CFL_DEFAULT, CFL_FACTOR_3D_FDTD,
    CFL_SAFETY_FACTOR, EPSILON, EV_PER_JOULE, FOUR_PI, MHZ_TO_HZ, MMHG_TO_PA, MPA_TO_PA,
    SECONDS_PER_DAY, SECONDS_PER_HOUR, SECONDS_PER_MINUTE, SMALL_VALUE, SOLVER_TOLERANCE, TWO_PI,
};

// State-dependent constants (temperature/pressure-dependent physical properties)
pub use state_dependent::StateDependentConstants;

// For comprehensive access to all constants, use the submodule namespaces:
// - `acoustic_parameters::*` - Acoustic pressure and intensity references
// - `cavitation::*` - Cavitation thresholds and bubble constants
// - `chemistry::*` - Chemical kinetics and ROS parameters
// - `hounsfield::*` - Hounsfield unit mappings for CT data
// - `optical::*` - Optical wavelengths and refractive indices
// - `state_dependent::*` - Temperature/pressure-dependent constants
