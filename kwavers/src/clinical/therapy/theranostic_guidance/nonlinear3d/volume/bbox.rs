...
fn body_cube_bbox(body: &Array3<bool>, spacing_mm: [f64; 3]) -> KwaversResult<BBox> {
...
fn mask_bounds(mask: &Array3<bool>) -> KwaversResult<BBox> {
    let mut bbox = BBox {
        x0: usize::MAX,
        x1: 0,
        y0: usize::MAX,
    ...
    for ((ix, iy, iz), active) in mask.indexed_iter() {
        if *active {
            bbox.x0 = bbox.x0.min(ix);
            bbox.x1 = bbox.x1.max(ix);
            bbox.y0 = bbox.y0.min(iy);
            bbox.y1 = bbox.y1.max(iy);
            bbox.z0 = bbox.z0.min(iz);
            bbox.z1 = bbox.z1.max(iz);
            any = true;
        }
    }
    any.then_some(bbox)
...
fn max_distance_to_bbox(center: [f64; 3], bbox: BBox, spacing_mm: [f64; 3]) -> f64 {
