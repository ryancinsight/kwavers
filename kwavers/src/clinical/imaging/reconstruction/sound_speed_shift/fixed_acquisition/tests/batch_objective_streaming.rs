use super::*;
use crate::core::error::KwaversError;

#[test]
fn batch_objective_streaming_reports_histories_without_images() {
    let mask = Array2::from_elem((3, 3), true);
    let mut truth = Array2::zeros((3, 3));
    truth.fill(8.0);
    let samples = horizontal_samples(&[-0.001, 0.0, 0.001]);
    let config = SoundSpeedShiftConfig {
        spacing_m: 0.001,
        iterations: 12,
        tikhonov_weight: 0.0,
        smoothness_weight: 0.0,
        ..Default::default()
    };
    let frame = predict_sound_speed_time_shifts(&truth, &samples, &mask, config).unwrap();
    let scaled_frame = frame.iter().map(|value| 0.5 * *value).collect::<Vec<_>>();
    let frame_refs = [&frame[..], &scaled_frame[..]];
    let plan = SoundSpeedShiftPlan::new(samples, &mask, config).unwrap();
    let rows_used = plan.rows_used();
    let rows_available = plan.rows_available();
    let active_voxels = plan.active_voxels();
    let mut workspace = SoundSpeedShiftPlanWorkspace::new();
    let mut summaries = Vec::new();
    let mut history_lengths = Vec::new();
    let mut history_ptrs = Vec::new();

    let stream = plan
        .reconstruct_frames_streaming_objectives_with_plan_workspace(
            &frame_refs,
            &mut workspace,
            |summary, history| {
                assert!(!history.is_empty());
                assert_eq!(summary.objective_initial, *history.first().unwrap());
                assert_eq!(summary.objective_final, *history.last().unwrap());
                assert_eq!(summary.objective_iterations, history.len() - 1);
                assert!(summary.objective_final <= summary.objective_initial);
                summaries.push(summary);
                history_lengths.push(history.len());
                history_ptrs.push(history.as_ptr() as usize);
                Ok(())
            },
        )
        .unwrap();

    assert_eq!(stream.frames_processed, 2);
    assert_eq!(stream.rows_used, rows_used);
    assert_eq!(stream.rows_available, rows_available);
    assert_eq!(stream.active_voxels, active_voxels);
    assert_eq!(stream.model_family, SOUND_SPEED_SHIFT_MODEL);
    assert_eq!(summaries[0].frame_index, 0);
    assert_eq!(summaries[1].frame_index, 1);
    assert!(history_lengths.iter().all(|length| *length > 0));
    assert_eq!(history_ptrs[0], history_ptrs[1]);

    let rhs_capacity = workspace.sampled_rhs_capacity();
    let retained_slots = workspace.allocated_slots();
    let repeated = plan
        .reconstruct_frames_streaming_objectives_with_plan_workspace(
            &frame_refs,
            &mut workspace,
            |summary, history| {
                assert_eq!(summary.objective_final, *history.last().unwrap());
                Ok(())
            },
        )
        .unwrap();

    assert_eq!(repeated.frames_processed, 2);
    assert_eq!(workspace.sampled_rhs_capacity(), rhs_capacity);
    assert_eq!(workspace.allocated_slots(), retained_slots);
}

#[test]
fn batch_objective_streaming_propagates_callback_error_and_stops() {
    let mask = Array2::from_elem((3, 3), true);
    let mut truth = Array2::zeros((3, 3));
    truth.fill(6.0);
    let samples = horizontal_samples(&[-0.001, 0.0, 0.001]);
    let config = SoundSpeedShiftConfig {
        spacing_m: 0.001,
        iterations: 8,
        tikhonov_weight: 0.0,
        smoothness_weight: 0.0,
        ..Default::default()
    };
    let frame = predict_sound_speed_time_shifts(&truth, &samples, &mask, config).unwrap();
    let scaled_frame = frame.iter().map(|value| 0.25 * *value).collect::<Vec<_>>();
    let frame_refs = [&frame[..], &scaled_frame[..]];
    let plan = SoundSpeedShiftPlan::new(samples, &mask, config).unwrap();
    let mut seen = 0usize;

    let error = plan
        .reconstruct_frames_streaming_objectives(&frame_refs, |summary, history| {
            seen += 1;
            assert_eq!(summary.frame_index, 0);
            assert_eq!(summary.objective_initial, *history.first().unwrap());
            Err(KwaversError::InvalidInput(
                "objective stream callback stop".to_owned(),
            ))
        })
        .unwrap_err();

    assert_eq!(seen, 1);
    assert!(error.to_string().contains("objective stream callback stop"));
}
