...
impl RealTimeSirtPipeline {
    ...
    pub fn process_frame(
        ...
        // Perform SIRT iterations: x^(k+1) = x^(k) + λ · Aᵀ · (b − A·x^(k))
        // Since we lack an explicit system matrix A, we use a simplified
        // projection/backprojection model: the forward projection sums along
        // the depth (z) axis, and the backprojection distributes the residual
        // uniformly back along that axis.
        ...
        for _ in 0..self.config.sirt_config.max_iterations {
            ...
            let mut projection = Array1::zeros(nx * ny);
            for i in 0..nx {
                for j in 0..ny {
                    let mut sum = 0.0;
                    for k in 0..nz {
            ...
            // ── Residual: b − A·x ──
            // Map measurement vector to projection length (truncate or zero-pad)
            let proj_len = projection.len();
            let mut residual = Array1::zeros(proj_len);
            for idx in 0..proj_len.min(n_meas) {
                residual[idx] = preprocessed[idx] - projection[idx];
            }

            // ── Backprojection: Aᵀ · residual  (spread back along z) ──
