...
use crate::core::constants::BOND_TRANSFORM_FACTOR;
use ndarray::Array2;
...
impl RotationMatrix {
    ...
    pub fn from_euler(phi: f64, theta: f64, psi: f64) -> Self {
        ...
        // ZYX rotation matrix
        r[[0, 0]] = ct * cp;
        r[[0, 1]] = (sp * st).mul_add(cp, -(cs * sp));
        r[[0, 2]] = (cs * st).mul_add(cp, sp * sp);

        r[[1, 0]] = ct * sp;
        r[[1, 1]] = (sp * st).mul_add(sp, cs * cp);
        r[[1, 2]] = (cs * st).mul_add(sp, -(sp * cp));
