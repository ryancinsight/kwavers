...
impl Modulation for FrequencyModulation {
    ...
    fn demodulate(&self, signal: &[f64], t: &[f64]) -> KwaversResult<Vec<f64>> {
        ...
        use crate::domain::signal::analytic::instantaneous_frequency;
        use ndarray::Array1;
        ...
        let demodulated: Vec<f64> = inst_freq
            .iter()
...
mod tests {
    ...
    fn test_fm_hilbert_based_demodulation_produces_finite_values() {
        ...
        let max_abs = demodulated.iter().map(|&x| x.abs()).fold(0.0f64, f64::max);

        assert!(
            max_abs > 1e-10,
            "Demodulated signal should have non-zero content: max = {}",
            max_abs
        );
    }
}
