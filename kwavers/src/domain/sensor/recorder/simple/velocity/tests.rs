...
use crate::domain::sensor::recorder::velocity_statistics::SampledVelocityStats;
...
fn velocity_views_expose_recorded_prefix_without_clone() {
    ...
    // uy / uz not allocated.
    assert_eq!(recorder.uy_data_view().map(|view| view.dim()), None);
    assert_eq!(recorder.uz_data_view().map(|view| view.dim()), None);
...
fn intensity_records_pressure_velocity_product_and_time_average() {
    ...
    assert_eq!(
        recorder.extract_ix_data().as_ref().map(|data| data.dim()),
        Some((1, 2))
    );
