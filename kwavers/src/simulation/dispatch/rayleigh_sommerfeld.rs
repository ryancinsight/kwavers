...
use crate::simulation::dispatch::shared::{next_pow2, trim_initial_recorder_sample};
use crate::simulation::types::{SimulationRunRequest, SimulationRunResult};
use crate::solver::analytical::transducer::{FNMConfig, FastNearfieldSolver};
...
pub fn run(req: &SimulationRunRequest<'_>) -> KwaversResult<SimulationRunResult> {
    ...
    let rho0 = req.medium.density(req.grid.nx / 2, req.grid.ny / 2, req.grid.nz / 2);
    let frequency = req
        .helmholtz
        .and_then(|h| h.frequency)
        .unwrap_or_else(|| {
