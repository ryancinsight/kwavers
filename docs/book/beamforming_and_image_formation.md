...
![Single-element circular-piston directivity](figures/ch04/fig01_element_directivity.png)

*Figure 7.1. Circular-piston element directivity D(θ) = 2J₁(ka sinθ)/(ka sinθ) (§7.1; `circular_piston_directivity`).*

---

## 7.2 Array Factor and the Grating Lobe Theorem

### 7.2.1 Linear Array of N Elements
...
for the negative side; the tighter constraint is (7.9). □

**Corollary 7.2 (λ/2 Rule).** For θ_s = 0 (no steering), the grating-lobe-free condition
...
![Array factor and grating lobes vs pitch](figures/ch04/fig02_array_factor_grating_lobes.png)

*Figure 7.2. Array factor |AF(θ)| and grating-lobe positions for d = λ/2 vs d = λ (§7.2; `beam_pattern_magnitude`, `grating_lobe_angles`).*


### 7.2.2 Beam Pattern (Combined Directivity × Array Factor)
...
![Two-dimensional array beam pattern](figures/ch04/fig05_beam_pattern_2d.png)

*Figure 7.3. Combined 2-D beam pattern P(θ) = D(θ)·AF(θ) (§7.2.2; `beam_pattern_2d_magnitude`).*

---

## 7.3 Focusing

### 7.3.1 Time-Delay Focusing Law

**Theorem 7.3 (Delay Law for a Focused Aperture).** Let element n be at position r_n and
...
*Figure 7.4. Lateral resolution 0.886 λ F# vs focal depth for fixed aperture (§7.3.2).*


### 7.3.3 Axial Resolution
...
### 7.4.1 Window Functions

**Definition 7.3 (Apodization).** Apodization refers to the application of a window
...
**Theorem 7.6 (Sidelobe–Resolution Trade-off).** For any apodization window with spectrum
...
**Corollary 7.3.** In tissue harmonic imaging (Chapter 9), transmit apodization is
...
![Apodization window functions](figures/ch04/fig03_apodization_windows.png)

*Figure 7.5. Apodization windows (rectangular/Hann/Hamming) and the main-lobe-width vs sidelobe-level trade-off (§7.4).*

---

## 7.5 Receive Beamforming: Delay-and-Sum
...
For each image point x_p:
    For each receive element n = 0..N-1:
        τ_n = 2|r_n − x_p| / c₀         (round-trip delay, Eq. 7.11)
...
### 7.5.2 Fourier-Domain Beamforming (f-k Migration)
...
synthetic aperture imaging and coherent plane-wave compounding (Chapter 9).

The f-k (Stolt) migration is implemented for plane-wave imaging in
`kwavers_diagnostics::workflows::fk_migration::fk_stolt_migration`.

---

### 7.5.3 MVDR Adaptive Beamformer

**Definition 7.4 (MVDR Beamformer).** The Minimum Variance Distortionless Response
...
In clinical applications R_xx is regularized: R_xx → R_xx + ε·I with ε ~ 1/30 × λ_max. MVDR/Capon adaptive beamforming is implemented in `kwavers_analysis::signal_processing::beamforming::adaptive::mvdr` (with a 3-D CPU variant).

---

## 7.6 Bandlimited Interpolation (BLI) Source Rasterization
...
## 7.7 Multi-Element Array Configurations
...
### 7.7.3 Annular Array and Axisymmetric Focusing
...
`kwavers_transducer::kwave_array` places rings at half-cell-offset grid origins
with centering convention matching k-Wave (integer N/2 centering).

---

## 7.8 Focused Transducer (Bowl / Arc / Hemispherical)
...
## 7.9 Grating Lobe Validation Protocol
...
1. Compute theoretical grating lobe angles from (7.8): sin θ_GL = sin θ_s + m λ/d
...
4. For each theoretical grating lobe angle θ_GL:
   ...
   b. Compare with AF(θ_GL) × D(θ_GL) prediction (Eq. 7.10).
   c. Require |P_sim − P_theory| / P_main < 0.05.
5. Verify grating lobe is absent when d = λ/2 (Corollary 7.2).
```
...
|---------|---------------|---------------|
| Phased array | `kwavers_transducer::transducers::phased_array` | `PhasedArrayTransducer` |
| Beamforming modes | `…::phased_array::beamforming` | `BeamformingMode::{Focus, Steer, Custom, PlaneWave}` |
| Apodization window | `kwavers_transducer::kwave_array` | `KwaveApodizationWindow` |
| Single-element directivity | `kwavers_physics::analytical::transducer` | `circular_piston_directivity()` |
| Focused bowl | `kwavers_transducer::transducers::focused::bowl` | `BowlTransducer` (`BowlConfig`) |
| Arc / ring | `kwavers_transducer::transducers::focused::arc` | `ArcSource` (`ArcConfig`) |
| Annular array | `kwavers_transducer::kwave_array` | `KWaveArray` (annular geometry) |
| BLI rasterization | `kwavers_transducer::kwave_array::bli_kernel` | `KwaveBli::map_surface_sample()` |
| Delay law (Eq. 7.11) | `kwavers_transducer::kwave_array::accessors::beamforming` | `get_focus_delays()`, `get_element_delays()` |
| Linear array | `kwavers_transducer::basic::linear_array` | `LinearArray` |
| Matrix array | `kwavers_transducer::basic::matrix_array` | `MatrixArray` |
| MVDR adaptive beamformer (§7.5.3) | `kwavers_analysis::signal_processing::beamforming::adaptive::mvdr` | MVDR / Capon |

---

## 7.11 Worked Example: Linear Array Steering and Grating Lobe
...
d = λ/2 = 0.154 mm eliminates the grating lobe (Corollary 7.2).
...
lobe level drops by 18.3 dB (Theorem 7.6).

---

## 7.12 Electronic Steering

**Scope.** This section consolidates the steering physics distributed through §7.2
(array factor and grating lobes) and §7.3 (focusing) into a single treatment of *electronic
...
a linear function of element index. Figure 7.7 shows the delay laws computed by
`delay_law_focus_2d` for a 64-element, `d = λ/2`, 2 MHz array at four steering angles: the
profiles are straight lines whose slope is proportional to `sin θ_s`, confirming Eq. 7.22.
...
*Figure 7.7. Linear-array steering delay laws τ_n vs element index (§7.12).*
...
bandwidth are treated in §7.12.4.

### 7.12.2 Beam Pattern Versus Steering Angle

Substituting the steering ramp into the array factor (Eq. 7.7) shifts the main-lobe peak
from broadside to `θ_s`, because the array factor depends only on the difference
`sin θ − sin θ_s`. Figure 7.8 overlays the full beam pattern `|AF(θ)·D(θ)|` (array factor
times element directivity, Eq. 7.10) for `θ_s ∈ {0°, 15°, 30°, 45°}` at `d = λ/2`: the main
...
*Figure 7.8. Steered beam pattern at d = λ/2, no grating lobes (§7.12).*

Two angle-dependent penalties accompany steering, both visible in Figure 7.8:
...
2. **Steering loss (element-factor roll-off).** The composite pattern is the array factor
   *modulated by the fixed element directivity* `D(θ)` (Eq. 7.3/4.6). Because `D(θ)` peaks
...
### 7.12.3 Grating Lobes and the Design Rule
...
with the `d = λ/2` design (Corollary 7.2) remaining grating-lobe-free across the entire
half-space. Figure 7.9 contrasts `d = λ/2` and `d = λ` at `θ_s = 30°`: the half-wavelength
...
*Figure 7.9. Grating-lobe onset: d = λ/2 (free) vs d = λ at θ_s = 30° (§7.12.3).*
...
### 7.12.4 Beam Squint Under Finite Bandwidth

Phase-only steering (Eq. 7.23) is exact only at the design frequency `ω₀`. A pulse of
...
seconds (Eq. 7.11/4.22) and applied to the broadband signal, not as a single-frequency
...
### 7.12.5 Two-Dimensional and Volumetric Steering
...
steer-and-focus, the basis of volumetric (4-D) imaging (§7.7.2). Figure 7.10 shows the
2-D steered-and-focused continuous-wave field computed by `beam_pattern_2d_magnitude` for a
focus placed on the **natural-focus arc** (the near-field transition range `N`, §7.12.6)
...
*Figure 7.10. Steered + focused CW field on the natural-focus arc (§7.12.6).*
...
### 7.12.6 Focusing Around the Natural Focus
...
electronic focusing: for a focal range `z_f ≲ N` the delay law (Eq. 7.11) tightens the beam
...
Figure 7.11 shows the on-axis pressure for focal ranges `0.5 N`, `N`, and `1.5 N`: the
...
*Figure 7.11. On-axis focal-gain saturation as z_f → N (§7.12.6).*
...
traces the focus along the arc shown in Figures 4.15–7.16, which is exactly the regime in
which electronic steer-and-focus is physically effective.

### 7.12.7 Steering in kwavers

| Steering concept | kwavers module | Key item |
|------------------|----------------|----------|
| Linear/matrix steer + focus delays (Eq. 7.11/4.22/4.27) | `kwavers_transducer::transducers::phased_array::beamforming` | `BeamformingMode` |
| Delay-law accessor (k-Wave parity) | `kwavers_transducer::kwave_array::accessors::beamforming` | `get_focus_delays()` / `get_element_delays()` |
| Hemispherical / focused-bowl steering | `kwavers_transducer::hemispherical::steering` | `SteeringController`, `SteeringMode`, `FocalPoint` |
| Aberration-corrected focal law (heterogeneous media) | `kwavers_therapy::therapy::theranostic_guidance::waveform::eikonal` | `eikonal_travel_time()` |
| Apodization (steered sidelobe control, §7.4) | `kwavers_transducer::transducers::apodization` | window enum |

In homogeneous media the steering delay is the geometric path law (Eq. 7.11). In
...
the per-element delay becomes
`τ_n = (T(r_n) − min_m T(r_m))`, where `T` solves `|∇T| = 1/c(r)`. This is *aberration-corrected
electronic steering*; it reduces to Eq. 7.11 when `c(r) = c₀` is uniform.

### 7.12.8 Validation

The figures in this section are generated by `crates/kwavers-python/examples/book/ch04_electronic_steering.py`,
...
grating-lobe prediction (Figure 7.9, `θ_GL = −30°` at `θ_s = 30°`, `d = λ`) matches Eq. 7.8
exactly, and the `d = λ/2` array is grating-lobe-free for all `θ_s`, confirming Corollary 7.2.
The steered delay profiles (Figure 7.7) are linear in element index with slope `∝ sin θ_s`,
confirming the far-field ramp of Eq. 7.22. The natural-focus length `N = D²/(4λ)` (Figure 7.11)
and the on-axis focal-gain saturation as `z_f → N` are verified by Rust unit tests in
`kwavers_physics::analytical::transducer` (`near_field_natural_focus_formula`,
`steering_focus_point_traces_natural_focus_arc`, `delay_law_steer_focuses_on_arc`). The
grating-lobe acceptance test of Algorithm 7.2 provides the corresponding numerical
...
localization. These topics are addressed in Chapter 9 (Diagnostic Ultrasound Imaging) and their
deeper treatment in `kwavers_analysis::signal_processing::beamforming`,
`kwavers_diagnostics`, and `kwavers_analysis::ml`.
