...
*Figure 22.1. Emission spectra (Definition 22.1): stable cavitation shows discrete harmonics + sub-/ultra-harmonics; inertial cavitation adds a 15–25 dB broadband floor. The plotted spectra are generated by Rust/PyO3 `normalized_cavitation_emission_spectrum`; Python applies only dB scaling and rendering.*
...
*Figure 22.3. DAS point-spread sensitivity (Theorem 22.1): Rust/PyO3 `passive_cavitation_point_source_rf` synthesizes the delayed point-source RF traces, and `passive_acoustic_map_das` reconstructs the energy map. The −6 dB lateral width $\delta_\perp \approx 0.886\,\lambda z/D$ broadens with depth $z$.*
...
`kwavers_analysis::signal_processing::pam::DelayAndSumPAM`.  The figure's
deterministic point-source receive traces are generated by Rust/PyO3
`passive_cavitation_point_source_rf`, which owns receive delay, Gaussian
emission envelope, carrier phase, and `1/r` spreading. The PyO3 function
...
This keeps the signal-generation and signal-processing contracts in Rust while
the Python layer provides only ABI conversion and plotting.
...
*Figure 22.5. Cross-spectral-matrix singular values (Theorem 22.2): Rust/PyO3 `eigenspace_covariance_eigenvalues` generates the deterministic $K$ source eigenvalues ($\sigma_s^2+\sigma_n^2$) and $N-K$ noise eigenvalues ($\sigma_n^2$), defining the signal-subspace projector $\mathbf{P}_S$.*
