...
```powershell
python crates/kwavers-python/examples/book/ch30_intravascular_ultrasound.py
```
...
The transducer-technology trade-off behind a flexible IVUS imaging element —
CMUT vs PMUT vs bulk piezo — is analysed quantitatively in **Chapter 33
(§33.8)**, where the kwavers `mems` models score CMUT as the IVUS imaging winner
(high bandwidth / fluid-loading match) while a bulk-piezo/PMUT element wins the
high-pressure therapy regime (§33.9).

## 29.3 Mathematical Contract
...
`alpha(f)` is tissue attenuation, and `r_c` is catheter radius. The
deterministic vessel anatomy, tissue-property maps, impedance-gradient
reflectivity, and speckle field are generated by Rust `kw.ivus_vessel_phantom`;
Python adapts the returned arrays for plotting. A Gaussian radial pulse kernel
models finite axial pulse length before logarithmic compression over a fixed
`60 dB` display range.
...
Generated by `crates/kwavers-python/examples/book/ch30_intravascular_ultrasound.py` into
`docs/book/figures/ch30/`:

![Dataset contract and analytic vessel anatomy.](figures/ch30/fig01_dataset_and_anatomy.png)

*Figure 29.1. Dataset contract and the Rust-generated analytic 384×384 vessel phantom (lumen, media–adventitia, fibrous cap, lipid) used in place of redistributed clinical IVUS frames.*

![Dual-frequency catheter transducer design.](figures/ch30/fig02_transducer_design.png)

*Figure 29.2. Transducer design: the 64-element 20 MHz circumferential imaging ring, guidewire lumen, and the side-looking 1.5 MHz therapy sector.*

![IVUS B-mode simulation: A-line frame and scan-converted image.](figures/ch30/fig03_ivus_bmode_simulation.png)

*Figure 29.3. IVUS B-mode (§Mathematical Contract): radial A-line frame and the scan-converted, log-compressed B-mode with recovered lumen and vessel-wall contours.*

![Microbubble therapy deposition maps.](figures/ch30/fig04_microbubble_therapy_map.png)

*Figure 29.4. Localized therapy: pressure, ARF-weighted delivery fraction, and temperature maps confined to the plaque/fibrous-cap sector.*

![Intravascular usage sequence.](figures/ch30/fig05_intravascular_usage_sequence.png)

*Figure 29.5. Usage sequence: catheter crossing, IVUS imaging, localized sector therapy, and the post-treatment check.*

## 29.5 Research Alignment
