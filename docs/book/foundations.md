...
construction; this contrast motivates the pseudospectral method.  The
travelling-pulse profile `g` and the d'Alembert reference are generated by Rust
`kw.gaussian_modulated_pulse_1d` and `kw.dalembert_split_solution_1d`; Python
only invokes the solver binding and renders the returned arrays.
