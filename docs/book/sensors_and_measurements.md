...
**Figure 8.1.** Circular-hydrophone receive directivity H(θ) = 2J₁(ka sinθ)/(ka sinθ),
computed by Rust `kw.circular_piston_directivity`, for
ka = 0.5, 1, 2, 5. Smaller elements (ka ≤ 1) stay near-omnidirectional; larger elements
...
u = p/(ρc) (in phase), with both fields generated by Rust/PyO3
`plane_wave_pressure_velocity_1d` and the ratio fixed by the specific acoustic
impedance Z = ρc.
...
with seeded measurement noise generated by Rust `kw.add_noise`,
...
| B-mode scan lines (raw) | 0.931748 (verified) | 0.982796 |
| Harmonic content (phased array) | 0.97–1.03 | ≥ 0.9968 |
| Photoacoustic initial pressure | 0.99–1.01 | ≥ 0.999 |

The raw B-mode scan-line contract is the physics parity oracle. The
log-compressed fundamental and harmonic display panels remain visualization-only
normalization residuals and are tracked separately from solver validation.
