...
*Figure 10.1. Photoacoustic pressure transient from a uniformly absorbing sphere (N-wave, Xu & Wang 2006), computed by `kw.pa_sphere_pressure_signal`. The bipolar shape is the time-domain signature of the spherical-shell Green's function (§10.3.2).*

---

## 10.1 Photoacoustic Signal Generation: Thermoelastic Mechanism

### 10.1.1 Absorbed Optical Energy Density
...
### 10.1.4 Initial Pressure Under Dual Confinement

**Theorem 10.1 (Initial Pressure).** *Under stress and thermal confinement, the initial acoustic pressure generated by absorbed optical energy density $H(\mathbf{r})$ is*
...
## 10.2 The Grüneisen Parameter

### 10.2.1 Definition and Thermodynamic Derivation

**Theorem 10.2 (Grüneisen Parameter).** *The Grüneisen parameter satisfies*
...
**Proof.** From the definition $\Gamma = \beta c_s^2 / C_p$ (established in Theorem 10.1), write the adiabatic compressibility as $\kappa_S = 1/(\rho c_s^2)$. The standard thermodynamic identity relating adiabatic and isothermal compressibilities is
...
*Figure 10.2. Grüneisen parameter Γ(T) of water from 0–100 °C (`kw.gruneisen_parameter_water`, Sigrist & Kneubühl 1978). The rise of Γ with T makes PA amplitude a tissue thermometer (Eq. 2.6), the basis of `GrueneisenModel`.*


### 10.2.3 Tissue Grüneisen Values
...
## 10.3 The Photoacoustic Wave Equation

### 10.3.1 Derivation from Thermoelastic Sources

**Theorem 10.3 (PA Wave Equation).** *In a homogeneous, lossless fluid with spatially uniform sound speed $c_s$ and thermoelastic source $H(\mathbf{r},t)$, the acoustic pressure satisfies*
...
## 10.4 Optical Absorption and Transport

### 10.4.1 Beer-Lambert Law
...
### 10.4.2 Optical Diffusion in Scattering Tissue
...
**Theorem 10.5 (Effective Attenuation).** *In a homogeneous, infinite medium under isotropic illumination, the fluence rate decays as $\Phi(r) \propto e^{-\mu_{\text{eff}} r}/r$ with effective attenuation coefficient*
...
*Figure 10.3. Molar absorption spectra of HbO₂ (solid) and Hb (dashed), 450–1000 nm, from `kw.hbo2_molar_absorption` / `kw.hb_molar_absorption` (Prahl 1999 fits). The isosbestic point near 797 nm and the 760/850 nm working pair (§10.5.3) are visible.*

---

## 10.5 Spectroscopic Unmixing and Oxygen Saturation Estimation

### 10.5.1 Linear Mixing Model
...
### 10.5.2 Two-Wavelength Oxygen Saturation

**Theorem 10.6 (Linear Unmixing).** *Given PA measurements at two wavelengths $\lambda_1$, $\lambda_2$ and the $2\times 2$ extinction matrix*
...
## 10.6 Image Reconstruction: Universal Back-Projection
...
### 10.6.2 Universal Back-Projection Formula

**Theorem 10.7 (Universal Back-Projection, Xu & Wang 2005).** *Let $\Omega_0$ be the full solid angle subtended by the detector surface $S_0$ as seen from any interior point. The initial pressure distribution is recovered by*
...
## 10.7 Time-Reversal Reconstruction

### 10.7.1 Principle and Setup
...
## 10.8 Frequency-Domain Analysis and Signal Bandwidth

### 10.8.1 PA Signal Bandwidth
...
**Theorem 10.9 (Bandwidth–Resolution Relation).** *For a spherical absorber of diameter $d$ in a medium with sound speed $c_s$, the PA pressure signal has dominant bandwidth*
...
*Figure 10.5. PA axial resolution δz = c/(2·BW) vs absorber radius / signal bandwidth (Xu & Wang 2006), illustrating B ≈ c_s/d (Theorem 10.9): smaller absorbers radiate broader-band signals and are resolved more finely.*


### 10.8.2 Signal-to-Noise Ratio
...
## 10.9 Photoacoustic Microscopy: OR-PAM and AR-PAM

### 10.9.1 Operating Regimes
...
### 10.9.2 Resolution Proof

**Theorem 10.10 (Rayleigh Resolution in PA Microscopy).** *The lateral spatial resolution of OR-PAM is bounded below by $0.51\lambda_{\text{opt}}/\text{NA}_{\text{opt}}$ and of AR-PAM by $0.71\lambda_{\text{us}}/\text{NA}_{\text{us}}$.*
...
## 10.10 Fluence Compensation and Quantitative PA Imaging

### 10.10.1 The Fluence Artifact
...
**Theorem 10.11 (Depth Bias).** *In a homogeneous medium with effective attenuation $\mu_{\text{eff}}$, the apparent absorption coefficient inferred from the raw PA signal without fluence compensation is*
...
**Theorem 10.12 (Convergence of Iterative qPA).** *The iterative fluence compensation algorithm converges to the true $\mu_a(\mathbf{r})$ when the forward diffusion map $\mu_a \mapsto F[\mu_a]$ is a contraction in the $L^2$ norm, i.e., when $\|dF/d\mu_a\|_{L^2} < 1$.*
...
### 10.10.3 Model-Based Quantification
...
## 10.11 Numerical Implementation in kwavers

### 10.11.1 Module Organization
...
└── HemoglobinDatabase                  # HbO₂/Hb molar extinction (§10.4.4)

kwavers_analysis::signal_processing
└── spectroscopy::SpectralUnmixer       # Tikhonov-regularized unmixing (§10.5)
```
...
### 10.11.3 Reconstruction Dispatch
...
- Bandwidth estimation to verify sensor frequency coverage matches absorber size (Theorem 10.9)
