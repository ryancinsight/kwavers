...
## 6.10 Aperture-Driven Array Design Synthesis

The preceding sections take a *fixed* element count and derive the resulting beam
(directivity §6.3, steering and grating lobes §6.9). Clinical array design poses the
**inverse** problem: given an overall **aperture** and an **operating frequency**,
choose the element layout — count, pitch, kerf, and channel wiring — that meets a
grating-lobe budget.

### Statement

For an aperture of extent `A` on a steered axis, wavelength `λ = c/f`, and a maximum
...
The choice `q = ½` is the λ/2 spatial-Nyquist criterion (grating-lobe condition §6.9,
spatial sampling §8.3): it makes the steered axis grating-lobe-free over the full
steering range. `q = 1` (λ pitch) halves the channel count but admits grating lobes
away from broadside. Each element spans `p − kerf` with `kerf = κ·p` (gap fraction
`κ`), so the areal **fill factor** of an equal-kerf 2-D cell is `(1 − κ)²`.
...
`kwavers_transducer::design` solves this synthesis in closed form.
`design_array(&ApertureDesignSpec{…})` returns an `ArrayDesign` carrying `nx`, `ny`,
both pitches, kerf, element size, `n_channels`, and a `grating_lobe_free` flag
(`is_grating_lobe_free`, the λ/2 test). `ArrayDesign::element_positions` and
...
## 6.11 Acoustic Lenses: Static, Dynamic, and Adaptive Focusing

Focusing can be imposed **passively** by a refractive lens or **actively** by the
phased-array delay law. Both curve the wavefront by the same focusing phase; they
differ only in whether that phase is fixed in material or reconfigured electronically.

### 6.11.1 Static refractive lens (lensmaker's equation)

A single-surface acoustic lens of radius of curvature `R` made of a material with
...
Because solids/elastomers used for lenses are typically **slower** than tissue
(silicone `c_lens ≈ 1000 m/s` < `c_m ≈ 1540 m/s`), a **concave** (`R > 0`) surface
converges — the opposite of an optical glass lens, where the lens is faster than
air. The f-number is `F/D` for aperture `D`. This is
`kwavers_transducer::transducers::physics::materials::AcousticLens`
(`focal_length`, `f_number`, `silicone`/`Polyurethane` materials; the underlying
refraction/critical-angle primitives are `analytical::propagation::SnellLawCalculator`).

### 6.11.2 A lens is a passive delay law
...
which is **exactly** the per-element focusing delay `τ_i` of the phased-array delay
law (§6.5) evaluated on the continuous aperture. A fixed lens is therefore a
*passive, non-steerable* delay law: it focuses at one depth `F` and one (on-axis)
direction only. `AcousticLens::aperture_delay_profile` returns this `τ(r)`, making
the lens↔delay-law equivalence explicit and testable (the paraxial `r²/(2c_mF)`
limit is asserted in the unit tests).

### 6.11.3 Dynamic and adaptive lensing

The phased array is the **reconfigurable** lens: choosing the per-element delays
`τ_i` electronically (§6.5) sets the focal depth and steering angle *per transmit*,
so the same aperture realizes a continuum of lenses:

- **Dynamic focusing / steering** — vary `τ_i` to move `F` (depth-dependent focus,
  e.g. dynamic receive focusing) or to tilt the wavefront (`sinθ_s`, §6.9). The
  static lens's fixed `τ(r)` is the `θ_s = 0`, single-`F` special case.
- **Adaptive focusing** — in an aberrating medium the homogeneous delay law no
  longer focuses; the correction adds a per-element phase from the medium's
  travel-time map (skull phase correction, Ch24 §24.3 / Ch26), i.e. an
  *aberration-correcting lens* synthesized from CT/measured data.

Graded-sound-speed (GRIN) focusing — continuous refraction rather than a single
...
## 6.13 Validation Against k-Wave
...
## 6.14 Figure References

Figures 6.1–6.5 (embedded in §6.3–§6.9) are generated by
`crates/kwavers-python/examples/book/ch11_sources_and_transducers.py`, which computes every quantity
...
rendering. The k-Wave parity results quoted in §6.13 are produced by the `at_focused_bowl_3D`,
