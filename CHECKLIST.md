...
- [x] Migrate fixed-acquisition and batch reconstruction APIs without keeping
      scalar compatibility paths; extract seconds only at numerical operators.
- [x] Preserve velocity/grid configuration and dense image storage boundaries;
...
- [x] Make non-finite and non-positive typed inputs return a typed error;
      migrate co-located tests and preserve migration value semantics.
- [x] Synchronize ADR 083, gap-audit, backlog, checklist, changelog, and the
      Atlas cross-repository audit; run focused package gates.
...
Evidence: diagnostics test-target check passes; focused Nextest passes 3/3 with
196 skipped; warning-denied Clippy, doctests, RustDoc, formatting, and diff
checks pass. The final public signature scan finds only typed physical inputs.
...
- [x] Type workflow latency configuration, acquisition/processing durations,
...
- [x] Remove synthetic GPU/memory samples and record individual stage intervals;
      migrate callers/tests and correct the neural timing base-unit conversion.
- [x] Synchronize ADR 078, `gap_audit.md`, backlog, checklist, changelog, and
...
## Owner: Codex — KWAVERS-AEQ-MET-41 typed stereotactic targeting quantities [major] — done 2026-07-31

- [x] Type AP/ML/DV stereotactic coordinates, Bregma, confidence, and targeting
      distance with Aequitas; preserve the atlas millimetre conversion boundary.
- [x] Migrate targeting callers/tests and add value-semantic coordinate,
      trajectory, validation, distance, and confidence regressions.
- [x] Synchronize ADR 080, `gap_audit.md`, backlog, checklist, changelog, and
      the Atlas cross-repository audit; run focused package gates and preserve
      peer-owned changes.
...
Evidence: package test-target check passes; focused targeting Nextest passes
10/10 with 184 skipped; warning-denied all-target Clippy, doctests (1
executable, 5 ignored), RustDoc, formatting, and diff checks pass. Shared
unused-provider-patch and linker warnings remain outside this item.
...
- [x] Restore the provider-first HIFU contracts and migrate all current tests
      and callers without a scalar compatibility facade.
- [x] Run focused format, check, Clippy, Nextest, doctest, and audit checks;
      preserve the active peer math/physics work.
- [x] Commit and push the verified correction.
...
- [x] Migrate validated thermal property accessors and the justified perfusion
      boundary without a compatibility facade; update all callers.
- [x] Add positive, invalid, and unit-sensitive value-semantic regressions.
- [x] Add/update ADR, child gap audit, changelog, and Atlas audit.
- [x] Run locked package checks and Nextest: `kwavers-medium` 191/191 and
      thermal/bubble physics 361/361. Package checks pass; warning-denied
      Clippy remains blocked by the three pre-existing `kwavers-math` findings.
- [x] Commit and push the verified increment, preserving peer-owned dirt.
