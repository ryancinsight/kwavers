            ...
            for j in 0..grid.ny {
                for k in 0..grid.nz {
                    ...
                    if !density_val.is_finite() || density_val <= 0.0 {
                        return Err(format!("Invalid density at ({}, {}, {}): {}", i, j, k, density_val));
                    }
                    if !speed_val.is_finite() || speed_val <= 0.0 {
                        return Err(format!("Invalid sound speed at ({}, {}, {}): {}", i, j, k, speed_val));
                    }
                    if !abs_val.is_finite() || abs_val < 0.0 {
                        return Err(format!("Invalid absorption at ({}, {}, {}): {}", i, j, k, abs_val));
                    }
                    if !nl_val.is_finite() {
                        return Err(format!("Invalid nonlinearity at ({}, {}, {}): {}", i, j, k, nl_val));
                    }
                }
            }
        }
...
pub mod testing_utilities {
    pub use super::{acoustic_properties, medium_properties, grid_properties};
}