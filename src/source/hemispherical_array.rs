...
impl HemisphericalArray {
    ...
    pub fn new(
        ...
        let elements = Self::generate_hemisphere_elements(
            radius,
            element_spacing,
            wavelength,
        )?;
    ...
    fn select_spiral_sparse(&mut self, pitch: f64) -> KwaversResult<()> {
        ...
        for (i, element) in self.elements.iter().enumerate() {
            ...
            if angle_diff < PI / 10.0 { // Within 18 degrees of spiral
    ...
    fn select_aperture_based(&mut self, angle: f64) -> KwaversResult<()> {
        ...
        for (i, element) in self.elements.iter().enumerate() {
            ...
            let target_mag = (target_dir[0].powi(2) + 
                             target_dir[1].powi(2) + 
                             target_dir[2].powi(2)).sqrt();
    ...
    fn select_adaptive(&mut self) -> KwaversResult<()> {
        ...
        if self.active_elements.len() < min_elements {
            ...
            let mut efficiencies: Vec<(usize, f64)> = self.elements
                .iter()
                .enumerate()
    ...
    fn calculate_element_efficiency(
        &self,
        ...
        let cos_angle = -(element.normal[0] * dir[0] +
                         element.normal[1] * dir[1] +
                         element.normal[2] * dir[2]);
    ...
    pub fn calculate_phase_delays(&mut self) -> KwaversResult<()> {
        ...
        let min_phase = self.phase_delays
            .iter()
            .enumerate()
    ...
    fn calculate_grating_lobe_positions(&self) -> Vec<[f64; 3]> {
        ...
        if avg_spacing > self.wavelength {
            ...
            for m in -grating_order_max..=grating_order_max {
                if m == 0 { continue; } // Skip main lobe
            ...
            for j in 0..ny {
                for k in 0..nz {
                    ...
                    let x = (i as f64 / (nx - 1) as f64 - 0.5) * 
                           2.0 * self.treatment_envelope.steering_range[0];
                    let y = (j as f64 / (ny - 1) as f64 - 0.5) * 
                           2.0 * self.treatment_envelope.steering_range[1];
                    let z = (k as f64 / (nz - 1) as f64 - 0.5) * 
                           2.0 * self.treatment_envelope.steering_range[2];
...
mod tests {
    ...
    fn test_sparse_selection() {
        let signal = Arc::new(SineWave::new(650e3, 1.0, 0.0));
        let mut array = HemisphericalArray::new(
            0.15,
            650e3,
            1.15e-3,
            signal,
        ...
        array.apply_sparse_selection(
            SparseSelectionStrategy::Random { density: 0.5 }
        ).unwrap();
    ...
    fn test_efficiency_calculation() {
        ...
        array.apply_sparse_selection(
            SparseSelectionStrategy::EfficiencyBased { threshold: 0.7 }
        ).unwrap();
