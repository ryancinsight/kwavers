...
impl PhasedArrayTransducer {
    ...
    pub fn new(
        ...
        let mut transducer = Self {
            ...
            elements,
            signal,
            beamforming_mode: BeamformingMode::Focus {
                target: default_focus,
            },
            sound_speed,
        ...
        for i in 0..n {
            for j in 0..n {
                if i == j {
                    ...
                    let distance =
                        Self::distance(self.elements[i].position, self.elements[j].position);
...
impl Source for PhasedArrayTransducer {
    ...
    fn get_source_term(&self, t: f64, x: f64, y: f64, z: f64, _grid: &Grid) -> f64 {
        ...
        for element in &self.elements {
            ...
            // Time-delayed signal
            let time_delay =
                element.phase_delay / (2.0 * std::f64::consts::PI * self.config.frequency);
            ...
            for (i, element) in self.elements.iter().enumerate() {
                for (j, other_element) in self.elements.iter().enumerate() {
                    if i != j {
                        ...
                        let spatial_response =
                            self.calculate_element_response(other_element, x, y, z);
                        let time_delay = other_element.phase_delay
                            / (2.0 * std::f64::consts::PI * self.config.frequency);
                        let delayed_time = t - time_delay;
                        let temporal_response =
                            self.signal.amplitude(delayed_time) * other_element.amplitude_weight;
...
impl PhasedArrayTransducer {
    ...
    fn calculate_element_response(
        &self,
        element: &TransducerElement,
        x: f64,
        y: f64,
        z: f64,
    ) -> f64 {
...
mod tests {
    ...
    fn test_focus_beamforming() {
        ...
        let result = array.set_beamforming_mode(BeamformingMode::Focus {
            target: focus_target,
        });
        assert!(result.is_ok());
