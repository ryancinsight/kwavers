...
impl CalibrationManager {
    ...
    pub fn triangulate_position(
        ...
        let decomp = at_a.lu();
        let solution = decomp.solve(&at_b)
    ...
    fn initialize_kalman_filter(&mut self, num_elements: usize, measurement_noise: f64) -> KwaversResult<()> {
        ...
        // Measurement noise
        let measurement_noise = DMatrix::identity(meas_dim, meas_dim) * measurement_noise * measurement_noise;
    ...
    fn kalman_filter_update(
        ...
        let kalman = self.kalman_state.as_mut()
        ...
        let k = &kalman.covariance * h_matrix.transpose() * s.try_inverse()
            .ok_or(crate::error::KwaversError::Numerical(
                crate::error::NumericalError::SolverFailed {
    ...
    /// Extract peak locations from pressure field
    fn extract_peaks(&self, pressure_field: &Array3<f64>, wavelength: f64) -> KwaversResult<Vec<[f64; 3]>> {
        let (nx, ny, nz) = pressure_field.dim();
        let mut peaks = Vec::new();
        
        // Peak detection threshold (based on RMS of field)
        let rms: f64 = pressure_field.iter().map(|&x| x * x).sum::<f64>().sqrt() 
            / (nx * ny * nz) as f64;
                ...
                for k in min_separation..(nz - min_separation) {
                    ...
                    if val > threshold {
                        ...
                        for di in 0..=2 {
                            for dj in 0..=2 {
                                for dk in 0..=2 {
                                    if di == 1 && dj == 1 && dk == 1 { continue; }
                                    let neighbor = pressure_field[[i+di-1, j+dj-1, k+dk-1]].abs();
                                    if neighbor > val {
                                        is_max = false;
                                        break;
                                    }
        ...
        for (i, peak) in peaks.iter().enumerate() {
            ...
            for (j, reflector) in reflectors.iter().enumerate() {
                let dist = ((peak[0] - reflector[0]).powi(2) 
                    ...
                    + (peak[2] - reflector[2]).powi(2)).sqrt();
