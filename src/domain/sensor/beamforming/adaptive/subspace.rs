...
/// use kwavers::sensor::beamforming::adaptive::subspace::MUSIC;
/// use kwavers::sensor::beamforming::adaptive::SourceEstimationCriterion;
/// use ndarray::Array2;
/// use num_complex::Complex64;
///
...
mod tests {
    ...
    fn test_eigenspace_mv() {
        ...
        let weights = super::super::conventional::BeamformingAlgorithm::compute_weights(
            &beamformer,
            &cov,
            &steering,
        );
    ...
    fn test_eigenspace_mv_vs_mvdr() {
        ...
        let mvdr = super::super::algorithms_old::MinimumVariance::default();
        let espmv = EigenspaceMV::new(n); // Full rank = equivalent to MVDR

        let weights_mvdr = super::super::algorithms_old::BeamformingAlgorithm::compute_weights(
            &mvdr, &cov, &steering,
        );
    ...
    fn test_music_with_source_estimation() {
        ...
        let msg_aic = format!("{err_aic:?}");
        let msg_mdl = format!("{err_mdl:?}");
        assert!(
            msg_aic.contains("UnsupportedOperation"),
            "unexpected error: {msg_aic}"
        );
        assert!(
            msg_mdl.contains("UnsupportedOperation"),
            "unexpected error: {msg_mdl}"
        );
    }
}
