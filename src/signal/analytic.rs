...
#[must_use]
pub fn hilbert_transform(signal: &Array1<f64>) -> Array1<Complex64> {
    let n = signal.len();

    // Edge case: empty or single-sample signal
    if n <= 1 {
        return signal.iter().map(|&x| Complex64::new(x, 0.0)).collect();
    }

    // Convert to complex and compute FFT using central cache
    let mut complex_signal = crate::fft::fft_1d_array(signal);

    // Apply Hilbert transform in frequency domain
    // Per Marple (1999): H[x] = -i * sign(ω) * X(ω)
    // Implemented as: zero negative frequencies, double positive frequencies
    for (i, complex_val) in complex_signal.iter_mut().enumerate() {
        if i == 0 || (n % 2 == 0 && i == n / 2) {
    ...
    crate::fft::ifft_1d_complex(&complex_signal)
}
