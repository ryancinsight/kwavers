...
impl AmplitudeModulation {
    pub fn new(
        ...
        assert!(carrier_frequency >= MIN_CARRIER_FREQUENCY, "Carrier frequency too low");
        assert!(carrier_amplitude > 0.0, "Carrier amplitude must be positive");
    ...
    pub fn with_modulation_index(mut self, index: f64) -> Self {
        assert!(index > 0.0 && index <= MAX_MODULATION_INDEX, "Invalid modulation index");
...
impl FrequencyModulation {
    pub fn new(
        ...
        assert!(carrier_frequency >= MIN_CARRIER_FREQUENCY, "Carrier frequency too low");
        assert!(carrier_amplitude > 0.0, "Carrier amplitude must be positive");
        assert!(frequency_deviation > 0.0, "Frequency deviation must be positive");
...
impl Signal for FrequencyModulation {
    fn amplitude(&self, t: f64) -> f64 {
        ...
        let total_phase = 2.0 * PI * self.carrier_frequency * t + phase_modulation + self.carrier_phase;
...
impl Signal for QuadratureAmplitudeModulation {
    ...
    fn duration(&self) -> Option<f64> {
        ...
        match (self.in_phase_signal.duration(), self.quadrature_signal.duration()) {
...
impl PulseWidthModulation {
    pub fn new(
        ...
        assert!(carrier_frequency > 0.0, "Carrier frequency must be positive");
    ...
    pub fn with_duty_cycle_range(mut self, min: f64, max: f64) -> Self {
        ...
        assert!(max > min && max <= 1.0, "Max duty cycle must be in (min, 1]");
...
mod tests {
    ...
    fn test_amplitude_modulation() {
        ...
        let am = AmplitudeModulation::new(1000.0, 1.0, modulating)
            .with_modulation_index(0.5);
        
        // Check that amplitude varies between 0.5 and 1.5
        let samples: Vec<f64> = (0..100)
            .map(|i| am.amplitude(i as f64 * 0.0001))
            .collect();
