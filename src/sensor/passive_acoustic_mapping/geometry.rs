...
impl ArrayGeometry {
    ...
    pub fn element_positions(&self) -> Vec<[f64; 3]> {
        match self {
            ...
            ArrayGeometry::Hemispherical { elements_theta, elements_phi, radius, center } => {
                let mut positions = Vec::with_capacity(elements_theta * elements_phi);
                
                for i in 0..*elements_theta {
                    let theta = std::f64::consts::PI * 0.5 * i as f64 / (*elements_theta as f64 - 1.0);
    ...
    pub fn num_elements(&self) -> usize {
        match self {
            ...
            ArrayGeometry::Hemispherical { elements_theta, elements_phi, .. } => elements_theta * elements_phi,
