...
//!
//! # Intended use (B: narrowband / adaptive localization)
...
//! - This module lives under `crate::sensor::beamforming` and is intended to be used by
//!   localization orchestrators (e.g., `sensor::localization::beamforming_search`) without
//!   reimplementing adaptive beamforming math in localization.
//!
...
use crate::sensor::beamforming::covariance::CovarianceEstimator;
use crate::sensor::beamforming::narrowband::snapshots::{
    extract_complex_baseband_snapshots, extract_narrowband_snapshots, BasebandSnapshotConfig,
    SnapshotSelection,
};
use crate::sensor::beamforming::narrowband::steering_narrowband::NarrowbandSteering;
use crate::sensor::beamforming::{SteeringVector, SteeringVectorMethod};
...
pub fn capon_spatial_spectrum_point(
    ...
    let r_inv = crate::solver::linear_algebra::LinearAlgebra::matrix_inverse(&r)?;
...
/// SSOT `crate::solver::linear_algebra::LinearAlgebra::solve_linear_system_complex`.
pub fn capon_spatial_spectrum_point_complex_baseband(
    ...
    let selection = cfg.snapshot_selection.clone().unwrap_or_else(|| {
        SnapshotSelection::Auto(
            crate::sensor::beamforming::narrowband::snapshots::SnapshotScenario {
                frequency_hz: cfg.frequency_hz,
                sampling_frequency_hz,
                // Conservative default: modestly narrowband if not specified by caller.
    ...
    let y = crate::solver::linear_algebra::LinearAlgebra::solve_linear_system_complex(&r, &a)?;
    ...
    fn capon_spectrum_is_finite_for_simple_case() {
        ...
        let cfg = CaponSpectrumConfig {
            ...
            covariance: CovarianceEstimator {
                ...
                num_snapshots: 1,
                post_process: crate::sensor::beamforming::covariance::CovariancePostProcess::None,
            },
