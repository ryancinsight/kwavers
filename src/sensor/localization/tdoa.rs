...
impl TDOAProcessor {
        ...
        for iteration in 0..MAX_ITERATIONS {
            ...
            let delta = match self.solve_3x3(jtj, jtr) {
                ...
                Err(_) => {
                    ...
                    log::warn!("TDOA: Singular matrix at iteration {}, returning current estimate", iteration);
                    break;
                }
            };
    ...
    fn compute_residuals_and_jacobian(&self, position: &Position, array: &SensorArray) -> (Vec<f64>, Vec<[f64; 3]>) {
        ...
        for measurement in &self.measurements {
            ...
            let dx_meas = if d_meas > 1e-10 { (position.x - meas_pos.x) / d_meas } else { 0.0 };
            let dy_meas = if d_meas > 1e-10 { (position.y - meas_pos.y) / d_meas } else { 0.0 };
            let dz_meas = if d_meas > 1e-10 { (position.z - meas_pos.z) / d_meas } else { 0.0 };
            
            let dx_ref = if d_ref > 1e-10 { (position.x - ref_pos.x) / d_ref } else { 0.0 };
            let dy_ref = if d_ref > 1e-10 { (position.y - ref_pos.y) / d_ref } else { 0.0 };
            let dz_ref = if d_ref > 1e-10 { (position.z - ref_pos.z) / d_ref } else { 0.0 };
