...
//!
//! - **Beamforming algorithms & numerics** are owned by `crate::sensor::beamforming`.
...
//! - All candidate point scoring is performed via shared `crate::sensor::beamforming` SSOT APIs.
    ...
    fn score_point(&self, sensor_data: &Array3<f64>, point: [f64; 3]) -> KwaversResult<f64> {
        match &self.cfg.method {
            config::LocalizationBeamformingMethod::SrpDasTimeDomain { delay_reference } => {
                ...
                let out = crate::sensor::beamforming::delay_and_sum_time_domain_with_reference(
                    sensor_data,
            ...
            config::LocalizationBeamformingMethod::CaponMvdrSpectrum {
                ...
                let p = match covariance_domain {
                    config::MvdrCovarianceDomain::ComplexBaseband {
                        ...
                        let cfg = crate::sensor::beamforming::CaponSpectrumConfig {
                            frequency_hz: *frequency_hz,
                            sound_speed: self.processor.config.sound_speed,
                            diagonal_loading: *diagonal_loading,
                            covariance: crate::sensor::beamforming::CovarianceEstimator::default(),
                        ...
                        let p = crate::sensor::beamforming::capon_spatial_spectrum_point_complex_baseband(
                            sensor_data,
                    ...
                    config::MvdrCovarianceDomain::RealTimeSamples => {
                        ...
                        let p = crate::sensor::beamforming::capon_spatial_spectrum_point(
                            sensor_data,
                            self.processor.sensor_positions(),
                            point,
                            &cfg,
...
mod tests {
    ...
    fn centered_cube_generates_points() {
        use crate::sensor::beamforming::TimeDomainDelayReference;

        let processor = make_processor(4);

        // Field-jargon correct default: SRP-DAS (time-domain DAS) with explicit delay datum.
        let mut cfg = LocalizationBeamformSearchConfig::default();
        cfg.method = LocalizationBeamformingMethod::SrpDasTimeDomain {
            // Recommended common default: deterministic reference sensor (element 0).
