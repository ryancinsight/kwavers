...
impl FieldAnalyzer {
            ...
            Zip::indexed(source_field).for_each(|(i, j, k), &source_amplitude| {
                if source_amplitude.abs() > 1e-12 {
                    ...
                    let field_contribution = match self.config.far_field_method {
                        FarFieldMethod::Fraunhofer => self.fraunhofer_contribution(
                            source_amplitude,
                            &source_pos,
                            &analysis_point,
                            wavenumber,
                        ),
                        FarFieldMethod::Fresnel => self.fresnel_contribution(
                            source_amplitude,
                            &source_pos,
                            &analysis_point,
                            wavenumber,
    ...
    pub fn analyze_field_metrics(
        ...
        let beam_width_focus =
            self.calculate_beam_width_at_depth(pressure_field, grid, focal_depth)?;
        ...
        let beam_efficiency =
            self.calculate_beam_efficiency(pressure_field, grid, &peak_location)?;
    ...
    pub fn calculate_field_uniformity(
        ...
        let variance = values_in_region
            .iter()
            .map(|&x| (x - mean).powi(2))
            .sum::<f64>()
            / values_in_region.len() as f64;
    ...
    fn exact_contribution(&self, amplitude: f64, distance: f64, wavenumber: f64) -> f64 {
        let phase = wavenumber * distance;
        amplitude * phase.cos() / distance
    }
    ...
    fn calculate_fwhm_width(&self, field_2d: &ArrayView2<f64>, grid: &Grid) -> KwaversResult<f64> {
    ...
    fn calculate_total_power(&self, field: ArrayView3<f64>, grid: &Grid) -> KwaversResult<f64> {
        ...
        let impedance = crate::constants::acoustic::WATER_DENSITY
