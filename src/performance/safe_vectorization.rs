...
impl SafeVectorOps {
    ...
    pub fn add_arrays(a: &Array3<f64>, b: &Array3<f64>) -> Array3<f64> {
        ...
        Array3::from_shape_vec(a.dim(), result)
    ...
    pub fn add_arrays_parallel(a: &Array3<f64>, b: &Array3<f64>) -> Array3<f64> {
        ...
        let result: Vec<f64> = a_vec.par_iter()
            .zip(b_vec.par_iter())
            .map(|(a_val, b_val)| a_val + b_val)
            .collect();
    ...
    pub fn scalar_multiply(array: &Array3<f64>, scalar: f64) -> Array3<f64> {
        let result: Vec<f64> = array.iter()
            .map(|val| val * scalar)
            .collect();
        
        Array3::from_shape_vec(array.dim(), result)
            .expect("Shape and data length must match")
    }
