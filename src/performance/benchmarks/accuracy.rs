...
use crate::{medium::Medium, Grid, HomogeneousMedium};
use ndarray::Array3;
...
pub fn benchmark_time_integration() -> Vec<AccuracyResult> {
    ...
    for cfl in cfl_numbers {
        ...
        println!(
            "Testing CFL={}: dx={}, dt={}, period={}, n_steps={}, actual_time={}",
...
pub fn print_results(results: &[AccuracyResult]) {
    ...
    for result in results {
        let status = if result.passed(0.1) {
            "✓ PASS"
        } else {
            "✗ FAIL"
        };
        println!(
            "{:<20} {:<30} {:<12.2e} {:<12.2e} {:<12.2e} {:<8}",
            result.test_name,
            result.parameters,
            result.max_error,
            result.avg_error,
...
mod tests {
    ...
    fn test_fd_convergence_rate() {
        ...
        for i in 1..results.len() {
            assert!(
                results[i].l2_error < results[i - 1].l2_error,
        ...
        if results.len() >= 2 {
            ...
            assert!(
                ratio > 3.5 && ratio < 4.5,
                "Should show approximately 2nd order convergence, got ratio: {}",
                ratio
            );
        }
    }
    ...
    fn test_time_integration_stability() {
        ...
        for result in results {
            if result.parameters.contains("CFL=0.") {
                println!(
                    "CFL test: {} - max_error: {}",
                    result.parameters, result.max_error
                );
