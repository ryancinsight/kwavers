...
impl BenchmarkSuite {
    ...
    fn benchmark_pstd(&mut self, grid_size: usize) -> KwaversResult<()> {
        ...
        for _ in 0..self.config.iterations {
            ...
            let field_memory =
                (pressure.len() + vx.len() + vy.len() + vz.len()) * std::mem::size_of::<f64>();
        ...
        let grid_updates_per_second =
            (total_points * self.config.time_steps) as f64 / runtime.as_secs_f64();
    ...
    fn initialize_gaussian_field(&self, field: &mut Array3<f64>, grid: &Grid) {
        ...
        field.indexed_iter_mut().for_each(|((i, j, k), value)| {
            let x = (i as f64 - center.0 as f64) * grid.dx;
            let y = (j as f64 - center.1 as f64) * grid.dy;
            let z = (k as f64 - center.2 as f64) * grid.dz;
            let r2 = x * x + y * y + z * z;
    ...
    fn initialize_sharp_feature(&self, field: &mut Array3<f64>, grid: &Grid) {
        ...
        field.indexed_iter_mut().for_each(|((i, j, k), value)| {
            if i.abs_diff(center) < width
                && j.abs_diff(center) < width
                && k.abs_diff(center) < width
            {
                *value = 1.0;
            }
        });
    ...
    fn print_console(&self) {
        ...
        for (solver, results) in by_solver {
            ...
            for result in results {
                println!(
                    "  {}³ grid: {:.2}M updates/s, {:.2}ms runtime, {:.1}MB memory",
                    result.grid_size,
