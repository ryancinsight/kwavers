...
impl StiffnessDetector {
        ...
        G: Fn(&Array3<f64>) -> KwaversResult<Array3<f64>>,
    {
        let metric = match self.method {
            StiffnessMethod::Eigenvalue => self.eigenvalue_detection(field, explicit_rhs, implicit_rhs)?,
            StiffnessMethod::PowerIteration => self.power_iteration_detection(field, explicit_rhs, implicit_rhs)?,
            StiffnessMethod::NormBased => self.norm_based_detection(field, explicit_rhs, implicit_rhs)?,
        };
        ...
        G: Fn(&Array3<f64>) -> KwaversResult<Array3<f64>>,
    {
        ...
        for _ in 0..10 {
            ...
            // Rayleigh quotient
            let eigenvalue = jv.iter().zip(&perturbation).map(|(&j, &p)| j * p).sum::<f64>();
