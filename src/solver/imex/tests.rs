...
mod tests {
    ...
    fn test_imex_rk_ssp2() {
        ...
        let result = integrator.advance(&field, dt, explicit_rhs, implicit_rhs).unwrap();
    ...
    fn test_operator_splitting_strang() {
        ...
        let result = splitting.split_step(&field, dt, operator_a, operator_b).unwrap();
        
        // For these operators, Strang splitting should be very accurate
        let expected = field.clone(); // Growth and decay cancel
        let error: f64 = (result - expected).iter().map(|&e| e * e).sum::<f64>().sqrt();
    ...
    fn test_conservation() {
        ...
        let implicit_rhs = |_field: &Array3<f64>| -> KwaversResult<Array3<f64>> {
            Ok(Array3::zeros((8, 8, 8)))
        };
