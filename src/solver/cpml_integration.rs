...
impl CPMLSolver {
    ...
    fn compute_pressure_gradients(&mut self, pressure: &Array3<f64>, grid: &Grid) {
        ...
        for i in 1..self.nx-1 {
            for j in 0..self.ny {
                for k in 0..self.nz {
                    self.grad_x[[i, j, k]] = (pressure[[i+1, j, k]] - pressure[[i-1, j, k]]) 
                        / (2.0 * grid.dx);
                }
            }
        }
        ...
        for i in 0..self.nx {
            for j in 1..self.ny-1 {
                for k in 0..self.nz {
                    self.grad_y[[i, j, k]] = (pressure[[i, j+1, k]] - pressure[[i, j-1, k]]) 
                        / (2.0 * grid.dy);
                }
            }
        }
        ...
        for i in 0..self.nx {
            for j in 0..self.ny {
                for k in 1..self.nz-1 {
                    self.grad_z[[i, j, k]] = (pressure[[i, j, k+1]] - pressure[[i, j, k-1]]) 
                        / (2.0 * grid.dz);
                }
            }
        }
