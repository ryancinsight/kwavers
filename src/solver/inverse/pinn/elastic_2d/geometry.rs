//! PINN-Specific Geometry Extensions for 2D Elastic Wave Equations
//!
...
use crate::domain::geometry::{GeometricDomain, PointLocation, RectangularDomain};
use crate::domain::physics::BoundaryCondition;
...
/// Multi-region domain for heterogeneous elastic media
///
...
impl MultiRegionDomain {
    ...
    pub fn sample_interface_points(
        ...
        for i in 0..self.regions.len() - 1 {
            // Sample boundary points from region i
            let boundary_i = self.regions[i].sample_boundary(n_points_per_interface * 2, seed);

            // Keep only points that are also on the boundary of region i+1
            let tolerance = 1e-8;
...
impl CollocationSampler {
    ...
    pub fn sample_interior(&self, n_points: usize) -> Array2<f64> {
        match self.strategy {
            ...
            SamplingStrategy::AdaptiveRefinement => {
                // Start with uniform, refine later based on loss
    ...
    fn sobol_sample(&self, n_points: usize, boundary: bool) -> Array2<f64> {
        // Simplified Sobol: use uniform for now
        // TODO: Implement true Sobol sequence generator
        if boundary {
            self.domain.sample_boundary(n_points, self.seed)
        } else {
            self.domain.sample_interior(n_points, self.seed)
...
mod tests {
    ...
    fn test_adaptive_refinement() {
        ...
        // Simulate high residuals at some points
        ...
        // Should have more points after refinement
