...
impl InterfaceGeometry {
    ...
    fn detect_interface(source: &Grid, target: &Grid) -> KwaversResult<(usize, f64)> {
        ...
        let overlaps = [
            (source_bounds.0 .1.min(target_bounds.0 .1)
                - f64::max(source_bounds.0 .0, target_bounds.0 .0))
            .abs(),
            (source_bounds.1 .1.min(target_bounds.1 .1)
                - f64::max(source_bounds.1 .0, target_bounds.1 .0))
            .abs(),
            (source_bounds.2 .1.min(target_bounds.2 .1)
                - f64::max(source_bounds.2 .0, target_bounds.2 .0))
            .abs(),
        ];
