...
use crate::{
    ...
    grid::Grid,
    medium::Medium,
    physics::plugin::{PhysicsPlugin, PluginMetadata, PluginState, PluginContext},
};
...
impl ThermalDiffusionSolver {
    ...
    pub fn new(config: ThermalDiffusionConfig, grid: &Grid) -> KwaversResult<Self> {
        info!("Initializing thermal diffusion solver with config: {:?}", config);
    ...
    pub fn update(
        ...
        self.metrics.insert("update_time".to_string(), start_time.elapsed().as_secs_f64());
        Ok(())
    }
    ...
    fn update_standard(
        ...
        Self::compute_laplacian(self.config.spatial_order, &self.temperature.clone(), &mut self.workspace1, grid)?;
    ...
    fn update_hyperbolic(
        ...
        Zip::indexed(&mut self.workspace2)
            ...
            .and(&self.workspace1)
            .and(heat_source)
            .for_each(|(i, j, k), result, &temp_curr, &temp_prev, &laplacian, &source| {
                let x = i as f64 * grid.dx;
                let y = j as f64 * grid.dy;
                let z = k as f64 * grid.dz;
    ...
    fn laplacian_2nd_order(
        ...
        Zip::indexed(result.slice_mut(interior))
            .for_each(|(i, j, k), laplacian| {
                let i = i + 1;
                let j = j + 1;
                let k = k + 1;
                
                *laplacian = (field[[i+1, j, k]] - 2.0 * field[[i, j, k]] + field[[i-1, j, k]]) * dx2_inv +
    ...
    fn laplacian_4th_order(
        ...
        Zip::indexed(result.slice_mut(interior))
            .for_each(|(i, j, k), laplacian| {
                ...
                // X-direction
                let d2x = C1 * field[[i+2, j, k]] + C2 * field[[i+1, j, k]] + 
                         C3 * field[[i, j, k]] + C2 * field[[i-1, j, k]] + 
                         C1 * field[[i-2, j, k]];
                
                // Y-direction
                let d2y = C1 * field[[i, j+2, k]] + C2 * field[[i, j+1, k]] + 
                         C3 * field[[i, j, k]] + C2 * field[[i, j-1, k]] + 
                         C1 * field[[i, j-2, k]];
                
                // Z-direction
                let d2z = C1 * field[[i, j, k+2]] + C2 * field[[i, j, k+1]] + 
                         C3 * field[[i, j, k]] + C2 * field[[i, j, k-1]] + 
                         C1 * field[[i, j, k-2]];
    ...
    fn laplacian_6th_order(
        ...
        Zip::indexed(result.slice_mut(interior))
            .for_each(|(i, j, k), laplacian| {
                ...
                // X-direction
                let d2x = C1 * field[[i+3, j, k]] + C2 * field[[i+2, j, k]] + 
                         C3 * field[[i+1, j, k]] + C4 * field[[i, j, k]] + 
                         C3 * field[[i-1, j, k]] + C2 * field[[i-2, j, k]] + 
                         C1 * field[[i-3, j, k]];
                
                // Y-direction
                let d2y = C1 * field[[i, j+3, k]] + C2 * field[[i, j+2, k]] + 
                         C3 * field[[i, j+1, k]] + C4 * field[[i, j, k]] + 
                         C3 * field[[i, j-1, k]] + C2 * field[[i, j-2, k]] + 
                         C1 * field[[i, j-3, k]];
                
                // Z-direction
                let d2z = C1 * field[[i, j, k+3]] + C2 * field[[i, j, k+2]] + 
                         C3 * field[[i, j, k+1]] + C4 * field[[i, j, k]] + 
                         C3 * field[[i, j, k-1]] + C2 * field[[i, j, k-2]] + 
                         C1 * field[[i, j, k-3]];
...
impl ThermalDiffusionPlugin {
    ...
    pub fn new(config: ThermalDiffusionConfig, grid: &Grid) -> KwaversResult<Self> {
        ...
        let metadata = PluginMetadata {
            ...
            description: "Comprehensive thermal diffusion solver with bioheat equation and dose tracking".to_string(),
...
mod tests {
    ...
    fn test_steady_state_diffusion() {
        ...
        let temp_range = solver.temperature.iter()
