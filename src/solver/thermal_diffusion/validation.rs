...
mod tests {
    ...
    fn test_1d_heat_diffusion_analytical() {
        ...
        let decay_factor = (-alpha * std::f64::consts::PI.powi(2) * final_time / length.powi(2)).exp();
        
        // Check center line (should be same for all y,z due to 1D nature)
        for i in 0..nx {
            let x = i as f64 * dx;
            let expected = t_ambient + t0 * (std::f64::consts::PI * x / length).sin() * decay_factor;
    ...
    fn test_point_source_greens_function() {
        ...
        solver.set_temperature(Array3::from_elem((n, n, n), t_ambient)).unwrap();
            ...
            for j in 0..n {
                for k in 0..n {
                    let r = (((i as f64 - center as f64) * dx).powi(2) +
                            ...
                            ((k as f64 - center as f64) * dx).powi(2)).sqrt();
                    
                    if r > 3.0 * dx { // Away from singularity
                        let expected = t_ambient + 
                            q_total / (rho * cp * (4.0 * std::f64::consts::PI * alpha * final_time).powf(1.5)) *
                            (-r.powi(2) / (4.0 * alpha * final_time)).exp();
                        
                        let actual = temperature[[i, j, k]];
                        
                        // Allow 5% error due to discretization
                        if expected > t_ambient + 0.1 { // Only check where temperature rise is significant
                            assert_relative_eq!(actual, expected, epsilon = 0.05 * (expected - t_ambient));
                        }
                    }
                }
            }
    ...
    fn test_pennes_steady_state() {
        ...
        let medium = HomogeneousMedium::from_minimal(1050.0, 1540.0, &grid, 0.0, 0.0);
        ...
        solver.set_temperature(Array3::from_elem((n, n, n), arterial_temp)).unwrap();
        ...
        println!("Expected steady state: {:.2}K, Actual average: {:.2}K", expected_temp, avg_temp);
        
        // Verify temperature is reasonable and moving towards steady state
        assert!(avg_temp > t_a, "Temperature should be above arterial");
        assert!(avg_temp < expected_temp, "Temperature should not exceed theoretical steady state");
        
        // Check uniformity (should be constant everywhere)
        let temp_std = temperature.iter()
            .map(|&t| (t - avg_temp).powi(2))
            .sum::<f64>()
            .sqrt() / (n * n * n) as f64;
        
        assert!(temp_std < 0.01, "Temperature should be uniform, but std = {}", temp_std);
    ...
    fn test_thermal_dose_cem43() {
        ...
        solver.set_temperature(Array3::from_elem((n, n, n), 316.15)).unwrap(); // 43°C
        ...
        solver2.set_temperature(Array3::from_elem((n, n, n), 318.15)).unwrap(); // 45°C
        ...
        solver3.set_temperature(Array3::from_elem((n, n, n), 314.15)).unwrap(); // 41°C
    ...
    fn test_hyperbolic_heat_wave() {
            ...
            for j in 0..n {
                for k in 0..n {
                    let r2 = ((i as f64 - center as f64).powi(2) +
                             ...
                             (k as f64 - center as f64).powi(2)) * 0.001_f64.powi(2);
        ...
        for step in 0..n_steps {
            ...
            if energy_change > 0.1 {
                println!("Step {}: Energy change = {:.2}%", step, energy_change * 100.0);
        ...
        println!("✓ Hyperbolic heat conduction completed {} steps successfully", n_steps);
    }
    ...
    fn test_energy_conservation() {
        ...
        for i in 0..n {
            for j in 0..n {
                for k in 0..n {
                    initial_temp[[i, j, k]] = 293.15 + 10.0 * ((i + j + k) as f64 / (3.0 * n as f64));
                }
            }
        }
        ...
        assert_relative_eq!(final_energy, initial_energy, epsilon = 1e-10 * initial_energy);
    ...
    fn test_spatial_order_convergence() {
        ...
        println!("Spatial convergence: diff_2_4={:.6}, diff_4_6={:.6}", diff_2_4, diff_4_6);
        
        // Just verify that higher order methods give reasonable results
        assert!(diff_4_6 < diff_2_4 * 2.0, 
