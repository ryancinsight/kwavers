...
impl HeterogeneousHandler {
        ...
        for i in 1..nx - 1 {
            for j in 1..ny - 1 {
                for k in 1..nz - 1 {
                    ...
                    let grad_rho_mag =
                        ((grad_rho_x.powi(2) + grad_rho_y.powi(2) + grad_rho_z.powi(2)).sqrt()
                            / rho_ref)
                            .abs();
                    let grad_c_mag =
                        ((grad_c_x.powi(2) + grad_c_y.powi(2) + grad_c_z.powi(2)).sqrt() / c_ref)
                            .abs();
    ...
    fn apply_gaussian_smoothing(
        ...
        let mask = self.interface_mask.as_ref().ok_or_else(|| {
            ...
            for j in 1..self.grid.ny - 1 {
                for k in 1..self.grid.nz - 1 {
                    if mask[[i, j, k]] {
                        ...
                        let (nx, ny, nz) = self.compute_interface_normal(density, i, j, k);
