...
use crate::constants::numerical::{
    LINEAR_VISCOSITY_COEFF, MAX_VISCOSITY_LIMIT, QUADRATIC_VISCOSITY_COEFF, STENCIL_COEFF_1_4,
    VON_NEUMANN_RICHTMYER_COEFF, WENO_EPSILON, WENO_WEIGHT_0, WENO_WEIGHT_1, WENO_WEIGHT_2,
};
            ...
            for j in 1..ny - 1 {
                for k in 1..nz - 1 {
                    ...
                    for (di, dj, dk) in &[
                        ...
                        let entropy_jump =
                            (s_center - s_neighbor).abs() / s_center.abs().max(1e-10);
