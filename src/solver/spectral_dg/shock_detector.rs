        ...
        for i in 1..shape.0 - 1 {
            for j in 1..shape.1 - 1 {
                for k in 1..shape.2 - 1 {
                    ...
                    let low_mode = 0.5 * (coefficients[[i - 1, j, k]] + coefficients[[i + 1, j, k]]);
                    
                    // High-order mode energy (oscillations/discontinuities)
                    let high_mode_x = coefficients[[i + 1, j, k]] 
                                    - 2.0 * value 
                                    + coefficients[[i - 1, j, k]];
                    let high_mode_y = coefficients[[i, j + 1, k]] 
                                    - 2.0 * value 
                                    + coefficients[[i, j - 1, k]];
                    let high_mode_z = coefficients[[i, j, k + 1]] 
                                    - 2.0 * value 
                                    + coefficients[[i, j, k - 1]];
                    
                    let high_mode_energy = (high_mode_x.powi(2) 
                                          + high_mode_y.powi(2) 
                                          + high_mode_z.powi(2)).sqrt();
                    ...
                    let max_jump = [
                        ...
                        (coefficients[[i, j, k - 1]] - value).abs(),
                    ].iter().fold(0.0_f64, |max, &v| max.max(v));
