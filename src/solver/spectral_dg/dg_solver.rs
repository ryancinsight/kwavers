...
impl DGSolver {
    ...
    pub fn new(config: DGConfig, grid: Arc<Grid>) -> KwaversResult<Self> {
        ...
        let vandermonde =
            Self::build_vandermonde(&xi_nodes, config.polynomial_order, config.basis_type)?;
        ...
        let stiffness_matrix =
            Self::compute_stiffness_matrix(&vandermonde, &xi_nodes, &weights, config.basis_type)?;
        ...
        for i in 0..n_quad {
            for j in 0..n {
                dphi[[i, j]] = match basis_type {
                    ...
                    BasisType::Lagrange => Self::lagrange_basis_deriv(j, nodes[i], nodes),
                };
            }
        }
        ...
        match self.config.flux_type {
            ...
            FluxType::HLL | FluxType::HLLC => {
                ...
                if s_left >= 0.0 {
                    ...
                    (s_right * c * u_left - s_left * c * u_right
                        + s_left * s_right * (u_right - u_left))
                        / (s_right - s_left)
                }
            }
        }
            ...
            LimiterType::MinMod => {
                for elem in 1..n_elements - 1 {
                    ...
                    for mode in 1..self.n_nodes {
                        let factor =
                            limited_slope / (slope_left.abs().max(slope_right.abs()) + 1e-12);
    ...
    pub fn apply_shock_detector(&self, field: &Array3<f64>) -> Array3<bool> {
        ...
        for i in 1..nx - 1 {
            for j in 1..ny - 1 {
                for k in 1..nz - 1 {
                    let grad_x =
                        (field[[i + 1, j, k]] - field[[i - 1, j, k]]).abs() / (2.0 * self.grid.dx);
                    let grad_y =
                        (field[[i, j + 1, k]] - field[[i, j - 1, k]]).abs() / (2.0 * self.grid.dy);
                    let grad_z =
                        (field[[i, j, k + 1]] - field[[i, j, k - 1]]).abs() / (2.0 * self.grid.dz);
