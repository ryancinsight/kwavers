...
mod tests {
    ...
    fn test_fdtd_plane_wave() {
        ...
        if peaks.len() >= 2 {
            ...
            assert!(
                error < 2.0 * expected_error,
                "FDTD wavelength error {} exceeds expected {}",
                error,
                expected_error
            );
        }
    ...
    fn test_fdtd_dispersion() {
        ...
        for order in [2, 4, 6] {
            ...
            .unwrap()
            .max_stable_dt(1500.0);
            let dt_order = solver.max_stable_dt(1500.0);

            if order > 2 {
                assert!(
                    dt_order <= dt2,
                    "Higher order FDTD should have more restrictive CFL"
                );
            }
        }
    }
    ...
    fn test_cfl_stability() {
        ...
        let configs = vec![(2, 1.0 / 3.0_f64.sqrt()), (4, 0.5), (6, 0.4)];
        ...
        for (order, expected_limit) in configs {
            ...
            assert!(
                cfl <= expected_limit * 1.01, // Small tolerance
                "CFL {} exceeds limit {} for order {}",
                cfl,
                expected_limit,
                order
            );
