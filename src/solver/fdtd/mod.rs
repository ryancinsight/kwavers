                ...
                for i in 0..nx {
                    for k in 0..nz {
                        ...
                        for j in 1..half_stencil.min(ny - 1) {
                            deriv[[i, j, k]] =
                                (field[[i, j + 1, k]] - field[[i, j - 1, k]]) / (2.0 * dx);
                        }

                        // Top boundary: backward difference at j=ny-1
                        if ny > 1 {
                            deriv[[i, ny - 1, k]] =
                                (field[[i, ny - 1, k]] - field[[i, ny - 2, k]]) / dx;
                        }
                ...
                for i in 0..nx {
                    for j in 0..ny {
                        ...
                        for k in 1..half_stencil.min(nz - 1) {
                            deriv[[i, j, k]] =
                                (field[[i, j, k + 1]] - field[[i, j, k - 1]]) / (2.0 * dx);
                        }

                        // Back boundary: backward difference at k=nz-1
                        if nz > 1 {
                            deriv[[i, j, nz - 1]] =
                                (field[[i, j, nz - 1]] - field[[i, j, nz - 2]]) / dx;
                        }
            ...
            for j in half_stencil..(ny - half_stencil) {
                for k in half_stencil..(nz - half_stencil) {
                    ...
                    for (idx, &coeff) in coeffs.iter().enumerate() {
                        ...
                        if offset != 0 {
                            dvx_dx +=
                                coeff * vx[[(i as isize + offset) as usize, j, k]] / self.grid.dx;
                            dvy_dy +=
                                coeff * vy[[i, (j as isize + offset) as usize, k]] / self.grid.dy;
                            dvz_dz +=
                                coeff * vz[[i, j, (k as isize + offset) as usize]] / self.grid.dz;
                        }
                    }
        ...
        for j in 0..ny {
            for k in 0..nz {
                ...
                divergence[[0, j, k]] = (vx[[1, j, k]] - vx[[0, j, k]]) / self.grid.dx
                    + (if j > 0 && j < ny - 1 {
                        (vy[[0, j + 1, k]] - vy[[0, j - 1, k]]) / (2.0 * self.grid.dy)
                    } else {
                        0.0
                    })
                    + (if k > 0 && k < nz - 1 {
                        (vz[[0, j, k + 1]] - vz[[0, j, k - 1]]) / (2.0 * self.grid.dz)
                    } else {
                        0.0
                    });

                // Right boundary (i=nx-1)
                divergence[[nx - 1, j, k]] = (vx[[nx - 1, j, k]] - vx[[nx - 2, j, k]])
                    / self.grid.dx
                    + (if j > 0 && j < ny - 1 {
                        (vy[[nx - 1, j + 1, k]] - vy[[nx - 1, j - 1, k]]) / (2.0 * self.grid.dy)
                    } else {
                        0.0
                    })
                    + (if k > 0 && k < nz - 1 {
                        (vz[[nx - 1, j, k + 1]] - vz[[nx - 1, j, k - 1]]) / (2.0 * self.grid.dz)
                    } else {
                        0.0
                    });
            }
