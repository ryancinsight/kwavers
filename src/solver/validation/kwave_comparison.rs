...
impl KWaveValidator {
    ...
    fn run_test(&self, test_case: &KWaveTestCase) -> KwaversResult<TestResult> {
        match test_case.name.as_str() {
            ...
            _ => Err(KwaversError::Config(ConfigError::InvalidValue {
                field: "test_case".to_string(),
                value: test_case.name.clone(),
                reason: "Unknown test case type".to_string(),
            })),
        }
    }
    ...
    fn test_pml_absorption(&self, test_case: &KWaveTestCase) -> KwaversResult<TestResult> {
        ...
        let initial_energy: f64 = pressure.iter()
            .map(|&p| p * p)
            .sum();
    ...
    fn test_heterogeneous_medium(&self, test_case: &KWaveTestCase) -> KwaversResult<TestResult> {
        ...
        let energy_layer1: f64 = final_pressure.slice(s![..self.grid.nx/3, .., ..])
            ...
            .map(|&p| p * p)
            .sum();
        let energy_layer2: f64 = final_pressure.slice(s![self.grid.nx/3..2*self.grid.nx/3, .., ..])
            .iter()
            .map(|&p| p * p)
            .sum();
        let energy_layer3: f64 = final_pressure.slice(s![2*self.grid.nx/3.., .., ..])
            .iter()
            .map(|&p| p * p)
            .sum();
    ...
    fn test_focused_transducer(&self, test_case: &KWaveTestCase) -> KwaversResult<TestResult> {
        ...
        let transducer = PhasedArrayTransducer::new(config);
        let medium = HomogeneousMedium::new(1500.0, 1000.0, 0.0);
        ...
        // Find focal point
        let (max_idx, _) = pressure.indexed_iter()
        ...
        let beam_width = focus_slice.iter()
            .enumerate()
            .filter(|(_, &p)| p.abs() > 0.5 * max_pressure)
            .count() as f64 * self.grid.dy;
        ...
        Ok(TestResult {
            ...
            error: focal_error,
            tolerance: test_case.tolerance,
            details: format!("Focal error: {:.1}mm, Beam width: {:.1}mm", 
                           focal_error * 1000.0, beam_width * 1000.0),
        })
    }
    ...
    fn test_time_reversal(&self, test_case: &KWaveTestCase) -> KwaversResult<TestResult> {
        ...
        let max_elsewhere = reconstructed.indexed_iter()
    ...
    fn compute_relative_error(&self, computed: &Array3<f64>, reference: &Array3<f64>) -> KwaversResult<f64> {
        let diff_squared: f64 = Zip::from(computed)
            .and(reference)
            .map_collect(|&c, &r| (c - r).powi(2))
            .sum();
        
        let ref_squared: f64 = reference.iter()
            .map(|&r| r.powi(2))
            .sum();
    ...
    fn compute_spectrum(&self, field: &Array3<f64>) -> KwaversResult<Array1<f64>> {
        ...
        // Convert to complex
        let mut data: Vec<Complex<f64>> = slice.iter()
    ...
    fn extract_boundary(&self, field: &Array3<f64>) -> Array3<f64> {
        ...
        // X boundaries
        boundary.slice_mut(s![0, .., ..]).assign(&field.slice(s![0, .., ..]));
        boundary.slice_mut(s![nx-1, .., ..]).assign(&field.slice(s![nx-1, .., ..]));
        
        // Y boundaries
        boundary.slice_mut(s![.., 0, ..]).assign(&field.slice(s![.., 0, ..]));
        boundary.slice_mut(s![.., ny-1, ..]).assign(&field.slice(s![.., ny-1, ..]));
