    ...
    fn run_test_case(&self, test_case: &KWaveTestCase) -> KwaversResult<TestResult> {
        ...
        let computed_result = match test_case.name.as_str() {
            "homogeneous_propagation" => {
                ...
                let p_rms =
                    (p_field.iter().map(|&p| p * p).sum::<f64>() / p_field.len() as f64).sqrt();
                p_rms
            }
            ...
            "focused_transducer" => {
                ...
                let p_field = solver.pressure_field();
                let focal_pressure =
                    p_field[[self.grid.nx / 2, self.grid.ny / 2, self.grid.nz * 3 / 4]];
                focal_pressure.abs()
            }
        ...
        let message = if passed {
            format!(
                "Test passed: error = {:.2e} < tolerance = {:.2e}",
                error, test_case.tolerance
            )
        } else {
            format!(
                "Test FAILED: error = {:.2e} >= tolerance = {:.2e}",
                error, test_case.tolerance
            )
        };
