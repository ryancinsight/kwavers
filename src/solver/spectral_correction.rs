...
/// Compute low-dispersion PSTD correction
///
/// This implements a more advanced dispersion correction for PSTD
/// that accounts for the fact that PSTD is more accurate than FDTD.
    ...
    for i in 0..grid.nx {
        for j in 0..grid.ny {
            for k in 0..grid.nz {
                ...
                if k_mag > 0.0 {
                    // Temporal correction only (since PSTD spatial differentiation is exact)
