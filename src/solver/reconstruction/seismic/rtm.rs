...
impl ReverseTimeMigration {
    ...
    pub fn migrate_shot(
        ...
        let receiver_wavefield = self.backward_propagation(
            shot_data,
            receiver_positions,
            grid,
            n_time_steps
    ...
    fn forward_propagation(
        ...
        let mut stored_wavefield = Array4::zeros((
            storage_size,
            grid.nx,
            grid.ny,
            grid.nz
        ...
        for t in 0..n_time_steps {
            ...
            if t % RTM_STORAGE_DECIMATION == 0 {
                ...
                stored_wavefield.slice_mut(s![storage_idx, .., .., ..]).assign(&pressure);
            }
