...
impl FieldRegistry {
    ...
    pub fn registered_fields(&self) -> Vec<UnifiedFieldType> {
        self.fields
            ...
            .enumerate()
            .filter_map(|(idx, opt)| {
                opt.as_ref().map(|_| match idx {
                    0 => UnifiedFieldType::Pressure,
                    1 => UnifiedFieldType::Temperature,
                    _ => UnifiedFieldType::Pressure,
                })
            })
            .collect()
    }
...
impl PluginBasedSolver {
    ...
    pub fn initialize(&mut self) -> KwaversResult<()> {
        ...
        self.plugin_manager
            .initialize_all(&self.grid, self.medium.as_ref())?;
        ...
        info!(
            "Solver initialized with {} plugins and {} fields",
    ...
    pub fn run_with_progress(
        &mut self,
        reporter: &mut dyn crate::solver::ProgressReporter,
    ) -> KwaversResult<()> {
        let dt = self.time.dt;
    ...
    fn calculate_fields_summary(&self) -> crate::solver::FieldsSummary {
        ...
        if let Some(data) = self.field_registry.data() {
            ...
            if let Ok(pressure) = self.field_registry.get_field(UnifiedFieldType::Pressure) {
                max_pressure = pressure.iter().map(|&p| p.abs()).fold(0.0, f64::max);
                total_energy += pressure.iter().map(|&p| p * p).sum::<f64>();
            }
            ...
            if let Ok(temperature) = self.field_registry.get_field(UnifiedFieldType::Temperature) {
                max_temperature = temperature.iter().map(|&t| t.abs()).fold(0.0, f64::max);
            }
        }
    ...
    fn report_metrics(&self) {
        ...
        info!(
            "Registered fields: {:?}",
            self.field_registry.registered_fields()
        );
...
impl PerformanceMetrics {
    ...
    pub fn total_execution_time(&self) -> f64 {
        self.plugin_execution_times
            ...
            .sum::<f64>()
            + self.field_update_times.iter().sum::<f64>()
    }
    ...
    pub fn summary(&self) -> String {
        ...
        for (plugin, times) in &self.plugin_execution_times {
            ...
            summary.push_str(&format!("  {}: {:.3} ms/step\n", plugin, avg_time * 1000.0));
        }
...
mod tests {
    ...
    fn test_field_registry() {
        ...
        // Register fields
        registry
            .register_field(UnifiedFieldType::Pressure, "Acoustic pressure".to_string())
            .unwrap();
        registry
            .register_field(
