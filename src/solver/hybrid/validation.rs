...
use crate::solver::hybrid::{HybridConfig, HybridSolver, ValidationResults};
use log::{info, warn};
...
impl HybridValidationSuite {
    ...
    fn register_standard_tests(&mut self) {
        if self.config.enable_physics_tests {
            ...
            self.test_cases
                .push(Box::new(InterfaceReflectionTest::new()));
        ...
        if self.config.enable_numerical_tests {
            ...
            self.test_cases
                .push(Box::new(DispersionAnalysisTest::new()));
            self.test_cases.push(Box::new(StabilityTest::new()));
        }

        if self.config.enable_performance_tests {
            self.test_cases
                .push(Box::new(PerformanceBenchmarkTest::new()));
    ...
    pub fn run_all_tests(
        ...
        info!(
            "Validation completed: {}/{} tests passed ({:.1}%)",
            passed_tests,
            self.test_cases.len(),
    ...
    pub fn run_convergence_analysis(
        ...
        info!(
            "Convergence analysis completed. Rate: {:.2}",
            convergence_rate
        );
    ...
    fn generate_recommendations(&self) -> Vec<ValidationRecommendation> {
        ...
        if self.results.quality_score < 0.9 {
            recommendations.push(ValidationRecommendation {
                ...
                description: "Consider increasing numerical precision or refining grid resolution"
                    .to_string(),
            });
        }
...
impl ValidationTestCase for PlaneWaveTest {
    ...
    fn run_test(
        ...
        let analytical = self
            .analytical_solution(grid, num_steps as f64 * dt)
            .unwrap();
...
impl ValidationTestCase for SphericalWaveTest {
    ...
    fn description(&self) -> &str {
        "Tests spherical wave propagation"
    }
    fn run_test(
        &self,
        _solver: &mut HybridSolver,
        _grid: &Grid,
        _medium: &dyn Medium,
    ) -> KwaversResult<TestResult> {
        Ok(TestResult {
    ...
    fn analytical_solution(&self, _grid: &Grid, _time: f64) -> Option<Array4<f64>> {
        None
    }
    fn error_bounds(&self) -> ErrorBounds {
        ErrorBounds::default()
...
impl ValidationTestCase for InterfaceReflectionTest {
    fn name(&self) -> &str {
        "Interface Reflection Test"
    }
    fn description(&self) -> &str {
        "Tests wave reflection at interfaces"
    }
    fn run_test(
        &self,
        _solver: &mut HybridSolver,
...
impl ValidationTestCase for ConservationLawTest {
    ...
    fn description(&self) -> &str {
        "Tests conservation of energy and momentum"
    }
    fn run_test(
        &self,
        _solver: &mut HybridSolver,
...
impl ValidationTestCase for DispersionAnalysisTest {
    fn name(&self) -> &str {
        "Dispersion Analysis Test"
    }
    fn description(&self) -> &str {
        "Tests numerical dispersion characteristics"
    }
    fn run_test(
        &self,
        _solver: &mut HybridSolver,
