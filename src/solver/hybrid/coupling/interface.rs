...
impl CouplingInterface {
    ...
    pub fn apply_coupling(
        ...
        let source_region = regions.first()
            .ok_or_else(|| KwaversError::Config(ConfigError::MissingParameter {
        ...
        // Apply interpolation
        let interpolated = self.interpolation.interpolate(&interface_data, &source_coords, &target_coords)?;
        ...
        match self.geometry.normal_direction {
            0 => {
                ...
                if sx < nx && sx < ex {
                    ...
                    fields.slice_mut(s![sx, sy..ey.min(ny), sz..ez.min(nz), 0])
                        .assign(&slice_data);
                }
            }
            1 => {
                ...
                if sy < ny && sy < ey {
                    let slice_data = data.slice(s![.., .., ..]);
                    fields.slice_mut(s![sx..ex.min(nx), sy, sz..ez.min(nz), 0])
                        .assign(&slice_data);
                }
            }
            2 => {
                ...
                if sz < nz && sz < ez {
                    let slice_data = data.slice(s![.., .., ..]);
                    fields.slice_mut(s![sx..ex.min(nx), sy..ey.min(ny), sz, 0])
                        .assign(&slice_data);
                }
            }
            _ => return Err(KwaversError::Config(ConfigError::InvalidValue {
