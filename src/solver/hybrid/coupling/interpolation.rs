...
impl InterpolationManager {
    ...
    fn cubic_spline_interpolation(
        ...
        for (idx, &(tx, ty, tz)) in target_coords.iter().enumerate().take(result.len()) {
            ...
            for (idx_x, val) in temp_x.iter_mut().enumerate() {
                *val = cubic_basis(wy, temp_xy[idx_x][0], temp_xy[idx_x][1], temp_xy[idx_x][2], temp_xy[idx_x][3]);
            }
    ...
    fn spectral_interpolation(
        ...
        if source_coords.len() != shape[0] * shape[1] * shape[2] || target_coords.len() != shape[0] * shape[1] * shape[2] {
            ...
            for di in 0..=1 {
                for dj in 0..=1 {
                    for dk in 0..=1 {
                        ...
                        let dist = ((tx - xi).powi(2) + (ty - yj).powi(2) + (tz - zk).powi(2)).sqrt();
