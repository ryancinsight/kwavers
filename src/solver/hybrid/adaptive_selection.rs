...
impl AdaptiveSelector {
    ...
    pub fn new(criteria: SelectionCriteria) -> KwaversResult<Self> {
        info!(
            "Initializing adaptive selector with criteria: {:?}",
            criteria
        );
    ...
    pub fn analyze_field_quality(
        ...
        debug!(
            "Field quality analysis completed: composite score = {:.3}",
        ...
        for i in window..grid.nx - window {
            for j in window..grid.ny - window {
                for k in window..grid.nz - window {
                    ...
                    let (grad_mag, curvature) =
                        self.compute_local_differential_metrics(&local_region, grid)?;
    ...
    fn compute_local_differential_metrics(
        ...
        let grad_x = (region[[center.0 + 1, center.1, center.2]]
            - region[[center.0 - 1, center.1, center.2]])
            / (2.0 * grid.dx);
        let grad_y = (region[[center.0, center.1 + 1, center.2]]
            - region[[center.0, center.1 - 1, center.2]])
            / (2.0 * grid.dy);
        let grad_z = (region[[center.0, center.1, center.2 + 1]]
            - region[[center.0, center.1, center.2 - 1]])
            / (2.0 * grid.dz);
        ...
        for i in window..field.shape()[0] - window {
            for j in window..field.shape()[1] - window {
                for k in window..field.shape()[2] - window {
                    ...
                    let high_freq_indicator =
                        self.compute_high_frequency_content(&local_region, grid)?;
        ...
        for i in 1..nx - 1 {
            for j in 1..ny - 1 {
                for k in 1..nz - 1 {
                    ...
                    let laplacian = (region[[i + 1, j, k]] - 2.0 * region[[i, j, k]]
                        + region[[i - 1, j, k]])
                        / (grid.dx * grid.dx)
                        + (region[[i, j + 1, k]] - 2.0 * region[[i, j, k]] + region[[i, j - 1, k]])
                            / (grid.dy * grid.dy)
                        + (region[[i, j, k + 1]] - 2.0 * region[[i, j, k]] + region[[i, j, k - 1]])
                            / (grid.dz * grid.dz);
    ...
    fn compute_composite_score(&self, metrics: &QualityMetrics) -> KwaversResult<f64> {
        let composite = self.criteria.smoothness_weight * metrics.smoothness_score
