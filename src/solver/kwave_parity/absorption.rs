...
pub fn apply_power_law_absorption(
    ...
    let has_absorption =
        tau.iter().any(|&t| t.abs() > 1e-14) || eta.iter().any(|&e| e.abs() > 1e-14);
    ...
    let k_magnitude = Zip::from(&k_vec.0)
        .and(&k_vec.1)
        .and(&k_vec.2)
        .map_collect(|&kx, &ky, &kz| (kx * kx + ky * ky + kz * kz).sqrt());
...
fn compute_multi_relaxation_operators(
    ...
    assert_eq!(
        tau.len(),
        weights.len(),
        "Relaxation times and weights must have same length"
    );
...
mod tests {
    ...
    fn test_multi_relaxation_coefficients() {
        ...
        assert!(
            tau_array[[0, 0, 0]] < 0.0,
            "Multi-relaxation tau should be negative"
        );
        assert!(
            eta_array[[0, 0, 0]] > 0.0,
            "Multi-relaxation eta should be positive"
        );
    ...
    fn test_causal_absorption_coefficients() {
        ...
        let (tau_array, eta_array) =
            compute_causal_absorption_operators(&grid, &relaxation_times, alpha_0, 1e6);
        ...
        let (tau_single, _) =
            compute_causal_absorption_operators(&grid, &single_time, alpha_0, 1e6);
    ...
    fn test_fft_based_absorption_reduces_amplitude() {
        ...
        assert!(
            reduction > 0.01,
            "Absorption should reduce amplitude by >1%, got {}%",
            reduction * 100.0
        );
    }
    ...
    fn test_fft_absorption_energy_dissipation() {
        ...
        assert!(
            energy_loss > 0.05,
            "Should lose >5% energy over 10 steps, got {}%",
            energy_loss * 100.0
        );
    }
