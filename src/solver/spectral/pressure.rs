...
impl SpectralSolver {
    ...
    pub(super) fn update_pressure(&mut self) {
    ...
    pub(super) fn update_density(&mut self, dt: f64) -> KwaversResult<()> {
    ...
    fn apply_pml_to_density(&mut self) -> KwaversResult<()> {
        if let Some(ref mut boundary) = self.boundary {
            boundary.apply_acoustic(self.rho.view_mut(), &self.grid, self.time_step_index)?;
        }
        Ok(())
    }
}
