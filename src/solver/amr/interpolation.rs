...
pub fn restrict_to_coarse(
    ...
    if nx % 2 != 0 || ny % 2 != 0 || nz % 2 != 0 {
        return Err(crate::error::DataError::InvalidFormat {
            ...
            reason: format!(
                "Fine field dimensions must be even for restriction, got ({}, {}, {})",
                nx, ny, nz
            ),
        }
        .into());
...
fn linear_interpolation(coarse_field: &Array3<f64>, octree: &Octree) -> KwaversResult<Array3<f64>> {
    let (nx, ny, nz) = coarse_field.dim();
    ...
    for i in 0..nx {
        for j in 0..ny {
            for k in 0..nz {
                ...
                let v110 = if i < nx - 1 && j < ny - 1 {
                    coarse_field[[i + 1, j + 1, k]]
                } else if i < nx - 1 {
                    v100
                } else {
                    v010
                };
                let v101 = if i < nx - 1 && k < nz - 1 {
                    coarse_field[[i + 1, j, k + 1]]
                } else if i < nx - 1 {
                    v100
                } else {
                    v001
                };
                let v011 = if j < ny - 1 && k < nz - 1 {
                    coarse_field[[i, j + 1, k + 1]]
                } else if j < ny - 1 {
                    v010
                } else {
                    v001
                ...
                fine_field[[fi + 1, fj + 1, fk + 1]] =
                    0.125 * (v000 + v100 + v010 + v001 + v110 + v101 + v011 + v111);
            }
        }
    }
...
fn weno5_interpolation(coarse_field: &Array3<f64>, octree: &Octree) -> KwaversResult<Array3<f64>> {
    let (nx, ny, nz) = coarse_field.dim();
    let mut fine_field = Array3::zeros((nx * 2, ny * 2, nz * 2));
                ...
                for di in 0..2 {
                    for dj in 0..2 {
                        for dk in 0..2 {
                            ...
                            fine_field[[fi + di, fj + dj, fk + dk]] = (val_x + val_y + val_z) / 3.0;
                        }
                    }
                }
            }
...
fn spectral_interpolation(
    ...
    for j in 0..ny {
        for k in 0..nz {
            let mut x_slice: Vec<Complex<f64>> =
                (0..nx).map(|i| complex_field[[i, j, k]]).collect();
    ...
    for i in 0..nx {
        for k in 0..nz {
            let mut y_slice: Vec<Complex<f64>> =
                (0..ny).map(|j| complex_field[[i, j, k]]).collect();
    ...
    for i in 0..nx {
        for j in 0..ny {
            let mut z_slice: Vec<Complex<f64>> =
                (0..nz).map(|k| complex_field[[i, j, k]]).collect();
    ...
    Zip::indexed(&complex_field).for_each(|(i, j, k), &val| {
    ...
    for i in 0..2 * nx {
        for j in 0..2 * ny {
            let mut z_slice: Vec<Complex<f64>> =
                (0..2 * nz).map(|k| padded_spectrum[[i, j, k]]).collect();
    ...
    for i in 0..2 * nx {
        for k in 0..2 * nz {
            let mut y_slice: Vec<Complex<f64>> =
                (0..2 * ny).map(|j| padded_spectrum[[i, j, k]]).collect();
    ...
    for j in 0..2 * ny {
        for k in 0..2 * nz {
            let mut x_slice: Vec<Complex<f64>> =
                (0..2 * nx).map(|i| padded_spectrum[[i, j, k]]).collect();
...
fn linear_restriction(fine_field: &Array3<f64>, octree: &Octree) -> KwaversResult<Array3<f64>> {
    let (nx, ny, nz) = fine_field.dim();
...
fn spectral_restriction(fine_field: &Array3<f64>, octree: &Octree) -> KwaversResult<Array3<f64>> {
...
fn get_conservative_stencil(field: &Array3<f64>, i: usize, j: usize, k: usize) -> Vec<f64> {
    ...
    for i in 0..2 {
        for j in 0..fine_field.dim().1 {
            for k in 0..fine_field.dim().2 {
                ...
                fine_field[[dim0 - 1 - i, j, k]] = fine_field[[dim0 - 3, j, k]];
            }
        }
    }
}
