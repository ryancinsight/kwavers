...
use super::{wavelet::WaveletTransform, WaveletType};
...
impl ErrorEstimator {
    ...
    pub fn new(wavelet_type: WaveletType, refine_threshold: f64, coarsen_threshold: f64) -> Self {
        Self {
        ...
        for i in 2..nx - 2 {
            for j in 2..ny - 2 {
                for k in 2..nz - 2 {
                    ...
                    let dxx = solution[[i + 1, j, k]] - 2.0 * solution[[i, j, k]]
                        + solution[[i - 1, j, k]];
                    let dyy = solution[[i, j + 1, k]] - 2.0 * solution[[i, j, k]]
                        + solution[[i, j - 1, k]];
                    let dzz = solution[[i, j, k + 1]] - 2.0 * solution[[i, j, k]]
                        + solution[[i, j, k - 1]];

                    let dxy = (solution[[i + 1, j + 1, k]]
                        - solution[[i + 1, j - 1, k]]
                        - solution[[i - 1, j + 1, k]]
                        + solution[[i - 1, j - 1, k]])
                        / 4.0;
                    let dxz = (solution[[i + 1, j, k + 1]]
                        - solution[[i + 1, j, k - 1]]
                        - solution[[i - 1, j, k + 1]]
                        + solution[[i - 1, j, k - 1]])
                        / 4.0;
                    let dyz = (solution[[i, j + 1, k + 1]]
                        - solution[[i, j + 1, k - 1]]
                        - solution[[i, j - 1, k + 1]]
                        + solution[[i, j - 1, k - 1]])
                        / 4.0;
    ...
    pub fn estimate_memory_savings(&self, error_field: &Array3<f64>) -> f64 {
        ...
        let cells_to_refine = error_field
            .iter()
            .filter(|&&e| self.should_refine(e))
            .count();
        let cells_to_coarsen = error_field.iter().filter(|&&e| self.can_coarsen(e)).count();
        ...
        let adaptive_cells =
            total_cells - cells_to_refine - cells_to_coarsen + refined_cells + coarsened_cells;
...
mod tests {
    ...
    fn test_gradient_error() {
        ...
        let field = Array3::from_shape_fn((8, 8, 8), |(i, j, _k)| if i < 4 { 0.0 } else { 1.0 });
