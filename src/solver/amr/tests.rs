...
mod tests {
    ...
    fn test_error_estimation() {
        ...
        println!(
            "Max error: {:.3e}, Cells refined: {}",
            result.max_error, result.cells_refined
        );
        ...
        if result.cells_refined == 0 {
            println!(
                "WARNING: No cells refined. Max error {:.3e} may be below threshold {:.3e}",
                ...
                if matches!(scheme, InterpolationScheme::Conservative) {
                    ...
                    if field_sum.abs() > 1e-10 {
                        ...
                        if fine_error >= 1e-10 || coarse_error >= 1e-10 {
                            ...
                            println!(
                                "    Field sum: {:.6e}, Fine sum: {:.6e}, Coarse sum: {:.6e}",
                                field_sum, fine_sum, coarse_sum
                            );
                            println!(
                                "    Fine error: {:.3e}, Coarse error: {:.3e}",
    ...
    fn test_wavelet_transforms() {
        ...
        for wavelet_type in &[
            ...
            println!(
                "{:?} - Smooth max: {:.2e}, Sharp max: {:.2e}",
                wavelet_type, smooth_max, sharp_max
            );
            println!(
                "  Coeffs - Smooth max: {:.2e}, Sharp max: {:.2e}",
    ...
    fn benchmark_amr_performance() {
        ...
        for size in sizes {
            ...
            println!(
                "  Memory saved: {:.1}%",
                amr.memory_stats().memory_saved_percent
            );
        }
    }
    ...
    fn create_linear_field(n: usize) -> Array3<f64> {
        Array3::from_shape_fn((n, n, n), |(i, j, k)| (i + j + k) as f64 / (3.0 * n as f64))
    }
