...
impl MediumBuilder {
    ...
    pub fn build(config: &MediumConfig, grid: &Grid) -> KwaversResult<Box<dyn Medium>> {
    ...
    fn build_heterogeneous(config: &MediumConfig, grid: &Grid) -> KwaversResult<Box<dyn Medium>> {
        ...
        if let MediumType::Heterogeneous {
            tissue_file,
            property_maps,
        } = &config.medium_type
        {
    ...
    fn build_anisotropic(config: &MediumConfig, grid: &Grid) -> KwaversResult<Box<dyn Medium>> {
        // Extract anisotropic configuration
        if let MediumType::Anisotropic {
            tensor_file,
