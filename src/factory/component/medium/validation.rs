...
use super::types::{MediumConfig, MediumType, LayerProperties};
...
impl MediumValidator {
    ...
    pub fn validate(config: &MediumConfig) -> KwaversResult<()> {
        match &config.medium_type {
            MediumType::Homogeneous { 
                ...
                Self::validate_homogeneous(*density, *sound_speed, *mu_a, *mu_s_prime)?;
            },
            MediumType::Heterogeneous { tissue_file, .. } => {
                Self::validate_heterogeneous(tissue_file)?;
            },
            MediumType::Layered { layers } => {
                Self::validate_layered(layers)?;
            },
            MediumType::Anisotropic { tensor_file, .. } => {
                Self::validate_anisotropic(tensor_file)?;
            },
        }
        Ok(())
    }
    ...
    fn validate_homogeneous(
        ...
        if !(MIN_SOUND_SPEED..=MAX_SOUND_SPEED).contains(&sound_speed) {
            return Err(ConfigError::InvalidValue {
                parameter: "sound_speed".to_string(),
                value: sound_speed.to_string(),
                constraint: format!("Sound speed must be within [{MIN_SOUND_SPEED}, {MAX_SOUND_SPEED}] m/s"),
            }.into());
        }
        ...
        if let Some(file_path) = tissue_file {
            ...
            if !valid_extensions.iter().any(|ext| file_path.ends_with(ext)) {
                return Err(ConfigError::InvalidValue {
                    ...
                    constraint: format!("File must have one of these extensions: {}", 
    ...
    fn validate_layered(layers: &[LayerProperties]) -> KwaversResult<()> {
        if layers.is_empty() {
            return Err(ConfigError::InvalidValue {
