...
pub struct InteractiveControls {
    ...
    validation_callbacks:
        HashMap<String, Box<dyn Fn(&ParameterValue) -> ValidationResult + Send + Sync>>,
    change_callbacks:
        HashMap<String, Box<dyn Fn(&ParameterChangeEvent) -> KwaversResult<()> + Send + Sync>>,
...
impl InteractiveControls {
    ...
    pub fn update_parameter(&mut self, name: &str, value: ParameterValue) -> KwaversResult<()> {
        let definition = self.parameters.get(name).ok_or_else(|| {
            KwaversError::Visualization(format!("Parameter '{}' not found", name))
        })?;
    ...
    pub fn render_ui(&mut self, ctx: &Context) -> KwaversResult<()> {
        ...
        Window::new("Simulation Controls")
            ...
            .show(ctx, |ui| self.render_control_panel(ui));
    ...
    fn render_control_panel(&mut self, ui: &mut Ui) {
            ...
            egui::ComboBox::from_label("")
                ...
                .show_ui(ui, |ui| {
                    for group in &self.ui_state.parameter_groups.clone() {
                        ui.selectable_value(
                            &mut self.ui_state.selected_group,
                            group.clone(),
                            group,
                        );
                    }
        ...
        egui::ScrollArea::vertical().show(ui, |ui| {
            let filtered_params: Vec<_> = self
                .parameters
                .iter()
                .filter(|(_, def)| {
                    def.group == self.ui_state.selected_group
                        && (self.ui_state.search_filter.is_empty()
                            || def
                                .display_name
                                .to_lowercase()
                                .contains(&self.ui_state.search_filter.to_lowercase()))
                })
        ...
        ui.horizontal(|ui| {
            ...
            ui.checkbox(
                &mut self.ui_state.show_extended_options,
        ...
        ui.group(|ui| {
            ...
            if let Some(current_value) = self.current_values.get(name).cloned() {
                match (&definition.parameter_type, current_value) {
                    ...
                    (
                        ParameterType::Vector3 { min, max, step },
                        ParameterValue::Vector3(mut values),
                    ) => {
    ...
    fn reset_all_parameters(&mut self) {
        ...
        for (name, definition) in &self.parameters {
            self.current_values
                .insert(name.clone(), definition.default_value.clone());
        }
        ...
        match (param_type, value) {
            ...
            (ParameterType::Vector3 { min, max, .. }, ParameterValue::Vector3(values)) => {
                for v in values {
                    if *v < *min || *v > *max {
                        return Err(KwaversError::Visualization(format!(
                            "Vector3 component {} out of range [{}, {}]",
                            v, min, max
                        )));
                    }
                }
