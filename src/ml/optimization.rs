...
impl ParameterOptimizer {
    ...
    fn parameters_to_state_vector(
        ...
        for (_, &value) in params.iter().take(5) { // Limit to first 5 parameters
            state_vec.push(value);
        }
        
        // Add simulation state
        for &value in simulation_state.iter().take(5) { // Limit to first 5 state variables
            state_vec.push(value);
        }
...
impl PatternRecognizer {
    ...
    pub fn analyze_simulation_patterns(
        ...
        let cavitation_events = self.cavitation_detector
            .detect_cavitation_events(pressure, bubble_radius)?;
        
        let acoustic_events = self.acoustic_analyzer
    ...
    fn generate_pattern_summary(
        ...
        let cavitation_intensity = cavitation_events.iter()
            .map(|e| e.intensity)
            .fold(0.0, |acc, x| acc + x) / cavitation_events.len().max(1) as f64;
        
        let dominant_frequency = acoustic_events.iter()
        ...
        PatternSummary {
            ...
            acoustic_energy_distribution: acoustic_events.iter()
                .map(|e| e.band_energy)
                .collect(),
    ...
    fn calculate_stability_metric(
        ...
        let max_energy = acoustic_events.iter()
            .map(|e| e.band_energy)
...
mod tests {
    ...
    fn test_cavitation_detection() {
        ...
        let events = detector.detect_cavitation_events(&pressure, &bubble_radius).unwrap();
    ...
    fn test_acoustic_event_analysis() {
        ...
        let events = analyzer.analyze_acoustic_spectrum(&spectrum, &frequencies).unwrap();
        assert!(!events.is_empty());
    }
