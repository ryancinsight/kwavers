    ...
    fn generate_random_phase(&mut self) -> f64 {
        match self.distribution {
            PhaseDistribution::Uniform => {
                self.rng.gen_range(0.0..MAX_PHASE_SHIFT)
            }
            
            PhaseDistribution::Discrete | PhaseDistribution::Binary |
            PhaseDistribution::Ternary | PhaseDistribution::Quadrature => {
...
impl TemporalRandomization {
    pub fn new(
        ...
        let randomizer = PhaseRandomizer::new(
    ...
    pub fn get_temporal_coherence(&self) -> f64 {
        ...
        for i in 1..n {
            ...
            let correlation: f64 = prev.iter()
                .zip(curr.iter())
                .map(|(&p1, &p2)| (p2 - p1).cos())
                .sum::<f64>() / prev.len() as f64;
...
impl PhaseShiftKeying {
    pub fn new(num_states: usize) -> Self {
        assert!(num_states >= 2 && num_states <= 8, "Number of states must be between 2 and 8");
    ...
    pub fn set_sequence(&mut self, sequence: Vec<usize>) {
        ...
        assert!(sequence.iter().all(|&s| s < self.num_states), "Invalid state in sequence");
...
mod tests {
    ...
    fn test_temporal_coherence() {
        let mut temporal = TemporalRandomization::new(
            4,
            100.0,
