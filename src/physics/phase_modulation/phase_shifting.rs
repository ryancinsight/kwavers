...
impl PhaseShifter {
    ...
    fn quantize_phase_static(phase: f64, quantization_levels: usize) -> f64 {
        ...
        let quantized =
            (normalized * quantization_levels as f64).round() / quantization_levels as f64;
        quantized * 2.0 * PI
    }
    ...
    pub fn calculate_focusing_phases(&mut self, focal_point: &[f64; 3]) -> &Array1<f64> {
        assert!(
            focal_point[2] >= MIN_FOCAL_DISTANCE / 1000.0,
            "Focal distance too small"
        );
    ...
    pub fn calculate_multifocus_phases(
        ...
        assert!(
            focal_points.len() <= MAX_FOCAL_POINTS,
            "Too many focal points"
        );
        assert!(
            focal_points.iter().all(|p| p.len() == 3),
            "Focal points must be 3D"
        );
        ...
        if self.quantization_enabled {
            self.phase_offsets
                .mapv_inplace(|p| Self::quantize_phase_static(p, PHASE_QUANTIZATION_LEVELS));
        }
...
impl BeamSteering {
    ...
    pub fn update(&mut self) -> &Array1<f64> {
        self.phase_shifter
            .calculate_steering_phases(self.steering_angle, self.azimuth_angle)
    }
...
impl DynamicFocusing {
    ...
    pub fn update(&mut self, time_fraction: f64) -> &Array1<f64> {
        ...
        if self.interpolation_enabled && self.focal_trajectory.len() > 1 {
            ...
            self.phase_shifter
                .calculate_focusing_phases(&self.focal_trajectory[self.current_index])
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_focusing() {
        let elements = Array2::from_shape_fn(
            (8, 2),
            |(i, j)| {
                if j == 0 {
                    i as f64 * 0.0005
