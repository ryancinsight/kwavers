...
mod tests {
    ...
    fn test_anisotropic_christoffel_equation() {
        ...
        assert!(p_error < 0.01, "P-wave speed error: {:.2}%", p_error * 100.0);
        assert!(s_error < 0.01, "S-wave speed error: {:.2}%", s_error * 100.0);
    ...
    fn test_tissue_heterogeneity() {
        ...
        assert!((t_water_liver - 1.04).abs() < 0.05, 
                "Water-liver transmission error: {:.3}", t_water_liver);
        assert!((t_liver_bone - 1.46).abs() < 0.1, 
                "Liver-bone transmission error: {:.3}", t_liver_bone);
    }
}