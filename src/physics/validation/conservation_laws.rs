...
mod tests {
    ...
    fn test_energy_conservation() {
        ...
        for _ in 0..100 {
                ...
                for j in 1..n-1 {
                    for k in 1..n-1 {
                        state.velocity_x[[i, j, k]] -= dt / DENSITY * 
                            (state.pressure[[i+1, j, k]] - state.pressure[[i-1, j, k]]) / (2.0 * dx);
                        state.velocity_y[[i, j, k]] -= dt / DENSITY * 
                            (state.pressure[[i, j+1, k]] - state.pressure[[i, j-1, k]]) / (2.0 * dx);
                        state.velocity_z[[i, j, k]] -= dt / DENSITY * 
                            (state.pressure[[i, j, k+1]] - state.pressure[[i, j, k-1]]) / (2.0 * dx);
                    }
                }
            }
            ...
            for i in 1..n-1 {
                for j in 1..n-1 {
                    for k in 1..n-1 {
                        let div_v = (state.velocity_x[[i+1, j, k]] - state.velocity_x[[i-1, j, k]]) / (2.0 * dx) +
                                   (state.velocity_y[[i, j+1, k]] - state.velocity_y[[i, j-1, k]]) / (2.0 * dx) +
                                   (state.velocity_z[[i, j, k+1]] - state.velocity_z[[i, j, k-1]]) / (2.0 * dx);
        ...
        assert!(energy_error < 0.01, "Energy conservation violated: {:.2}% error", energy_error * 100.0);
    }
    ...
    fn test_momentum_conservation() {
        ...
        let momentum_error = (final_momentum - initial_momentum).abs() / initial_momentum.abs().max(1e-10);
        assert!(momentum_error < 0.01, "Momentum not conserved: {:.2}% error", momentum_error * 100.0);
    }
    ...
    fn test_mass_conservation() {
        ...
        for i in 0..n {
            for j in 0..n {
                let x = i as f64 * dx;
                let y = j as f64 * dx;
                velocity_x[[i, j, 0]] = 10.0 * (2.0 * std::f64::consts::PI * y / (n as f64 * dx)).sin();
                velocity_y[[i, j, 0]] = -10.0 * (2.0 * std::f64::consts::PI * x / (n as f64 * dx)).sin();
            }
        }
            ...
            for i in 1..n-1 {
                for j in 1..n-1 {
                    ...
                    let flux_x = (density[[i+1, j, 0]] * velocity_x[[i+1, j, 0]] -
                                 density[[i-1, j, 0]] * velocity_x[[i-1, j, 0]]) / (2.0 * dx);
                    let flux_y = (density[[i, j+1, 0]] * velocity_y[[i, j+1, 0]] -
                                 density[[i, j-1, 0]] * velocity_y[[i, j-1, 0]]) / (2.0 * dx);
        ...
        assert!(mass_error < 0.01, "Mass not conserved: {:.2}% error", mass_error * 100.0);
    }
            ...
            for j in 0..state.pressure.shape()[1] {
                for k in 0..state.pressure.shape()[2] {
                    energy += state.pressure[[i, j, k]].powi(2) / 
                             (2.0 * DENSITY * SOUND_SPEED.powi(2)) * dv;
                }
            }
        }
