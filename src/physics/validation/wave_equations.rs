...
mod tests {
    ...
    fn test_1d_wave_equation_analytical() {
        ...
        assert!(error < 0.05, "Wave propagation error: {:.2}%", error * 100.0);
    }
    ...
    fn test_standing_wave_rigid_boundaries() {
        ...
        assert!(energy_error < 0.01, "Energy conservation error: {:.2}%", energy_error * 100.0);
    }
    ...
    fn test_spherical_spreading() {
        ...
        assert!(error < 0.15, "Spherical spreading error: {:.2}%", error * 100.0);
    }
}