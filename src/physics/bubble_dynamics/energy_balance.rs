...
impl EnergyBalanceCalculator {
    ...
    pub fn calculate_energy_rate(
        ...
        let wall_velocity = Velocity::new::<uom::si::velocity::meter_per_second>(
            state.wall_velocity
        );
    ...
    pub fn calculate_heat_transfer_rate(
        &self,
        state: &BubbleState,
        peclet_number: f64,
    ) -> Power {
        ...
        let nusselt = NUSSELT_BASE + NUSSELT_PECLET_COEFF * peclet_number.powf(NUSSELT_PECLET_EXPONENT);
...
pub fn update_temperature_energy_balance(
    ...
    let peclet = calculator.calculate_peclet_number(
        state.radius,
        state.wall_velocity,
        thermal_diffusivity,
    );
    ...
    let latent_heat_rate = Power::new::<watt>(
        mass_transfer_rate * LATENT_HEAT_VAPORIZATION
    );
    
    // Calculate total energy rate
    let energy_rate = calculator.calculate_energy_rate(
        state,
        pressure,
        heat_transfer_rate,
        latent_heat_rate,
    ...
    // Update temperature
    let new_temp = calculator.update_temperature_from_energy(
        state,
        energy_rate,
        time_step,
        heat_capacity,
