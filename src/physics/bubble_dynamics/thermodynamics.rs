...
impl ThermodynamicsCalculator {
    ...
    fn clausius_clapeyron(&self, temperature: f64) -> f64 {
        ...
        let h_vap_t = self.h_vap * ((T_CRITICAL_WATER - temperature) 
            / (T_CRITICAL_WATER - self.t_ref)).powf(0.38);
    ...
    fn wagner_equation(&self, temperature: f64) -> f64 {
        ...
        let ln_pr = (A1 * tau 
            ...
            + A4 * tau.powf(3.5)
            + A5 * tau.powf(4.0)
            + A6 * tau.powf(7.5)) / (temperature / T_CRITICAL_WATER);
...
impl MassTransferModel {
    ...
    pub fn mass_transfer_rate(
        ...
        let coeff = self.thermo.mass_transfer_coefficient(temperature, self.accommodation_coeff);
