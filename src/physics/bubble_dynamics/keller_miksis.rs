...
impl KellerMiksisModel {
    ...
    pub fn calculate_acceleration(
        ...
        let dp_wall_dt = self.estimate_wall_pressure_derivative(
            state, p_gas, r, v, gamma, dp_dt, omega, t
        )?;
    ...
    fn calculate_vdw_pressure(&self, state: &BubbleState) -> KwaversResult<f64> {
        ...
        if volume <= excluded_volume {
            return Err(crate::error::PhysicsError::InvalidParameter {
                parameter: "bubble_volume".to_string(),
                value: volume,
                reason: format!("Volume {} m³ must be greater than excluded volume {} m³", volume, excluded_volume),
            }
            .into());
        }
    ...
    pub fn update_mass_transfer(&self, state: &mut BubbleState, dt: f64) -> KwaversResult<()> {
        ...
        if n_total_new < 0.0 || n_total_new.is_nan() || n_total_new.is_infinite() {
            return Err(crate::error::PhysicsError::InvalidParameter {
                ...
                value: state.n_vapor,
                reason: format!("Invalid vapor content: n_vapor={}, n_total={}", state.n_vapor, n_total_new),
            }
            .into());
        }
    ...
    pub fn update_temperature(&self, state: &mut BubbleState, dt: f64) -> KwaversResult<()> {
        ...
        if !(0.0..=50000.0).contains(&t_new) || t_new.is_nan() || t_new.is_infinite() {
            return Err(crate::error::PhysicsError::InvalidParameter {
                ...
                value: t_new,
                reason: format!("Temperature {} K is outside valid range (0 K < T < 50000 K)", t_new),
            }
            .into());
        }
...
mod tests {
    ...
    fn test_keller_miksis_expansion() {
        ...
        assert!(accel < 0.0, "Expansion should decelerate: accel = {}", accel);
    }
    ...
    fn test_keller_miksis_acoustic_forcing() {
        ...
        assert!(result.is_ok(), "Acoustic forcing calculation should succeed");
            ...
            assert!(
                matches!(
                    ...
                    crate::error::KwaversError::Physics(
                        crate::error::PhysicsError::NumericalInstability { .. }
                    )
                ),
