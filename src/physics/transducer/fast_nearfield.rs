...
//! fields from transducer elements with O(n) complexity instead of O(n²).
//!
//! ## Overview
//!
//! Traditional methods compute the Rayleigh-Sommerfeld integral for each transducer
//! element independently, leading to O(n²) complexity for n elements. The Fast
//! Nearfield Method factorizes the computation to achieve O(n) complexity.
//!
//! ## Mathematical Foundation
//!
//! The pressure field at point (x,y,z) from a transducer element at (x₀,y₀,0)
//! with normal velocity v₀ is:
//!
//! p(x,y,z) = (ρ₀ c₀ k₀ / (2π)) ∫∫ v₀(x₀,y₀) G(x-x₀,y-y₀,z) dx₀ dy₀
//!
//! Where G is the Rayleigh-Sommerfeld Green's function:
//!
//! G(r) = (exp(jkr) / r) * (z / (r(r+z))) * (jkr + 1)
//!
//! ## FNM Factorization
//!
//! The FNM decomposes the Green's function as:
//!
//! G(x,y,z) = (1/(2π)) ∫∫ Ĝ(kx,ky,z) exp(j(kx x + ky y)) dkx dky
//!
//! Where Ĝ is the angular spectrum of the Green's function.
//!
//! ## Implementation
//!
//! 1. **Preprocessing**: Compute angular spectrum factors for each z-plane
//! 2. **Forward Transform**: FFT of transducer velocity distribution
//! 3. **Multiplication**: Multiply by angular spectrum factors
//! 4. **Inverse Transform**: IFFT to get pressure field
//!
//! ## References
//!
...
//! - Kelly, J. F., & McGough, R. J. (2006). "A fast nearfield method for calculating
//!   therapeutic ultrasound fields." *IEEE Transactions on Ultrasonics,
//!   Ferroelectrics, and Frequency Control*, 53(12), 2570-2582.
//!
//! ## Usage
//!
//! ```rust,ignore
//! use kwavers::physics::transducer::fast_nearfield::{FastNearfieldSolver, FNMConfig};
//!
//! // Create FNM solver
//! let config = FNMConfig::default();
//! let mut solver = FastNearfieldSolver::new(config)?;
//!
//! // Setup transducer geometry (rectangular elements)
//! let transducer = RectangularTransducer {
//!     width: 10e-3,    // 10 mm
//!     height: 10e-3,   // 10 mm
//!     frequency: 1e6,  // 1 MHz
//!     elements: (32, 32), // 32x32 element array
//! };
//!
//! // Precompute angular spectrum factors for z = 50 mm
//! solver.precompute_factors(&transducer, 50e-3)?;
//!
//! // Compute pressure field for uniform velocity distribution
//! let velocity = Array2::<Complex<f64>>::ones((32, 32));
//! let pressure = solver.compute_field(&velocity)?;
//!
