...
impl PhysicsTestUtils {
    ...
    pub fn analytical_plane_wave_with_dispersion(
        ...
        let k_corrected = if dispersion_correction {
            ...
            k_analytical
                * (1.0
                    + DISPERSION_CORRECTION_SECOND_ORDER * k_ratio.powi(2)
                    + DISPERSION_CORRECTION_FOURTH_ORDER * k_ratio.powi(4))
        } else {
            k_analytical
        };
...
mod tests {
    ...
    fn test_plane_wave_propagation_corrected() {
        ...
        use crate::constants::physics::{
            DEFAULT_ULTRASOUND_FREQUENCY, STANDARD_PRESSURE_AMPLITUDE,
        };
        ...
        let initial_pressure = PhysicsTestUtils::analytical_plane_wave_with_dispersion(
            ...
            frequency,
            amplitude,
            medium.sound_speed(0.0, 0.0, 0.0, &grid),
            0.0,
            true,
        );
        ...
        info!(
            "Starting plane wave propagation test with {} steps",
            num_steps
        );
        ...
        assert!(
            correlation > 0.0,
            "Wave correlation should be positive, got: {:.2e}",
            correlation
        );
    }
    ...
    fn test_amplitude_preservation() {
        ...
        for i in 0..grid.nx {
            for j in 0..grid.ny {
                for k in 0..grid.nz {
                    ...
                    let r_sq =
                        (x - center_x).powi(2) + (y - center_y).powi(2) + (z - center_z).powi(2);
        ...
        assert!(
            ...
            "Energy conservation violated: ratio={:.4} (expected ~1.0)",
            energy_ratio
        );
    }
    ...
    fn test_acoustic_attenuation() {
        ...
        let medium = HomogeneousMedium::from_minimal(rho0, c0, &grid, 0.0, 0.0)
        ...
        for i in 0..nx {
            let x = i as f64 * dx;
            let envelope =
                amplitude * (-(x - pulse_center).powi(2) / (2.0 * pulse_width.powi(2))).exp();
        ...
        let initial_pressure = fields
            .index_axis(ndarray::Axis(0), UnifiedFieldType::Pressure.index())
            .to_owned();
        let initial_max = initial_pressure
            .iter()
            .map(|&p| p.abs())
        ...
        for step in 0..n_steps {
            ...
            prev_pressure
                .assign(&fields.index_axis(ndarray::Axis(0), UnifiedFieldType::Pressure.index()));

            if step == 0 {
                let pressure_after_1 =
                    fields.index_axis(ndarray::Axis(0), UnifiedFieldType::Pressure.index());
                let max_after_1 = pressure_after_1
                    .iter()
                    .map(|&p| p.abs())
                    .fold(0.0, f64::max);
        ...
        let actual_attenuation = if initial_max > 0.0 {
    ...
    fn test_standing_wave() {
        ...
        for i in 0..nx {
            ...
            let window = if x < window_width {
                ...
                0.5 * (1.0
                    - (PI * (x - ((nx as f64 - 1.0) * dx - window_width)) / window_width).cos())
            } else {
                1.0
            };
    ...
    #[test]
    fn test_kuznetsov_functionality() {
        let _ = env_logger::builder().is_test(true).try_init();
        ...
        assert!(
            max_val < 1e10,
            "Solver produced unreasonably large values: {}",
            max_val
        );

        println!(
            "Basic functionality test passed. Max pressure: {:.2e}",
            max_val
        );
    }
            ...
            for j in 0..grid.ny {
                for k in 0..grid.nz {
                    assert_eq!(
                        pressure[[i, j, k]],
                        expected_value,
                        "Plane wave should have constant value in y-z plane at x index {}",
                        i
                    );
                }
            }
