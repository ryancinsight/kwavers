...
use ndarray::{Array3};
use super::{
    ros_species::{ROSSpecies, ROSConcentrations, calculate_ros_generation},
...
impl SonochemistryModel {
        ...
        for ((i, j, k), state) in bubble_states.indexed_iter() {
            ...
            if state.is_collapsing && state.temperature > 2000.0 {
                ...
                if state.temperature > 3000.0 {
                    let mut plasma = PlasmaChemistry::new(state.temperature, state.pressure_internal);
    ...
    pub fn total_yield(&self) -> SonochemicalYield {
        ...
        if !self.yields.is_empty() {
            total.energy_efficiency = self.yields.values()
                .map(|y| y.energy_efficiency)
                .sum::<f64>() / self.yields.len() as f64;
        }
    ...
    pub fn oxidative_stress(&self) -> Array3<f64> {
        ...
        Array3::from_elem((self.grid.nx, self.grid.ny, self.grid.nz), stress_value)
    }
...
mod tests {
    ...
    fn test_sonochemistry_model() {
        ...
        let mut bubble_states = Array3::from_elem((10, 10, 10), BubbleState {
            radius: 5e-6,
            temperature: 300.0,
    ...
    fn test_ph_update() {
        ...
        // Add some H2O2
        if let Some(h2o2) = model.ros_concentrations.get_mut(ROSSpecies::HydrogenPeroxide) {
