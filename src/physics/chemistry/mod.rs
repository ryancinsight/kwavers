...
use crate::error::{GridError, KwaversError, KwaversResult, PhysicsError, ValidationError};
use crate::grid::Grid;
...
impl<'a> ChemicalUpdateParams<'a> {
    ...
    pub fn new(
        ...
        if light.shape() != expected_shape {
            return Err(KwaversError::Grid(GridError::InvalidDimensions {
                ...
                nz: light.shape()[2],
                reason: format!(
                    "Light array shape {:?} doesn't match grid dimensions {:?}",
                    light.shape(),
                    expected_shape
                ),
            }));
        ...
        if emission_spectrum.shape() != expected_shape {
            return Err(KwaversError::Grid(GridError::InvalidDimensions {
                ...
                reason: format!(
                    "Emission spectrum array shape {:?} doesn't match grid dimensions {:?}",
                    emission_spectrum.shape(),
                    expected_shape
                ),
            }));
        }
        ...
        if bubble_radius.shape() != expected_shape {
            return Err(KwaversError::Grid(GridError::InvalidDimensions {
                ...
                reason: format!(
                    "Bubble radius array shape {:?} doesn't match grid dimensions {:?}",
                    bubble_radius.shape(),
                    expected_shape
                ),
            }));
        ...
        if temperature.shape() != expected_shape {
            return Err(KwaversError::Grid(GridError::InvalidDimensions {
                ...
                reason: format!(
                    "Temperature array shape {:?} doesn't match grid dimensions {:?}",
                    temperature.shape(),
                    expected_shape
                ),
            }));
    ...
    pub fn validate(&self) -> KwaversResult<()> {
        ...
        let invalid_pressure_values: Vec<f64> = self
            .pressure
            .iter()
        ...
        let negative_temperatures: Vec<f64> = self
            .temperature
            .iter()
            .filter(|&&x| x < 0.0)
            .copied()
        ...
        if !negative_temperatures.is_empty() {
            return Err(ValidationError::FieldValidation {
                ...
                constraint: format!(
                    "Must be non-negative. Found {} negative values",
        ...
        let high_temperatures: Vec<f64> = self
            .temperature
            .iter()
            .filter(|&&x| x > 1000.0)
            .copied()
        ...
        if !high_temperatures.is_empty() {
            log::warn!(
                "High temperatures detected in chemical model: {} values > 1000K, max: {:.2e} K",
        ...
        let high_pressures: Vec<f64> = self
            .pressure
            .iter()
        ...
        if !high_pressures.is_empty() {
            log::warn!(
                "High pressures detected in chemical model: {} values > 100 MPa, max: {:.2e} Pa",
                high_pressures.len(),
...
impl ChemicalMetrics {
    ...
    pub fn get_average_times(&self) -> HashMap<String, f64> {
        ...
        averages.insert(
            "radical_initiation_time".to_string(),
            self.radical_initiation_time / count,
        );
        averages.insert("kinetics_time".to_string(), self.kinetics_time / count);
        averages.insert(
            "photochemical_time".to_string(),
            self.photochemical_time / count,
        );
        averages.insert(
...
impl ChemicalModel {
    ...
    pub fn add_reaction_config(
        &mut self,
        name: String,
        config: ChemicalReactionConfig,
    ) -> KwaversResult<()> {
        config.validate()?;
    ...
    pub fn update_chemical_with_views(
        ...
        let params = ChemicalUpdateParams::new(
            ...
            dt,
            medium,
            crate::constants::physics::DEFAULT_ULTRASOUND_FREQUENCY, // Default frequency
        )
        .unwrap_or_else(|e| {
...
impl ChemicalModelTrait for ChemicalModel {
    fn update_chemical(
        ...
        context
            .metadata
            .insert("nx".to_string(), (grid.nx as f64).to_string());
        context
            .metadata
            .insert("ny".to_string(), (grid.ny as f64).to_string());
        context
            .metadata
            .insert("nz".to_string(), (grid.nz as f64).to_string());
        context
            .metadata
            .insert("dx".to_string(), grid.dx.to_string());
        context
            .metadata
            .insert("dy".to_string(), grid.dy.to_string());
        context
            .metadata
            .insert("dz".to_string(), grid.dz.to_string());

        // Add medium parameters
        context.metadata.insert(
            "density".to_string(),
            medium.density(0.0, 0.0, 0.0, grid).to_string(),
        );
        context.metadata.insert(
            "sound_speed".to_string(),
            medium.sound_speed(0.0, 0.0, 0.0, grid).to_string(),
        );
