...
impl AcousticWaveSolver {
    ...
    pub fn update_wave(
        ...
        match self.config.model_type {
            ...
            AcousticModelType::Westervelt => self.update_westervelt(pressure, medium, source_term, dt)?,
            AcousticModelType::Kuznetsov => self.update_kuznetsov(pressure, medium, source_term, dt)?,
        }
    ...
    fn update_linear(
        ...
        Zip::from(&mut pressure_complex).and(pressure.view())
    ...
    fn compute_kuznetsov_rhs(
        ...
        let mut pressure_complex = Array3::<Complex<f64>>::zeros(pressure.dim());
        Zip::from(&mut pressure_complex).and(pressure)
            .for_each(|c, &r| *c = Complex::new(r, 0.0));
        ...
        Zip::from(&mut laplacian).and(&laplacian_hat)
            .for_each(|l, c| *l = c.re);
    ...
    fn compute_gradient_spectral(&self, field: &Array3<f64>) -> KwaversResult<Array3<f64>> {
        ...
        Zip::from(&mut field_complex).and(field)
            .for_each(|c, &r| *c = Complex::new(r, 0.0));
        ...
        Zip::from(&mut grad_x_hat).and(&self.kx)
            .for_each(|g, &k| *g *= Complex::new(0.0, k));
        Zip::from(&mut grad_y_hat).and(&self.ky)
            .for_each(|g, &k| *g *= Complex::new(0.0, k));
        Zip::from(&mut grad_z_hat).and(&self.kz)
            .for_each(|g, &k| *g *= Complex::new(0.0, k));
        ...
        Zip::from(&mut grad_x).and(&grad_x_hat).for_each(|g, c| *g = c.re);
        Zip::from(&mut grad_y).and(&grad_y_hat).for_each(|g, c| *g = c.re);
        Zip::from(&mut grad_z).and(&grad_z_hat).for_each(|g, c| *g = c.re);
        
        let mut grad_magnitude = Array3::zeros(field.dim());
        Zip::from(&mut grad_magnitude)
            .and(&grad_x).and(&grad_y).and(&grad_z)
            .for_each(|g, &gx, &gy, &gz| {
            ...
            1 => {
                ...
                Zip::from(pressure_hat)
                    ...
                    .for_each(|p, &k2| {
                        let k = k2.sqrt();
                        let arg = k * dx / 2.0;
                        let sinc = if arg.abs() < 1e-10 { 1.0 } else { arg.sin() / arg };
                        *p *= sinc.powi(2);
                    });
            }
            2 => {
    ...
    fn update_max_sound_speed(&mut self, medium: &dyn Medium) {
        let sound_speed = medium.sound_speed_array();
        self.max_sound_speed = sound_speed.iter()
    ...
    fn check_stability(&self, dt: f64) -> KwaversResult<()> {
        ...
        if cfl > max_cfl {
            return Err(KwaversError::Validation(ValidationError::FieldValidation {
                field: "CFL".to_string(),
                value: format!("{}", cfl),
                constraint: format!("must be <= {} (c_max={}, dt={}, dx={})", 
