...
#[cfg(test)]
mod tests {
    use super::super::{KuznetsovConfig, KuznetsovWave, AcousticEquationMode};
    use crate::grid::Grid;
    use crate::medium::HomogeneousMedium;
    use crate::physics::constants::{SOUND_SPEED_WATER, DENSITY_WATER};
    ...
    fn test_homogeneous_no_warning() {
        ...
        solver.update_wave(&mut fields, &prev, &source, &grid, &medium, 1e-7, 0.0).unwrap();
    }
            ...
            for j in 0..grid.ny {
                for i in 0..grid.nx {
                    let r2 = ((i as f64 - center.0 as f64).powi(2) +
                              ...
                              (k as f64 - center.2 as f64).powi(2)) / (sigma * sigma);
    ...
    fn test_nonlinear_harmonic_generation() {
        ...
        let pressure = fields.index_axis(ndarray::Axis(0), 0);
        let probe_point = pressure.slice(ndarray::s![grid.nx*3/4, grid.ny/2, grid.nz/2]).to_owned();
        
        // In nonlinear propagation, we expect energy at harmonics
        // This is a simplified check - proper spectral analysis would use FFT
        assert!(probe_point.abs() > 0.0, "Should have non-zero pressure at probe point");
    }
}