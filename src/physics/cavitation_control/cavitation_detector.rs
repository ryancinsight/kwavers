...
impl CavitationDetector for SpectralDetector {
    fn detect(&mut self, signal: &ArrayView1<f64>) -> CavitationMetrics {
        ...
        if fundamental_bin < psd.len() {
            ...
            for n in 2..=5 {
                ...
                if harmonic_bin < psd.len() {
                    harmonic_content +=
                        psd[harmonic_bin] / fundamental_power.max(MIN_SPECTRAL_POWER);
                }
            }
        }
        ...
        let intensity =
            (subharmonic_level * 0.4 + ultraharmonic_level * 0.3 + broadband_level * 0.3).min(1.0);

        // Calculate confidence based on signal strength
        let signal_power: f64 = psd.sum();
        let confidence = (signal_power / MIN_SPECTRAL_POWER)
            .log10()
            .max(0.0)
            .min(1.0)
            / 6.0;
