            ...
            for k in 1..(grid.nz - 1) {
                ...
                for &(ni, nj, nk) in &neighbors {
                    ...
                    if relative_jump > threshold {
                        ...
                        let dx = (density_array[[i + 1, j, k]] - density_array[[i - 1, j, k]]) / (2.0 * grid.dx);
                        let dy = (density_array[[i, j + 1, k]] - density_array[[i, j - 1, k]]) / (2.0 * grid.dy);
                        let dz = (density_array[[i, j, k + 1]] - density_array[[i, j, k - 1]]) / (2.0 * grid.dz);
