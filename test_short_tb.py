"""Short tone burst: wave hasn't reached boundary, no PML effects."""
import sys
...
f0 = 0.5e6
n_cycles = 3
# Very short: wave travels max ~30 steps * 1.5mm/step = 45mm, no boundary reflections
# dt=2e-7, t_end=6e-6 (30 steps) - source emits 3 cycles, barely reaches sensor 1
t_end = 30e-6  # 150 steps to let wave fully pass sensors
...
n_kw = kw_p.shape[1]
n_kwa = kwa_p.shape[1]
if n_kwa == n_kw + 1: kw_aligned, kwa_aligned = kw_p, kwa_p[:, 1:]
elif n_kw == n_kwa + 1: kw_aligned, kwa_aligned = kw_p[:, 1:], kwa_p
elif n_kw == n_kwa: kw_aligned, kwa_aligned = kw_p[:, 1:], kwa_p[:, :-1]
else:
    n = min(n_kw, n_kwa)
    kw_aligned, kwa_aligned = kw_p[:, :n], kwa_p[:, :n]
print(f"aligned: kw={kw_aligned.shape}, kwa={kwa_aligned.shape}")
...
for i, off in enumerate(sensor_offsets):
    ...
    print(f"  Sensor +{off}: corr={corr:.4f}, rms_ratio={rms_r:.4f}, amp_ratio={amp_r:.4f}")
