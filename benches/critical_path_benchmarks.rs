...
fn bench_fdtd_derivatives(c: &mut Criterion) {
    ...
    for order in [2, 4, 6] {
        ...
        for size in [32, 64, 128] {
            let _grid = Grid::new(size, size, size, 0.001, 0.001, 0.001)
            ...
            group.bench_with_input(
                ...
                |b, _| {
                    b.iter(|| {
                        let deriv = fd.compute_derivative(&field.view(), black_box(0), spacing)
...
fn bench_kspace_operators(c: &mut Criterion) {
    ...
    for size in [32, 64, 128, 256] {
        let grid = Grid::new(size, size, size, 0.001, 0.001, 0.001)
            .expect("Grid creation");
            ...
            for i in (0..128).step_by(8) {
                for j in (0..128).step_by(8) {
                    for k in (0..128).step_by(8) {
                        let coords = grid.indices_to_coordinates(
                            black_box(i),
                            black_box(j),
                            black_box(k)
                        );
            ...
            for x in [0.01, 0.05, 0.10] {
                for y in [0.01, 0.05, 0.10] {
                    for z in [0.01, 0.05, 0.10] {
                        let indices = grid.coordinates_to_indices(
                            black_box(x),
                            black_box(y),
                            black_box(z)
                        );
...
fn bench_field_operations(c: &mut Criterion) {
    ...
    for size in [64, 128] {
        ...
        group.bench_with_input(
            BenchmarkId::new("element_add", size),
            &size,
            |b, _| {
                b.iter(|| {
                    field1.iter_mut()
