...
|---|---|---|---|---|
| `KWAVERS-AEQ-MET-13` | `kwavers-driver/src/validate/kwavers_beam.rs`, `experiment/acoustic.rs`, and `experiment/metrics.rs` exposed aperture, frequency, sound speed, focal depth, timing, pitch, wavelength, pressure, intensity, and beam extents as raw SI scalars after the transducer boundary was typed. | Type driver beam-step and acoustic/aggregate result DTOs with Aequitas quantities; convert only at manifest text, numerical, and check-report boundaries. Keep lane indices, mechanical index, grating/far-field flags, and model coefficients at their explicit dimensionless/model boundaries. | Kwavers, Aequitas | **IMPLEMENTED in this increment; verification blocked.** `KwaversBeamStep`, `KwaversBeamValidation`, `PressureMap`, `ThermalState`, and `ExperimentMetrics` now carry Aequitas geometry, timing, pressure, intensity, power-margin, and temperature-difference quantities; scalar extraction is explicit at manifest, formula, and check-report boundaries. Direct rustfmt and diff checks pass. Cargo cannot load the workspace before source compilation because `D:\atlas\worktrees\coeus\coeus-autograd\Cargo.toml` is missing. See [ADR 059](docs/ADR/059-driver-beam-result-quantities.md). |
| `KWAVERS-AEQ-MET-12` | `kwavers-transducer/src/design/{mod,propagation}.rs` exposed aperture dimensions, pitch, element and kerf dimensions, wavelength, frequency, sound speed, focal pressure, intensity, and focal extents as raw SI scalars. The closed-form design and focused-propagation calculations already have value-semantic tests. | Type the public design and propagation contracts with Aequitas `Length`, `Frequency`, `Velocity`, `Pressure`, `Intensity`, `AcousticImpedance`, `ElectricCurrent`, and `PressurePerElectricCurrent`; retain coordinate arrays and dimensionless model flags at their explicit source/grid boundaries. | Kwavers, Aequitas | **IMPLEMENTED in this increment; verification blocked.** Design geometry and propagation inputs/results now use Aequitas quantities; driver callers extract at the explicit source/kernel boundary. Direct rustfmt and diff checks pass. Consumer Cargo cannot load `D:\atlas\worktrees\coeus\coeus-autograd\Cargo.toml`; provider Nextest is queued behind the shared target lock. See [ADR 058](docs/ADR/058-transducer-design-propagation-quantities.md). |
...
| `KWAVERS-AEQ-MET-11` | `kwavers-transducer/src/ultrafast/sequencer` exposed scheduled transmission time, tilt angle, PRF, frame rate, sound speed, and imaging depth as raw scalars with unit-bearing documentation. The schedule is a public physical timing/geometry contract. | Carry scheduled times as `Time`, angles as `Angle`, rates as `Frequency`, sound speed as `Velocity`, and depth as `Length`; retain event indices and element indices as structural values. | Kwavers | **IMPLEMENTED in this increment; verification blocked before source compilation by the peer Coeus graph.** `TransmissionSequencer`, `TransmissionEvent`, and `TransmissionSchedule` use Aequitas quantities; analytical PRF, timing, interleaving, flash, STA, and frame-rate tests migrate to typed values. Rustfmt/diff checks pass; package checks, Nextest, warning-denied Clippy, doctests, and Rustdoc remain blocked at `D:\atlas\worktrees\coeus\coeus-autograd\Cargo.toml`. See [ADR 057](docs/ADR/057-ultrafast-sequencer-quantities.md). |
