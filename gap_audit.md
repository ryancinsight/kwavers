...
|---|---|---|---|---|
| `KWAVERS-AEQ-MET-12` | `kwavers-transducer/src/design/{mod,propagation}.rs` exposes aperture dimensions, pitch, element and kerf dimensions, wavelength, frequency, sound speed, focal pressure, intensity, and focal extents as raw SI scalars. The closed-form design and focused-propagation calculations already have value-semantic tests. | Type the public design and propagation contracts with Aequitas `Length`, `Frequency`, `Velocity`, `Pressure`, `Intensity`, `AcousticImpedance`, `ElectricCurrent`, and `PressurePerElectricCurrent`; retain coordinate arrays and dimensionless model flags at their explicit source/grid boundaries. | Kwavers, Aequitas | **CLAIMED; implementation in progress.** Driver manifest and beam-step fields are excluded from this slice and remain a separate serialization/validation boundary to audit after this contract closes. See [ADR 058](docs/ADR/058-transducer-design-propagation-quantities.md). |
