...
`kwavers-physics::electromagnetic::plasmonics`. Particle radii, wavelengths,
array positions and spacing, particle number density, and scattering and
absorption cross-sections were raw SI scalars. The complex Mie polarizability
also crossed the public boundary without a physical dimension. The typed
closure uses Aequitas `Length`, `NumberDensity`, `Area`, `Polarizability`,
`Frequency`, and `ReciprocalVolume`; relative dielectric functions and field,
medium, and Purcell enhancement factors use dimensionless quantities.

The existing Eunomia complex representation remains the scalar provider:
`Polarizability<Complex64>` preserves real and quadrature components under
linear unit conversion, with no imaginary-unit type. Scalar extraction remains
inside Johnson-Christy interpolation, Drude/Mie formulas, and dense numerical
geometry. The separate plasmonic equation trait and nanoparticle geometry enum
were migrated in the same boundary so no raw public sibling API remains.

The provider addition is Aequitas `Polarizability` with coherent
`FaradSquareMeter`, plus semantic `ReciprocalVolume` for geometric coupling
coefficients. Closure evidence is Aequitas `cargo check --tests --offline`
plus nextest run `6190f2e1-92f1-4879-8938-66ed2fa0a8f1` (1/1 complex-unit
law), Kwavers `cargo check -p kwavers-physics --tests --offline`, nextest run
`bcb725a9-8714-4cb5-84d0-2a167e91a545` (10/10 analytical tests), warning-denied
package Clippy, targeted rustfmt, and a clean raw-signature residue scan.
Provider and consumer locks may still carry peer-owned resolver edits; those
are integration state, not metric gaps. See [ADR 071](docs/ADR/071-plasmonics-quantities.md).
...
## Live sonogenetics refresh — 2026-07-27

`KWAVERS-AEQ-MET-30` closes the remaining named-consumer metric gap. Aequitas
now provides `ElectricCharge`, `ElectricPotential`, `ElectricConductance`, and
`Capacitance` with coherent Coulomb, Volt, Siemens, and Farad units. Sonogenetics
membrane geometry, channel gating parameters, ion-current contracts, and LIF
parameters/state now carry typed `Length`, `Area`, `SurfaceTension`, `Pressure`,
`ElectricConductance`, `ElectricPotential`, `ElectricCurrent`, `Capacitance`,
`Time`, and `Frequency` values. Dense fields, probabilities, channel counts,
and phenomenological thermal-coupling coefficients remain explicit numerical or
...
edges are recorded in `Cargo.lock`. The obsolete Consus NPY branch selector is
removed, and `cargo metadata --locked --no-deps` passes outside the Atlas
overlay. The no-overlay package check now reaches Tyche and is blocked by
Tyche `main`'s nonportable sibling `../eunomia` path dependency; the overlay
...
Shock-wave peak pressure, pulse duration, and repetition rate now use Aequitas
`Pressure`, `Time`, and `Frequency`. The shock-wave, stone-fracture, cavitation
cloud, and bioeffects solver components remain separate implementation work;
they are not represented as completed by this metric closure.

The physics test-target check, focused therapy tests, warning-denied Clippy,
doctests, Rustdoc, rustfmt, and `git diff --check` pass. See [ADR 066](docs/ADR/066-lithotripsy-quantities.md).

## Live therapeutic-cavitation refresh — 2026-07-27

`KWAVERS-AEQ-MET-27` closes the public therapeutic cavitation detector gap.
Detector frequency, nucleus radius, Blake threshold, Minnaert resonance
frequency, and pressure inputs now use Aequitas `Frequency`, `Length`, and
`Pressure`. Cavitation index/probability and stable/inertial classifications
remain dimensionless or boolean model outputs. Dense `Array3<f64>` pressure
fields remain an explicit storage/formula boundary. The constructor's unused
pressure argument was removed; the provider-backed Blake threshold is now
returned as typed `Pressure`.

The physics test-target check passes; the cavitation-filtered Nextest run passes
259/259 with 1,298 skipped; doctests pass 8/8 with 4 ignored; warning-denied
Clippy passes; Rustdoc exits 0 with two pre-existing link warnings; and touched
rustfmt plus `git diff --check` pass. See [ADR 065](docs/ADR/065-cavitation-quantities.md).

## Live hemispherical-array refresh — 2026-07-27

`KWAVERS-AEQ-MET-26` closes the live public hemispherical-array contract gap
in `kwavers-transducer`. Hemisphere geometry, element positions/radii and
phase, steering frequency/sound speed/time, focal pressure, focal volume,
validation pressure, steering angle, and the nominal source sound-speed
constant now use Aequitas `Length`, `Angle`, `Frequency`, `Velocity`, `Time`,
`Pressure`, and `Volume`. The source waveform uses the configured frequency;
it no longer embeds a 650 kHz literal. Normals, apodization, efficiency,
grating-lobe ratios, and element counts remain dimensionless or structural.
Scalar extraction is confined to geometry formulas, mesh/source construction,
Signal, and logging boundaries. See [ADR 064](docs/ADR/064-hemispherical-quantities.md).
...
## Live cavitation-control refresh — 2026-07-27

`KWAVERS-AEQ-MET-15` is implemented on the current branch. The public
cavitation-control boundaries now carry Aequitas `Frequency` for detector
carrier/sample rates and controller carrier/output frequency, `Time` for
response and pulse timing, and `Pressure`/`ThermodynamicTemperature` for
safety limits. Therapy callers and the detection doctest use the typed
contracts. Frequency modulation applies its dimensionless shift to the typed
carrier before reporting the output; detector levels, duty cycles, amplitudes,
control scores, and frequency shifts remain dimensionless model values.

The unused raw sample-rate parameter was removed from `PowerModulator::new`;
no in-repository callers existed. The clean photoacoustic caller was also
updated from the removed peer solver method `solve_leto` to `solve`, because
...
## Live acoustic-solver refresh — 2026-07-27

`KWAVERS-AEQ-MET-17` is resolved in commit `328a46f03`. The public
`AcousticWaveSolver` boundary now returns Aequitas `Pressure` and `Intensity`
values and accepts/returns `Time` for durations, time steps, and current time.
The acoustic heating focal depth is typed `Length`; safety, cavitation,
chemical, microbubble, and lithotripsy helper intervals are typed `Time`.
Dense Leto fields remain scalar mesh storage, with extraction confined to
backend, formula, and mesh boundaries. The package check passes; focused
Nextest passes 349/349 with one skip and two slow tests in 37.399 seconds;
doctests pass 8/8 with one ignored, warning-denied Clippy passes, and Rustdoc
exits 0. The shared graph still emits peer-provider warnings and unused local
patch warnings; they are outside this slice.
...
and typed Rayleigh propagation contracts through the public transducer physics
boundary. This refresh also types annular-sector start/span angles as Aequitas
`Angle`, with radians retained only at the quadrature trigonometric boundary.
This increment closes the remaining public material scalar:
`PiezoMaterial::curie_temperature` is now Aequitas
`ThermodynamicTemperature` stored in kelvin, with the Celsius catalog values
converted at construction. Focused `kwavers-transducer` Nextest passes 218/218
with one skip; doctests pass 1/1 with 6 ignored; Rustdoc exits 0 with the
pre-existing unresolved link at `design/mod.rs:151`; and warning-denied Clippy
for the transducer library/tests exits 0. Direct rustfmt checks pass for the
two touched Rust files; the package-wide formatter still reports its
pre-existing diffs in `rasterizer_planar.rs`, `tests/elements.rs`, and
`physics/mod.rs`.
...
| `KWAVERS-AEQ-MET-30` | `kwavers-physics/src/acoustics/therapy/sonogenetics/**` exposed membrane geometry and LIF/channel electrical quantities as raw metres, farads, siemens, volts, and seconds. Aequitas lacked the required capacitance, conductance, potential, and charge dimensions. | Add the missing electrical dimensions upstream in Aequitas, then type sonogenetics public contracts and migrate callers/tests without consumer wrappers; retain dimensionless gating, probabilities, and dense fields at their formula/storage boundaries. | Aequitas, Kwavers | **RESOLVED in this increment.** Aequitas provides the four electrical dimensions and coherent SI units; sonogenetics public membrane, channel, ion-current, and LIF contracts use typed quantities. `kwavers-physics` Nextest passes 1,556/1,556 with one skip; package check, warning-denied Clippy, doctests, targeted rustfmt, and diff checks pass. See [ADR 068](docs/ADR/068-sonogenetics-quantities.md). |
| `KWAVERS-AEQ-MET-33` | `kwavers-physics/src/electromagnetic/plasmonics/**` exposed nanoparticle geometry, optical wavelength, particle concentration, near-field coupling, cross-sections, and complex polarizability as raw values. | Type the public plasmonics contracts with Aequitas and add provider-owned `Polarizability`/`ReciprocalVolume` dimensions; keep relative dielectric and enhancement metrics dimensionless. | Aequitas, Kwavers | **RESOLVED in `f6a77671e`.** Mie, effective-medium, coherent-array, nanoparticle-geometry, and Drude equation APIs use typed quantities; the complex polarizability path uses Eunomia `Complex64`. Package check, nextest 10/10, warning-denied Clippy, rustfmt, and residue scans pass. See [ADR 071](docs/ADR/071-plasmonics-quantities.md). |
...
- `KWAVERS-AEQ-MET-05` reaches the public diagnostics caller and
  ...
  `KWAVERS-AEQ-MET-29` closes the CEUS imaging gap, and
  `KWAVERS-AEQ-MET-30` closes the sonogenetics electrical-contract gap. The
  named CFDrs, Helios, and Kwavers consumer metric ledger is empty; the next
  public inventory re-audit remains required after a new provider or consumer
...
- **kwavers-physics focused direct ndarray/Rayon edge - RESOLVED [patch].**
  ...
  `parallel.rs` now owns tuple-output `zip_mut_with` so one-input updates and
  ARF's intensity/body-force update share the traversal SSOT.
  Evidence tier:
  compile-time integration, focused empirical tests, and static source audit.
  `rustup run nightly cargo check -p kwavers-physics --lib` passed; `rustup run
...
- **Atlas extension: Eunomia Complex64 SSOT for `csr.rs` - RESOLVED (2026-07-29).** The former `num_complex::Complex64 → CsrScalar` and `num_traits` residue no longer exists in the live source. `kwavers-math::linear_algebra::sparse::csr` uses `eunomia::Complex64` directly for its complex specialization, while Eunomia supplies the generic `ComplexField` and unit-scaling contracts. No imaginary-unit type or unsealed `NumericElement` implementation is required at this boundary. `cargo check -p kwavers-math --tests --offline` passes against the current provider graph.
...
- **kwavers-physics direct Rayon edge - RESOLVED [patch].**
  ...
  field update through the reusable tuple-output form of
  `crate::parallel::zip_mut_with` instead of ndarray/Rayon `Zip::par_for_each`.
  The regression
