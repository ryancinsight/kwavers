...
| `KWAVERS-AEQ-MET-04` | `kwavers-therapy/src/therapy/hifu_planning/{types,schedule}.rs` exposed focal dimensions/volumes, power, peak pressure, frequency, dwell, and temperature metrics as suffixed scalar fields. | Type the planning DTOs and derived metrics through the existing Aequitas seam; leave mechanical index and CEM43 as dimensionless/consumer-semantic values. | Kwavers | **IMPLEMENTED in `a5c101a4c`; verification blocked.** HIFU transducer frequency/power/geometry, focal pressure/width/volume/location, ablation-target geometry, and sonication schedule coordinates now use Aequitas `Frequency`, `Power`, `Length`, `Pressure`, `Volume`, and validated `CartesianPosition`; dwell and temperatures remain typed. Scalar conversion is confined to closed-form model arithmetic. Rustfmt, metadata, diff, and source audit pass; HIFU planning value/formula tests, package Clippy, and doctests remain pending because the package check is blocked before Kwavers sources by peer `coeus-nn` API drift. See ADR 052. |
