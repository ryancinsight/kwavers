...
| `KWAVERS-AEQ-MET-03` | `kwavers-transducer/src/transducers/physics/{frequency,geometry,rayleigh,materials}.rs` exposes frequency, bandwidth, lengths, area/volume, range, attenuation, wavelength, and acoustic impedance as raw values. | Migrate the bounded transducer modules to typed constructors/results; retain dimensionless Q, bandwidth fraction, directivity, reflection coefficients, attenuation coefficients, and coherent phase/pressure accumulations as scalars. | Kwavers | **PARTIAL after two increments.** `frequency.rs`, `geometry.rs`, `design.rs`, and the material/lens APIs now use Aequitas `Frequency`, `Length`, `Velocity`, `Area`, `Volume`, `MassDensity`, `AcousticImpedance`, `ThermodynamicTemperature`, `Time`, and `ReciprocalLength`; Rayleigh propagation coefficients and ranges are typed. Material/lens and Rayleigh value oracles remain green in the full 221/221 transducer suite. Residual: Rayleigh aperture coordinates, radii, observation points, and the KWaveArray rasterizer boundary remain scalar. See [ADR 050](docs/ADR/050-transducer-materials-rayleigh-quantities.md). |
...
- `KWAVERS-AEQ-MET-03` is partially implemented: frequency, element geometry,
  materials/lenses, and Rayleigh propagation coefficients are closed for the
  current increments; aperture coordinates and observation-point arrays remain
  open. The parent audit must be refreshed after each child slice so the
