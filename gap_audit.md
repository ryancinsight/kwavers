...
coefficients. Acceptance is the unchanged plasmonics analytical suite,
warning-denied package Clippy, rustfmt, and a source scan for the old raw
plasmonics signatures. Provider and consumer locks may still carry peer-owned
resolver edits; those are integration state, not metric gaps. See
[ADR 071](docs/ADR/071-plasmonics-quantities.md).
...
| `KWAVERS-AEQ-MET-33` | `kwavers-physics/src/electromagnetic/plasmonics/**` exposed nanoparticle geometry, optical wavelength, particle concentration, near-field coupling, cross-sections, and complex polarizability as raw values. | Type the public plasmonics contracts with Aequitas and add provider-owned `Polarizability`/`ReciprocalVolume` dimensions; keep relative dielectric and enhancement metrics dimensionless. | Aequitas, Kwavers | **IMPLEMENTED in this increment.** Mie, effective-medium, coherent-array, nanoparticle-geometry, and Drude equation APIs use typed quantities; the complex polarizability path uses Eunomia `Complex64`. Focused package check, unchanged analytical plasmonics tests, warning-denied Clippy, rustfmt, and residue scans are the acceptance gates. See [ADR 071](docs/ADR/071-plasmonics-quantities.md). |
