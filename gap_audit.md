...
audit. The affected Kwavers packages pass `cargo check --offline --locked`,
warning-denied Clippy, and doctests. The broader Nextest run passes 2,913/2,913
...
|---|---|---|---|---|
| `KWAVERS-AEQ-MET-13` | `kwavers-driver/src/validate/kwavers_beam.rs`, `experiment/acoustic.rs`, and `experiment/metrics.rs` exposed aperture, frequency, sound speed, focal depth, timing, pitch, wavelength, pressure, intensity, and beam extents as raw SI scalars after the transducer boundary was typed. | Type driver beam-step and acoustic/aggregate result DTOs with Aequitas quantities; convert only at manifest text, numerical, and check-report boundaries. Keep lane indices, mechanical index, grating/far-field flags, and model coefficients at their explicit dimensionless/model boundaries. | Kwavers, Aequitas | **VERIFIED in this increment.** `KwaversBeamStep`, `KwaversBeamValidation`, `PressureMap`, `ThermalState`, and `ExperimentMetrics` carry Aequitas geometry, timing, pressure, intensity, power-margin, and temperature-difference quantities. The Coeus paths now target `../coeus/crates/*`; workspace check, driver Nextest 487/487, warning-denied Clippy, and driver doctests pass. See [ADR 059](docs/ADR/059-driver-beam-result-quantities.md). |
| `KWAVERS-AEQ-MET-12` | `kwavers-transducer/src/design/{mod,propagation}.rs` exposed aperture dimensions, pitch, element and kerf dimensions, wavelength, frequency, sound speed, focal pressure, intensity, and focal extents as raw SI scalars. The closed-form design and focused-propagation calculations already have value-semantic tests. | Type the public design and propagation contracts with Aequitas `Length`, `Frequency`, `Velocity`, `Pressure`, `Intensity`, `AcousticImpedance`, `ElectricCurrent`, and `PressurePerElectricCurrent`; retain coordinate arrays and dimensionless model flags at their explicit source/grid boundaries. | Kwavers, Aequitas | **VERIFIED in this increment.** Design geometry and propagation inputs/results use Aequitas quantities; driver callers extract at the explicit source/kernel boundary. Workspace check, transducer/boundary Nextest 318/318 (1 skipped), warning-denied Clippy, and doctests pass after correcting the Coeus `../coeus/crates/*` paths. See [ADR 058](docs/ADR/058-transducer-design-propagation-quantities.md). |
| `KWAVERS-AEQ-MET-10` | `kwavers-boundary/src/coupling/impedance/{mod,types}.rs` exposed target/medium acoustic impedance, representative frequency, Gaussian center/bandwidth, and custom profile sample frequencies as raw `f64` values documented in Rayl or Hz. The existing implementation already computes dimensionless impedance ratios and reflection coefficients from those inputs. | Type the physical boundary with Aequitas `AcousticImpedance` and `Frequency`; keep profile response, impedance ratio, and reflection coefficient dimensionless. | Kwavers | **VERIFIED in this increment.** `ImpedanceBoundary` and `FrequencyProfile` carry typed impedance/frequency inputs; duplicate profile interpolation is removed. Workspace check, transducer/boundary Nextest 318/318 (1 skipped), warning-denied Clippy, and doctests pass. See [ADR 056](docs/ADR/056-impedance-boundary-quantities.md). |
| `KWAVERS-AEQ-MET-11` | `kwavers-transducer/src/ultrafast/sequencer` exposed scheduled transmission time, tilt angle, PRF, frame rate, sound speed, and imaging depth as raw scalars with unit-bearing documentation. The schedule is a public physical timing/geometry contract. | Carry scheduled times as `Time`, angles as `Angle`, rates as `Frequency`, sound speed as `Velocity`, and depth as `Length`; retain event indices and element indices as structural values. | Kwavers | **VERIFIED in this increment.** `TransmissionSequencer`, `TransmissionEvent`, and `TransmissionSchedule` use Aequitas quantities; workspace check, transducer/boundary Nextest 318/318 (1 skipped), warning-denied Clippy, and doctests pass after correcting the Coeus local paths. See [ADR 057](docs/ADR/057-ultrafast-sequencer-quantities.md). |
...
| `KWAVERS-AEQ-MET-04` | `kwavers-therapy/src/therapy/hifu_planning/{types,schedule}.rs` exposed focal dimensions/volumes, power, peak pressure, frequency, dwell, and temperature metrics as suffixed scalar fields. | Type the planning DTOs and derived metrics through the existing Aequitas seam; leave mechanical index and CEM43 as dimensionless/consumer-semantic values. | Kwavers | **VERIFIED in this increment.** HIFU transducer frequency/power/geometry, focal pressure/width/volume/location, ablation-target geometry, and sonication schedule coordinates use Aequitas `Frequency`, `Power`, `Length`, `Pressure`, `Volume`, and validated `CartesianPosition`; dwell and temperatures remain typed. `cargo check` passes; the five-package Nextest run passes 2,913/2,913 with two skips; warning-denied Clippy and doctests pass. See ADR 052. |
| `KWAVERS-AEQ-MET-05` | `kwavers-analysis/src/signal_processing/vasculature/mod.rs` reports diameter and total length as voxel-unit `f64`, and Doppler velocity returns `f64`; spacing is left to the caller. | Make physical voxel spacing an explicit validated `Length` input, then return physical `Length`/`Velocity` instead of caller-applied scalars. | Kwavers | **VERIFIED in this increment.** Commit `9f95aa826` supplies validated anisotropic `VoxelSpacing`, physical `Length<f64>` geometry, typed Doppler `Frequency`/`Velocity` boundaries, and value-semantic regressions. The five-package `cargo check`, Nextest, warning-denied Clippy, and doctest gates pass. |
...
| `KWAVERS-AEQ-MET-07` | `kwavers-grid/src/lib.rs` exposed derived spacing, physical size, volume, and CFL timestep values as raw scalars. | Type public derived grid metrics with Aequitas `Length`, `Volume`, `Velocity`, and `Time`; convert only at coordinate and stability-kernel boundaries. | Kwavers | **VERIFIED in this increment.** `min_spacing`, `max_spacing`, `physical_size`, `volume`, `cell_volume`, and `cfl_timestep` carry SI quantities, with direct value tests and migrated callers. `cargo check`, the five-package Nextest run, warning-denied Clippy, and doctests pass. Raw `Grid` storage and coordinate arrays remain numerical kernel boundaries. See ADR 053. |
| `KWAVERS-AEQ-MET-08` | `kwavers-physics` thermal-diffusion/Pennes/Cattaneo configs and `kwavers-simulation::ThermalConfig` exposed perfusion, blood properties, temperature, relaxation, conductivity, frequency, heat, and thermal-step values as raw scalars. | Carry the configuration contracts with Aequitas quantities; compose the typed Pennes coefficient before converting at the numerical kernel boundary; retain Python scalar arguments as the explicit conversion boundary. | Kwavers | **VERIFIED in this increment.** `ReciprocalTime`, `MassDensity`, `SpecificHeatCapacity`, `ThermodynamicTemperature`, `ThermalConductivity`, `VolumetricPowerDensity`, `Frequency`, and `Time` cross the Rust configuration and solver seams. The coupled-config SI serialization round-trip and numerical callers are migrated; `cargo check`, 2,913/2,913 Nextest, warning-denied Clippy, and doctests pass. See ADR 054. |
| `KWAVERS-AEQ-MET-09` | `kwavers-transducer/src/basic/piston.rs` exposed piston centre, diameter/radius, and Gaussian sigma as raw scalar geometry while the adjacent Rayleigh aperture API already used Aequitas `Length` and `CartesianPosition`. | Type the piston geometry and apodization length scale; convert once at the grid/source-kernel boundary and retain raw coordinates only for the `Source` trait contract. | Kwavers | **VERIFIED in this increment.** `PistonConfig`, `PistonBuilder`, accessors, factory construction, Gaussian apodization, and the typed geometry regression use Aequitas. Transducer/boundary Nextest passes 318/318 with one skip; warning-denied Clippy and doctests pass. See ADR 055. |
