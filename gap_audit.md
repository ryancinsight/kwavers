...
- Full `kwavers` library tests pass, but `solver::forward::nonlinear::kzk::solver::tests::test_conservation_diagnostics_disable` and `solver::validation::numerical_accuracy::pstd::tests::test_pstd_phase_velocity_accuracy` exceed the 60-second progress threshold. The next increment should profile the production KZK/PSTD paths and optimize them without reducing test coverage.
- `MieTheory::gold_in_water` still uses simplified Drude-Lorentz constants rather than a table-backed Johnson-Christy/Rakic dielectric strategy, so gold resonance modernization remains open.
