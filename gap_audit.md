...
| `KWAVERS-AEQ-MET-04` | `kwavers-therapy/src/therapy/hifu_planning/{types,schedule}.rs` exposed focal dimensions/volumes, power, peak pressure, frequency, dwell, and temperature metrics as suffixed scalar fields. | Type the planning DTOs and derived metrics through the existing Aequitas seam; leave mechanical index and CEM43 as dimensionless/consumer-semantic values. | Kwavers | **RESOLVED.** HIFU transducer frequency/power/geometry, focal pressure/width/volume/location, ablation-target geometry, and sonication schedule coordinates now use Aequitas `Frequency`, `Power`, `Length`, `Pressure`, `Volume`, and validated `CartesianPosition`; dwell and temperatures remain typed. Scalar conversion is confined to closed-form model arithmetic. HIFU planning value/formula tests, package Clippy, doctests, and format gates are pending the external Coeus dependency repair; the attempted package check is blocked before Kwavers sources by peer `coeus-nn` API drift. See ADR 052. |
