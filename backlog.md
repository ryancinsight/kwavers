...
| KWAVERS-AEQ-MET-40 | Type clinical workflow orchestration latency, stage timing, GPU utilization, memory accounting, and acquisition/processing timing boundaries with Aequitas where dimensions exist; preserve raw byte accounting only at the explicit storage boundary. | [major] | in-progress | Codex | `crates/kwavers-diagnostics/src/workflows/{config,results,orchestrator}/**`, ADR 078, PM artifacts; package test-target check and warning-denied library Clippy pass; focused Nextest/doctest collection blocked by peer-owned `ritk-mgh` and `kwavers-grid` compile failures |
