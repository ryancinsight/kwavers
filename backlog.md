...
| KWAVERS-AEQ-MET-53 | Type the public therapeutic microbubble state, shell properties, radiation force, and streaming-velocity contracts with Aequitas; add provider-owned acceleration vocabulary; keep Keller–Miksis/Marmottant formula scalars and future complex acoustic fields at explicit numerical boundaries. | [major] | in-progress | current session | `crates/kwavers-physics/src/therapy/microbubble/{state,shell,forces}/**`, direct `kwavers-therapy` callers/tests, `repos/aequitas` acceleration vocabulary, ADR and synchronized audit artifacts |
