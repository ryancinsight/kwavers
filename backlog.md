...
| KWAVERS-AEQ-MET-41 | Type functional-ultrasound stereotactic coordinates, Bregma, confidence, and targeting distance with Aequitas; keep scalar extraction only at the atlas millimetre conversion boundary and preserve real-valued Eunomia semantics. | [major] | in-progress | Codex | `crates/kwavers-diagnostics/src/functional_ultrasound/targeting.rs`, direct callers/tests, ADR 080, PM artifacts; public API migration, typed value-semantic regressions, package check/Nextest/Clippy/doctest/RustDoc/format/diff gates |
