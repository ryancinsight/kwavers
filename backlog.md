...
| KWAVERS-AEQ-MET-50 | Type OpenPros mean absolute and root mean square sound-speed errors with Aequitas `Velocity`; extract base scalars only inside the numerical image comparison, retain normalized/correlation/objective values and Leto image storage as explicit numerical/provider boundaries. | [major] | in-progress | Codex | `crates/kwavers-diagnostics/src/reconstruction/sound_speed_shift/openpros_benchmark/{metrics,types,tests}.rs`, ADR 089, PM artifacts |
