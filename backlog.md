...
| KWAVERS-AEQ-MET-48 | Type sound-speed-shift reference velocity, grid spacing, curved-path sagitta, and finite-frequency length scales with Aequitas `Velocity` and `Length`; extract base scalars only at ray and solver formulas, retain solver-owned coordinates and dense image storage as explicit boundaries. | [major] | in-progress | Codex | `crates/kwavers-diagnostics/src/reconstruction/sound_speed_shift/{types,ray,propagation,operator}/**`, ADR 087, PM artifacts |
