...
## KW-SOL-088 - FDTD convergence test measured against a half-cell-offset reference [patch] - done 2026-08-13

- The reference was built at `x = i*dx`, but the staggered path stores pressure
  at cell **centres** `(i+0.5)*dx`, with walls at `0` and `L = nx*dx`. Against
  the old pressure-release wall the mismatch did not dominate; against the rigid
  wall (ADR 106) it did, and the observed order fell to 1.
- Correcting the position drops the coarse-grid error from `1.05e-1` to
  `1.97e-5` - four orders of magnitude, so the offset *was* the error.
- **My predicted explanation was falsified and the assertion follows the data.**
  I expected the ratio to drift toward 4 as the grid refined, with the coarse
  point polluted by a higher-order term. Measured across nx = 30/60/120: orders
  2.97 and 2.99 - a stable third-order regime, not a drift. The cause holds up
  though: at `t = T/2` the mode sits at an extremum, where a phase error enters
  as `cos(phi) ~ 1 - phi^2/2`, suppressing the leading `O(dx^2)` term.
- The old `[2.0, 6.0]` ratio band encoded "faster than third order is
  implausible", true away from an extremum but not here. Replaced by an observed
  *order* in `[2.7, 3.3]` on both refinement levels, with the derivation at the
  assertion. Tighter than what it replaced, not looser.
- Also fixed alongside: `prop_invalid_spatial_order_rejected` still treated
...
- Delivery residual: the historical `RUSTSEC-2026-0235` path is resolved by
  ...
  pending.
## KWAVERS-AEQ-MET-65 — Type MEMS crosstalk mechanical impedance [major] [arch] — done 2026-08-05

- Owner: Codex; scope: `crates/kwavers-transducer/src/mems/crosstalk.rs`,
  Aequitas mechanical-impedance quantity, ADR 070, and synchronized audit
  records. CMUT/PMUT cell-output and flexible-array boundaries remain separate
  items.
- Outcome: typed Aequitas geometry and fluid inputs cross the public boundary;
  the complex result is `MechanicalImpedance<eunomia::Complex64>` with coherent
  `kg/s` units for both real and quadrature components.
...
## KWAVERS-AEQ-MET-FLEX — Audit flexible-array dynamic metrics [major] [arch] — done 2026-08-05

- Owner: Codex; scope: the flexible-array configuration, geometry, beamforming,
  direct callers, and tests. The prior dirty claim was stale by board and file
  timestamps; this session takes it over and composes the existing
  position-visitor change without discarding it.
- Candidate contracts: timestamps and delays use `Time`; focus coordinates use
  `Length`; sound speed uses `Velocity`; calibration confidence and quality
  ratios use `Dimensionless`; position uncertainty uses `Length`; orientation
  uncertainty uses `Angle`; strain and safety limits use `Dimensionless`;
  curvature/radius use reciprocal-length and length; stress uses `Pressure`;
  deformation strain-energy density uses `EnergyPerVolume`.
...
- Outcome: typed timestamps, focus/speed/delay contracts, calibration
  confidence, and deformation outputs with raw extraction limited to mesh,
  calibration, and numerical formula boundaries. The old turning-angle
...
- Implementation: typed design and propagation contracts, direct driver
  ...
  collection before diagnostics. The hosted repository-owned matrix passes at
  final head `27ce1029a`: stable, beta, nightly, Miri, security, solver,
  benchmark, PINN, feature, wheel, documentation, architecture, migration,
