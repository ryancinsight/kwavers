...
use kwavers::physics::mechanics::{
    KuznetsovConfig, KuznetsovWave, NonlinearWave, TimeIntegrationScheme,
};
use kwavers::*;
// use kwavers::physics::{KuznetsovWaveComponent, PhysicsPipeline, PhysicsContext};
use kwavers::medium::HomogeneousMedium;
use kwavers::source::{NullSource, Source};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!(
        "Domain size: {:.1}mm x {:.1}mm x {:.1}mm\n",
...
fn demonstrate_nonlinear(grid: &Grid, medium: &HomogeneousMedium) -> Result<(), Box<dyn Error>> {
    ...
    for step in 0..steps {
        ...
        if step % 10 == 0 {
            ...
            println!(
                "    Step {}: max pressure = {:.2e} Pa, max gradient = {:.2e} Pa/m",
                step, max_p, grad_max
            );
        }
    }
