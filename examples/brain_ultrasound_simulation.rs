...
impl BrainUltrasoundSimulation {
    ...
    /// Create heterogeneous medium from brain model
    fn create_brain_medium(brain_model: &Array3<u8>, grid: &Grid) -> KwaversResult<HeterogeneousMedium> {
        let (nx, ny, nz) = brain_model.dim();
    ...
    pub fn run_focusing_algorithm(&mut self, target_point: (usize, usize, usize)) -> KwaversResult<BrainSimulationResults> {
        ...
        println!("Created {} transducers on skull surface", transducer_positions.len());
    ...
    fn simulate_travel_times(
        ...
        let source_pos = source_pos.ok_or(KwaversError::Physics(PhysicsError::InvalidConfiguration {
    ...
    fn run_focused_simulation(
        ...
        println!("Running focused simulation with {} transducers", transducer_positions.len());
...
pub fn main() -> KwaversResult<()> {
    ...
    println!("Created brain simulation with grid: {}x{}x{}", 
             simulation.get_grid().nx, simulation.get_grid().ny, simulation.get_grid().nz);
    
    // Define target points to focus (similar to original)
    // Scale target points to fit within the grid
    let (nx, ny, nz) = (simulation.get_grid().nx, simulation.get_grid().ny, simulation.get_grid().nz);
    ...
    for (i, &target_point) in target_points.iter().enumerate() {
        println!("\n--- Focusing on target point {}: {:?} ---", i + 1, target_point);
        ...
        println!("- Focusing delays calculated: {} values", results.focusing_delays.len());
        println!("- Maximum pressure at focus: {:.2e} Pa", 
                 results.pressure_max.iter().fold(0.0f64, |acc, &x| acc.max(x)));
    ...
    println!("This implementation provides equivalent functionality to the original k-Wave version");
...
mod tests {
    ...
    fn test_pixel_to_tissue_mapping() {
        assert_eq!(BrainUltrasoundSimulation::pixel_to_tissue(0), BrainTissue::Water);
        assert_eq!(BrainUltrasoundSimulation::pixel_to_tissue(5), BrainTissue::CerebrospinalFluid);
        assert_eq!(BrainUltrasoundSimulation::pixel_to_tissue(45), BrainTissue::WhiteMatter);
        assert_eq!(BrainUltrasoundSimulation::pixel_to_tissue(150), BrainTissue::GreyMatter);
        assert_eq!(BrainUltrasoundSimulation::pixel_to_tissue(240), BrainTissue::Skull);
    }
