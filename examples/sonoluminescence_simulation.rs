...
use kwavers::{
    ...
    error::KwaversError,
    io::{generate_summary, save_light_data, save_pressure_data},
...
fn main() -> KwaversResult<()> {
    ...
    grid.iter_points().for_each(|((i, j, k), (x, y, z))| {
        ...
        for i in 1..grid.nx - 1 {
            for j in 1..grid.ny - 1 {
                for k in 1..grid.nz - 1 {
                    ...
                    new_temp[[i, j, k]] =
                        temperature[[i, j, k]] + thermal_diffusivity * laplacian * dt;
                }
            }
        }
    ...
    println!(
        "Average time per step: {:.2?}",
        total_time / time.n_steps as u32
    );
