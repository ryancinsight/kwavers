...
use kwavers::{
    ...
    source::{BowlTransducer, PointSource, Source},
    time::Time,
    Grid, KwaversResult,
};
...
fn run_mbsl_simulation(params: MBSLParameters) -> KwaversResult<()> {
    ...
    for step in 0..num_steps {
        ...
        let collective =
            kwavers::physics::bubble_dynamics::CollectiveEffects::new(params.bubble_density);
        ...
        bubble_cloud.update(&pressure, &p_collective, dt, &grid, &interactions);
        ...
        fields
            ...
            .assign(&bubble_states.temperature);
        fields
            .index_axis_mut(Axis(0), 3)
            .assign(&bubble_states.radius);
        ...
        if step % output_interval == 0 {
            ...
            println!("Progress: {:.1}%, Time: {:.3} ms", progress, t * 1e3,);
            println!(
                "  SL Events: {} (discrete collapse events)",
                stats.total_sl_events,
            );
            println!("  Total Photons: {:.2e}", stats.total_sl_photons,);
            println!(
    ...
    println!(
        "  Total SL Events: {} (discrete bubble collapses)",
        final_stats.total_sl_events
    );
    ...
    println!(
        "  Max Light Intensity: {:.2e} W/m² (continuous field)",
        final_stats.max_light_intensity
    );
    println!();
    ...
    if params.enable_full_monitoring {
        ...
        println!(
            ...
            final_stats.total_cavitation_events
        );
    ...
    if let Some(sl_map) = recorder.sonoluminescence_intensity_map() {
        ...
        println!("\nSL Intensity Map:");
        println!(
            "  Max cumulative photons at single location: {:.2e}",
            max_intensity
        );
...
fn analyze_sl_distribution(recorder: &Recorder, grid: &Grid) {
    ...
    if !recorder.sl_events.is_empty() {
        let first_event = recorder
            .sl_events
            .iter()
            .map(|e| e.time)
            .fold(f64::INFINITY, f64::min);
        let last_event = recorder
            .sl_events
            .iter()
            .map(|e| e.time)
            .fold(0.0, f64::max);
