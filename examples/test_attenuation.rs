...
fn main() -> KwaversResult<()> {
    ...
    assert!((center_val - 1.0).abs() < 0.01, "Center point should have minimal attenuation");
    ...
    let corner_distance = ((0.0 - source_position[0]).powi(2) +
                          ...
                          (0.0 - source_position[2]).powi(2)).sqrt();
    ...
    assert!((corner_val - expected_corner).abs() < 0.01, "Corner attenuation incorrect");
    ...
    assert!(classical_abs > 0.0 && classical_abs < 1.0, "Classical absorption out of range");
