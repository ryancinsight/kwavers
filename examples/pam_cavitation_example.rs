...
use kwavers::{
    ArrayGeometry, BeamformingMethod, BowlRecon, FdtdConfig, FdtdPlugin, FilterType, Grid,
    HomogeneousMedium, InterpolationMethod, KwaversResult, LineRecon, PAMConfig,
    PassiveAcousticMappingPlugin, PlaneRecon, PluginManager, ReconstructionAlgorithm,
    ReconstructionConfig, Source, Time,
};
use ndarray::Array3;
...
fn create_hemispherical_array_pam(grid: &Grid) -> KwaversResult<PassiveAcousticMappingPlugin> {
    ...
    println!(
        "Created hemispherical array with {} elements",
...
fn create_linear_array_pam(grid: &Grid) -> KwaversResult<PassiveAcousticMappingPlugin> {
    ...
    println!(
        "Created linear array with {} elements",
        config.array_geometry.element_count()
    );
...
fn create_planar_array_pam(grid: &Grid) -> KwaversResult<PassiveAcousticMappingPlugin> {
    ...
    let config = PAMConfig {
        ...
        beamforming: BeamformingMethod::RobustCapon {
    ...
    println!(
        "Created planar array with {} elements",
        config.array_geometry.element_count()
    );
...
fn create_circular_array_pam(grid: &Grid) -> KwaversResult<PassiveAcousticMappingPlugin> {
    ...
    let config = PAMConfig {
        ...
        beamforming: BeamformingMethod::MUSIC {
            signal_subspace_dim: 4,
        },
        integration_time: 1e-3,
        detect_cavitation: true,
        detect_sonoluminescence: false,
    ...
    println!(
        "Created circular array with {} elements",
        config.array_geometry.element_count()
    );
...
fn create_phased_array_pam(grid: &Grid) -> KwaversResult<PassiveAcousticMappingPlugin> {
    ...
    println!(
        "Created custom phased array with {} elements in spiral pattern",
        config.array_geometry.element_count()
    );
...
fn simulate_hifu_induced_cavitation(
    ...
    for i in 0..grid.nx {
        for j in 0..grid.ny {
            for k in 0..grid.nz {
                ...
                // Distance from focus
                let r = ((x - focus[0]).powi(2) + (y - focus[1]).powi(2) + (z - focus[2]).powi(2))
                    .sqrt();
    ...
    println!(
        "  Focus: {:?} mm",
        [focus[0] * 1000.0, focus[1] * 1000.0, focus[2] * 1000.0]
    );
