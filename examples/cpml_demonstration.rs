...
fn compare_pml_cpml(grid: &Grid) -> KwaversResult<()> {
    ...
    println!(
        "  Standard PML - Final energy: {:.3e} (reflection: {:.2}%)",
        pml_energy,
        (pml_energy / initial_energy) * 100.0
    );
    println!(
        "  C-PML - Final energy: {:.3e} (reflection: {:.2}%)",
        cpml_energy,
        (cpml_energy / initial_energy) * 100.0
    );
