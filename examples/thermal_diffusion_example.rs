...
fn main() -> KwaversResult<()> {
    ...
    let medium = HomogeneousMedium::new(
        ...
        &grid, 0.5, // absorption coefficient [Np/m]
...
fn standard_thermal_diffusion(
    ...
    for i in 0..grid.nx {
        for j in 0..grid.ny {
            for k in 0..grid.nz {
                ...
                initial_temp[[i, j, k]] = 310.15 + 10.0 * (-r2 / (2.0 * sigma2)).exp();
                // Up to 47°C at center
            }
        }
    }
    ...
    let heat_source = grid.create_field();
...
fn thermal_dose_tracking(grid: &Grid, medium: &dyn kwavers::medium::Medium) -> KwaversResult<()> {
    ...
    for step in 0..n_steps {
        ...
        if step % 60 == 59 {
            ...
            println!(
                "t = {}min: Max dose = {:.1} CEM43, Ablation volume = {} voxels",
                (step + 1) / 60,
                max_dose,
                dose_volume
            );
