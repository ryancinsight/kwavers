...
fn test_heat_diffusion() {
    ...
    println!("     - Expected max (analytical): {:.2} K", T0 * amplitude_ratio);
...
fn test_wave_dispersion() {
    ...
    for ppw in points_per_wavelength {
        ...
        let status = if phase_error < 0.1 { "✓ Good" } else { "✗ Poor" };
...
fn test_acoustic_absorption() {
    ...
    for alpha_np in test_alphas {
        ...
        println!("   {:8.1} | {:8.1} cm | {:8.4} | {:8.4} | {:.2}%",
                 alpha_np, distance * 100.0, A_expected, amplitude, error * 100.0);
    }
