...
use kwavers::{
    ...
    physics::mechanics::acoustic_wave::{
        kuznetsov::AcousticEquationMode, KuznetsovConfig, KuznetsovWave,
    },
...
fn main() -> KwaversResult<()> {
    ...
    println!(
        "All features demonstrate production-ready implementations with literature validation."
    );
...
fn demo_kzk_equation(grid: &Grid, medium: &HomogeneousMedium) -> KwaversResult<()> {
    ...
    println!(
        "  📊 Propagating KZK equation for {} time steps...",
        num_steps
    );
    ...
    println!("  📈 KZK Results:");
    println!(
        "    - Maximum on-axis pressure: {:.2e} Pa",
        max_pressure_on_axis
    );
...
fn demo_seismic_imaging(grid: &Grid) -> KwaversResult<()> {
    ...
    for receiver in 0..num_receivers {
        for t in 0..num_time_samples {
            ...
            if time > reflection_arrival && time < reflection_arrival + 0.03 {
                ...
                observed_data[[receiver, t]] +=
                    0.5 * (1.0 - 2.0 * ricker_arg.powi(2)) * (-ricker_arg.powi(2)).exp();
            }
        }
    }
    ...
    // Run RTM
    let mut rtm = ReverseTimeMigration::new(seismic_config);
    ...
    // Analyze results
    let velocity_range = velocity_model
        .iter()
...
fn demo_focus_transducers(grid: &Grid, medium: &HomogeneousMedium) -> KwaversResult<()> {
    ...
    println!(
        "    - Array configuration: {} elements at {:.1} MHz",
        num_elements,
        frequency * 1e-6
    );
    ...
    let near_field_length = element_width.powi(2) / (4.0 * (medium.sound_speed / frequency));
...
fn demo_kuznetsov_vs_kzk(grid: &Grid, medium: &HomogeneousMedium) -> KwaversResult<()> {
    ...
    let correlation = correlation_sum / ((full_energy * kzk_energy) as f64).sqrt();
    ...
    println!(
        ...
        100.0 * (full_max - kzk_max).abs() / full_max
    );
