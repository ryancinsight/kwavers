...
fn demo_acoustic_propagation() -> KwaversResult<()> {
    ...
    for step in 0..n_steps {
        ...
        update_acoustic_wave_safe(&mut pressure, &pressure_prev, &mut velocity_x, 
                                 dt, dx, c0, rho0)?;

        // Validate against analytical solution every 10 steps
        if step % 10 == 0 {
            let (step_error, step_samples) = validate_against_analytical(
                &pressure, amplitude, k, omega, t, &grid
            );
    ...
    // Results
    println!("   Performance:");
    println!("     Simulation time: {:.2} ms", simulation_time.as_secs_f64() * 1000.0);
    println!("     Points per second: {:.1e}", 
...
fn apply_sinusoidal_source(
    ...
    pressure.slice_mut(ndarray::s![0, .., ..])
        .indexed_iter_mut()
...
fn demo_gaussian_beam_propagation() -> KwaversResult<()> {
    ...
    for (i, (&num, &ana)) in beam_width_numerical.iter()
        .zip(beam_width_analytical.iter()).enumerate() {
        let error = (num - ana).abs() / ana;
        max_width_error = max_width_error.max(error);
        
        if i % 10 == 0 {
            println!("   z = {:.2}λ: w_num = {:.3}λ, w_ana = {:.3}λ, error = {:.2}%",
    ...
    println!("\n   Performance:");
    println!("     Propagation time: {:.2} ms", propagation_time.as_secs_f64() * 1000.0);
...
fn apply_paraxial_propagation(
    ...
    for i in 1..nx-1 {
        for j in 1..ny-1 {
            for k_idx in 0..nz {
                let laplacian = (pressure[[i+1, j, k_idx]] - 2.0 * pressure[[i, j, k_idx]] + pressure[[i-1, j, k_idx]]) / (dx * dx) +
                               (pressure[[i, j+1, k_idx]] - 2.0 * pressure[[i, j, k_idx]] + pressure[[i, j-1, k_idx]]) / (dx * dx);
...
fn demo_nonlinear_wave_propagation() -> KwaversResult<()> {
    ...
    for step in 0..n_steps {
        ...
        if step % 50 == 0 {
            ...
            println!("   Step {}: F1 = {:.0} kPa, F2 = {:.0} kPa, F2/F1 = {:.3}",
                     step, f1 / 1e3, f2 / 1e3, f2 / f1);
        }
    ...
    println!("\n   Performance:");
    println!("     Simulation time: {:.2} ms", simulation_time.as_secs_f64() * 1000.0);
    println!("     Shock formation distance: {:.1} mm", shock_distance * 1e3);
    println!("     Propagation distance: {:.1} mm", distance * 1e3);

    let final_f2_f1_ratio = second_harmonic_amplitude.last().unwrap() / fundamental_amplitude.last().unwrap();
...
fn update_nonlinear_wave_safe(
    ...
    for i in 1..nx-1 {
        for j in 0..ny {
            for k in 0..nz {
                // Linear term: c₀²∇²p
                let d2p_dx2 = (pressure[[i+1, j, k]] - 2.0 * pressure[[i, j, k]] + pressure[[i-1, j, k]]) / (dx * dx);
                ...
                new_pressure[[i, j, k]] = 2.0 * pressure[[i, j, k]] - pressure_prev[[i, j, k]] + dt * dt * d2p_dt2;
            }
        }
    }
...
fn demo_performance_comparison() -> KwaversResult<()> {
    ...
    for &size in &sizes {
        ...
        println!("     Safe vectorization:      {:.2e} ops/sec ({:.2} ms)", 
                 ops_per_sec_safe, time_safe.as_secs_f64() * 1000.0);
        println!("     Parallel safe version:   {:.2e} ops/sec ({:.2} ms)",
                 ops_per_sec_parallel, time_parallel.as_secs_f64() * 1000.0);
        println!("     Speedup (parallel):      {:.1}x", 
