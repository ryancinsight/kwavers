...
use kwavers::{
    constants::PI,
    physics::bubble_dynamics::{
        BubbleParameters, GasSpecies, GilmoreSolver, KellerMiksisSolver, RayleighPlessetSolver,
        SonoluminescenceAnalyzer,
    },
...
fn run_sbsl_simulation(config: SBSLConfig) -> KwaversResult<()> {
    ...
    println!(
        "  Pressure: {:.2} atm",
        config.pressure_amplitude / 101325.0
    );
    ...
    let mut cavitation_damage =
        CavitationDamage::new((n, n, n), config.wall_material.clone(), damage_params);
    ...
    let mut light_emission = SonoluminescenceEmission::new((n, n, n), emission_params);
    ...
    for step in 0..n_steps {
        ...
        let mut dp_dt = grid.create_field();
        ...
        pressure.indexed_iter_mut().for_each(|((i, j, k), p)| {
            let x = i as f64 * dx;
            let y = j as f64 * dx;
            let z = k as f64 * dx;
            *p = config.pressure_amplitude
                * (omega * time).sin()
                * (k_wave * x).sin()
                * (k_wave * y).sin()
                * (k_wave * z).sin();
        ...
        dp_dt.indexed_iter_mut().for_each(|((i, j, k), dp)| {
            *dp = -config.pressure_amplitude
                * 2.0
                * PI
                * config.frequency
...
fn analyze_results(
    ...
    println!("\nValidation:");
    println!(
        "  Compression ratio: {} (Literature: 10-15 ✓)",
        if compression > 10.0 && compression < 15.0 {
            "PASS"
        } else {
            "FAIL"
        }
    ...
    println!(
        "  Max temperature: {} (Literature: 10,000-50,000 K ✓)",
        if t_max > 10000.0 && t_max < 50000.0 {
            "PASS"
        } else {
            "FAIL"
        }
