...
use kwavers::{
    ...
    Grid, KwaversError, KwaversResult,
};
...
impl NiftiLoader {
    ...
    pub fn load_brain_model(&self) -> KwaversResult<(Array3<u16>, NiftiHeader)> {
        ...
        println!(
            "  Scale: slope={:.3}, intercept={:.3}",
            header.scl_slope, header.scl_inter
        );

        // Seek to data offset
        reader
            .seek(SeekFrom::Start(header.vox_offset as u64))
    ...
    fn read_brain_data(
        ...
        match header.datatype {
            2 => {
                ...
                let brain_data = Array3::from_shape_vec(
                    (nx, ny, nz),
                    data.into_iter().map(|x| x as u16).collect(),
                )
                .map_err(|e| {
                    KwaversError::Physics(PhysicsError::InvalidConfiguration {
                        component: "NiftiLoader".to_string(),
            ...
            4 => {
                // DT_SIGNED_SHORT (16-bit)
                let mut data = vec![0i16; total_voxels];
                let bytes = unsafe {
                    std::slice::from_raw_parts_mut(data.as_mut_ptr() as *mut u8, total_voxels * 2)
                };
                reader
                    .read_exact(bytes)
                    .map_err(|e| KwaversError::Io(format!("Failed to read data: {}", e)))?;
            ...
            512 => {
                // DT_UNSIGNED_SHORT (16-bit unsigned)
                ...
                let brain_data = Array3::from_shape_vec((nx, ny, nz), data).map_err(|e| {
                    KwaversError::Physics(PhysicsError::InvalidConfiguration {
                        component: "NiftiLoader".to_string(),
                        reason: format!("Failed to reshape data: {}", e),
                    })
            ...
            _ => Err(KwaversError::Physics(PhysicsError::InvalidConfiguration {
                component: "NiftiLoader".to_string(),
...
impl RealBrainUltrasoundSimulation {
    ...
    pub fn new(nifti_file: &str, config: RealBrainSimulationConfig) -> KwaversResult<Self> {
        println!(
            "Creating real brain ultrasound simulation from: {}",
            nifti_file
        );
        ...
        println!(
            "Original brain model loaded: {}x{}x{}",
            brain_model.dim().0,
            brain_model.dim().1,
            brain_model.dim().2
        );
        ...
        if config.add_skull {
            brain_model =
                Self::add_skull_layer(&brain_model, &nifti_header, config.skull_thickness)?;
            println!(
                "Added artificial skull layer ({:.1}mm thickness)",
        ...
        if config.undersample_rate != 1.0 {
            ...
            println!(
                "Applied undersampling: rate = {:.1}",
                config.undersample_rate
            );
        }
    ...
    fn add_skull_layer(
        ...
        println!(
            "Adding skull: {:.1}mm ({} voxels) thickness",
            thickness_mm, thickness_voxels
        );
    ...
    pub fn run_focusing_algorithm(
    ...
    ) -> KwaversResult<RealBrainSimulationResults> {
        ...
        println!(
            "Created {} transducers on skull/scalp surface",
    ...
    fn create_transducer_array(&self) -> KwaversResult<Vec<(usize, usize, usize)>> {
        ...
        for i in 0..n_elements {
            ...
            for r_step in 0..20 {
                let step_x = (center_x as i32
                    + ((pos_x as i32 - center_x as i32) * (20 - r_step as i32) / 20))
                    as usize;
                let step_y = (center_y as i32
                    + ((pos_y as i32 - center_y as i32) * (20 - r_step as i32) / 20))
                    as usize;
                let step_z = (center_z as i32
                    + ((pos_z as i32 - center_z as i32) * (20 - r_step as i32) / 20))
                    as usize;
    ...
    fn calculate_focusing_delays(
        ...
        for &(tx, ty, tz) in transducer_positions {
            let distance = self.calculate_acoustic_path_length((tx, ty, tz), *target_point)?;
            ...
            for j in 0..ny {
                for k in 0..nz {
                    ...
                    if r < focus_radius as f64 {
                        ...
                        pressure_max[[i, j, k]] =
                            2e6 * tissue_factor * (1.0 - r / focus_radius as f64);
                    } else {
...
pub fn main() -> KwaversResult<()> {
    ...
    let mut simulation =
        simulation.ok_or_else(|| KwaversError::Io("No valid NIFTI files found".to_string()))?;
    ...
    for (i, &target_point) in target_points.iter().enumerate() {
        ...
        let target_intensity =
            simulation.get_brain_model()[[target_point.0, target_point.1, target_point.2]];
        let target_tissue = AtlasTissue::from_intensity(target_intensity);
        println!(
            "Target tissue: {:?} (intensity: {})",
            target_tissue, target_intensity
        );
        ...
        println!(
            "  - Focusing delays: {} values calculated",
            results.focusing_delays.len()
        );
        println!(
            "  - Brain dimensions: {}x{}x{}",
        ...
        let avg_delay =
            results.focusing_delays.iter().sum::<f64>() / results.focusing_delays.len() as f64;
        let delay_std = (results
            .focusing_delays
            .iter()
            .map(|&d| (d - avg_delay).powi(2))
            .sum::<f64>()
            / results.focusing_delays.len() as f64)
            .sqrt();
        println!(
            "  - Focusing delays: {:.1}±{:.1} μs",
            avg_delay * 1e6,
            delay_std * 1e6
        );
    }
...
mod tests {
    ...
    fn test_tissue_intensity_mapping() {
        ...
        assert_eq!(
            AtlasTissue::from_intensity(5),
            AtlasTissue::CerebrospinalFluid
        );
        ...
        assert_eq!(
            AtlasTissue::from_intensity(125),
            AtlasTissue::DeepGreyMatter
        );
    }
