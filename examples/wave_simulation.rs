...
use kwavers::{
    init_logging, FactorySimulationConfig, FactorySourceConfig, GridConfig, KwaversResult,
    MediumConfig, MediumType, PhysicsConfig, SimulationFactory, SimulationResults, TimeConfig,
    ValidationConfig,
};
...
fn analyze_results(results: &SimulationResults) {
    ...
    println!(
        "  Maximum pressure reached: {:.2e} Pa",
    ...
    if timestep_data.len() >= 3 {
        ...
        println!(
            "    Expected travel time across domain: {:.3} μs",
            expected_travel_time * 1e6
        );
        ...
        if initial_pressure > 1e5 {
            println!(
                "    ✅ Source successfully initialized with {:.2e} Pa",
                initial_pressure
            );
        } else {
...
mod tests {
    ...
    fn test_source_config_parameters() {
        ...
        assert!(config
            .validation
            .validation_rules
            .contains(&"source_validation".to_string()));
    }
