...
use kwavers::{
    source::{PhasedArrayTransducer, PhasedArrayConfig, BeamformingMode, Source},
    signal::SineWave,
...
fn main() -> KwaversResult<()> {
    ...
    println!("   Demonstrated: Electronic focusing, beam steering, custom patterns, cross-talk modeling");
...
fn demonstrate_focus_beamforming() -> KwaversResult<()> {
    ...
    for &depth in &focus_depths {
        ...
        array.set_beamforming_mode(BeamformingMode::Focus { target: focus_target })?;
        
        let delays = array.element_delays();
        let max_delay = delays.iter().fold(0.0_f64, |acc, &x| acc.max(x.abs()));
        let delay_range = delays.iter().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap() - 
                         delays.iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap();
        
        println!("  Focus at {:.0}mm: Max delay = {:.2} rad, Range = {:.2} rad", 
                depth * 1e3, max_delay, delay_range);
...
fn demonstrate_beam_steering() -> KwaversResult<()> {
    ...
    for &angle_deg in &steering_angles {
        ...
        println!("  Steer to {:.0}°: Delay gradient = {:.3} rad/element", 
...
fn demonstrate_custom_patterns() -> KwaversResult<()> {
    ...
    for i in 0..array_config.num_elements {
        ...
        let delay = if element_pos < 0.0 {
            ...
            calculate_focus_delay(element_pos * array_config.element_spacing, (-10e-3, 0.0, 40e-3), 1500.0, 2.5e6)
        } else {
    ...
    array.set_beamforming_mode(BeamformingMode::Custom { delays: dual_focus_delays.clone() })?;
    println!("    Applied {} custom delays for dual focus", dual_focus_delays.len());
    ...
    array.set_beamforming_mode(BeamformingMode::Custom { delays: gaussian_delays.clone() })?;
    ...
    array.set_beamforming_mode(BeamformingMode::Custom { delays: sinusoidal_delays })?;
...
fn demonstrate_cross_talk_effects() -> KwaversResult<()> {
    ...
    for &coefficient in &crosstalk_coefficients {
        ...
        let source_at_focus = array.get_source_term(0.0, focus_target.0, focus_target.1, focus_target.2, &grid);
...
fn calculate_focus_delay(element_x: f64, target: (f64, f64, f64), sound_speed: f64, frequency: f64) -> f64 {
