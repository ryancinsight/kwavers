...
impl KWaveReplicationSuite {
    ...
    pub fn basic_wave_propagation(&self) -> KwaversResult<ReplicationResult> {
        ...
        println!(
            "Medium: c={} m/s, ρ={} kg/m³ (water properties)",
            sound_speed, density
        );
        ...
        println!(
            "Initial condition: Gaussian pulse, width={} pixels, max={:.1e} Pa",
        ...
        for step in 0..num_steps {
            ...
            for i in 1..(nx - 1) {
                for j in 1..(ny - 1) {
                    ...
                    new_pressure[[i, j, 0]] =
                        pressure[[i, j, 0]] - dt * density * sound_speed * sound_speed * divergence;
                }
            }
        ...
        let time_series_file = format!(
            "{}/{}_pressure_time_series.csv",
            self.output_dir, example_name
        );
        ...
        let final_field_file = format!(
            "{}/{}_final_pressure_field.csv",
            self.output_dir, example_name
        );
        ...
        println!(
            "  Domain crossing time: {:.1} µs",
            domain_crossing_time * 1e6
        );
    ...
    pub fn frequency_response_analysis(&self) -> KwaversResult<ReplicationResult> {
        ...
        println!(
            "Analyzing frequency response at {} frequencies",
            frequencies.len()
        );
        ...
        for (freq_idx, &frequency) in frequencies.iter().enumerate() {
            ...
            println!(
                "    Results: max_amp={:.1e} Pa, atten={:.3}, c_phase={:.1} m/s",
        ...
        let freq_response_file = format!(
            "{}/{}_frequency_response.json",
            self.output_dir, example_name
        );
        let freq_data = json!({
            "example_name": example_name,
        ...
        for result in &results {
            writeln!(
                ...
                result["max_amplitude_pa"],
                result["attenuation_ratio"],
                result["phase_velocity_ms"],
                result["velocity_error_percent"]
            )?;
        }
        output_files.push(csv_file);

        println!(
            "Saved frequency response data: {} files",
            output_files.len()
        );

        // Validation
        let avg_velocity_error: f64 = results
            .iter()
    ...
    pub fn focused_bowl_transducer(&self) -> KwaversResult<ReplicationResult> {
        ...
        let grid = Grid::new(nx, ny, nz, dx, dx, dx)?;
        println!(
            "Grid: {}x{}x{}, dx={:.1e} m, wavelength={:.2} mm",
            nx,
            ny,
            nz,
            dx,
        ...
        println!(
            "Transducer: f={:.1} MHz, R={:.1} mm, aperture={:.1} mm",
            frequency * 1e-6,
            radius_of_curvature * 1000.0,
            ...
            for i in 0..nx {
                for j in 0..ny {
                    if source_mask[[i, j, 0]] {
                        ...
                        let y = j as f64 * dx;
                        let dist_to_focus =
                            ((x - focal_x).powi(2) + (y - (focal_j as f64 * dx)).powi(2)).sqrt();
                        let phase_delay =
                            2.0 * std::f64::consts::PI * frequency * dist_to_focus / sound_speed;
    ...
    pub fn phased_array_beamforming(&self) -> KwaversResult<ReplicationResult> {
        ...
        println!(
            "Array: {} elements, pitch={:.1} mm, f={:.1} MHz",
            num_elements,
            element_pitch * 1000.0,
            frequency * 1e-6
        );
    ...
    pub fn time_reversal_reconstruction(&self) -> KwaversResult<ReplicationResult> {
        ...
        let max_forward = pressure_forward
            .iter()
            .fold(0.0f64, |a, &b: &f64| a.max(b.abs()));
        ...
        let max_reversed = pressure_reversed
            .iter()
            .fold(0.0f64, |a, &b: &f64| a.max(b.abs()));
        ...
        let reversed_file = format!(
            "{}/{}_reconstructed_field.csv",
            self.output_dir, example_name
        );
    ...
    pub fn photoacoustic_imaging(&self) -> KwaversResult<ReplicationResult> {
        ...
        for i in 0..nx {
            for j in 0..ny {
                let dist1_sq = ((i as i32 - vessel1_x as i32).pow(2)
                    + (j as i32 - vessel1_y as i32).pow(2)) as f64;
                let dist2_sq = ((i as i32 - vessel2_x as i32).pow(2)
                    + (j as i32 - vessel2_y as i32).pow(2)) as f64;
        ...
        let execution_time = start_time.elapsed();
        println!(
            "Photoacoustic simulation completed in {:.2?}",
            execution_time
        );
        ...
        let json_str = serde_json::to_string_pretty(&metadata).map_err(|e| {
            kwavers::error::KwaversError::InvalidInput(format!("JSON error: {}", e))
        })?;
    ...
    pub fn nonlinear_propagation(&self) -> KwaversResult<ReplicationResult> {
        ...
        println!(
            "Expect harmonic generation at {} MHz, {} MHz...",
            2.0 * f0 / 1e6,
            3.0 * f0 / 1e6
        );
    ...
    fn generate_summary_report(&self, results: &[ReplicationResult]) -> KwaversResult<()> {
        ...
        println!(
            "Total output files: {}",
            results.iter().map(|r| r.output_files.len()).sum::<usize>()
        );
...
fn main() -> KwaversResult<()> {
    ...
    for result in &results {
        ...
        println!(
            "📁 {}: {} ({:.2?})",
            result.example_name, status, result.execution_time
        );
        println!(
