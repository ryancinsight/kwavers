...
use kwavers::{
    factory::{
        GridConfig, MediumConfig, MediumType, SimulationConfig, SimulationFactory,
        SimulationResults, SourceConfig,
    },
    init_logging, KwaversResult, PhysicsConfig, TimeConfig, ValidationConfig,
};
...
fn main() -> KwaversResult<()> {
    ...
    let results = simulation
        .run_with_initial_conditions(|fields, grid| set_initial_conditions(fields, grid))?;
...
fn set_focused_acoustic_source(
    ...
    println!(
        "  Focused acoustic source at ({}, {}, {}) - {:.2e} Pa",
        cx, cy, cz, amplitude
    );
    Ok(())
}
...
fn analyze_results(results: &SimulationResults) {
    ...
    if timestep_data.len() >= 3 {
        ...
        if early_growth > 2.0 && late_growth < 1.5 {
            println!(
                "  🔬 Multi-physics coupling detected: initial amplification, later stabilization"
            );
...
fn validate_physics_principles(results: &SimulationResults) -> KwaversResult<()> {
    ...
    let pressure_energies: Vec<f64> = timestep_data
        .iter()
    ...
    println!(
        "    Expected wave travel time: {:.3} μs",
