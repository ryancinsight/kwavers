...
fn run_pstd_simulation(
    ...
    println!(
        "  Time per step: {:.2} ms",
        computation_time * 1000.0 / n_steps as f64
    );
...
fn compare_results(pstd: &SimulationResult, fdtd: &SimulationResult, _grid: &Grid) {
    ...
    let (normalized_max_diff, normalized_rms_diff) = if max_p > 1e-9 {
        // Use a small epsilon
    ...
    println!(
        "  Maximum difference: {:.2e} ({:.2}%)",
        max_diff,
        normalized_max_diff * 100.0
    );
    println!(
        "  RMS difference: {:.2e} ({:.2}%)",
        rms_diff,
        normalized_rms_diff * 100.0
    );
    ...
    println!(
        "  Speedup factor: {:.2}x",
        fdtd.computation_time / pstd.computation_time
    );
    ...
    println!(
        "  Step ratio: {:.2}x",
        fdtd.time_steps as f64 / pstd.time_steps as f64
    );
}
...
fn analyze_dispersion(
    ...
    for i in 1..grid.nx - 1 {
        ...
        if pstd_val > pstd_slice[[i - 1, y_center]]
            && pstd_val > pstd_slice[[i + 1, y_center]]
            && pstd_val > 0.1 * pstd.max_pressure
        {
            pstd_peaks.push(i);
        }
        if fdtd_val > fdtd_slice[[i - 1, y_center]]
            && fdtd_val > fdtd_slice[[i + 1, y_center]]
            && fdtd_val > 0.1 * fdtd.max_pressure
        {
            fdtd_peaks.push(i);
        }
    }
    ...
    if let (Some(pstd_wl), Some(fdtd_wl)) = (
        calc_avg_wavelength(&pstd_peaks),
        calc_avg_wavelength(&fdtd_peaks),
    ) {
        println!("Numerical dispersion analysis:");
        println!("  Theoretical wavelength: {:.3} mm", wavelength * 1000.0);
        println!(
            "  PSTD measured wavelength: {:.3} mm ({:.2}% error)",
            pstd_wl * 1000.0,
            (pstd_wl - wavelength) / wavelength * 100.0
        );
        println!(
            "  FDTD measured wavelength: {:.3} mm ({:.2}% error)",
            fdtd_wl * 1000.0,
            (fdtd_wl - wavelength) / wavelength * 100.0
        );
    } else {
...
fn estimate_fdtd_phase_error(points_per_wavelength: f64, order: usize) -> f64 {
    ...
    match order {
        ...
        6 => {
            1.0 - (45.0 * (k_dx).sin() - 9.0 * (2.0 * k_dx).sin() + (3.0 * k_dx).sin())
                / (30.0 * k_dx)
        }
