...
use kwavers::{
    grid::Grid,
    io::{save_light_data, save_pressure_data},
    medium::HomogeneousMedium,
    recorder::{config::RecorderConfig, Recorder},
    sensor::SensorConfig,
    signal::{sine_wave::SineWave, Signal},
    solver::fdtd::{FdtdConfig, FdtdSolver},
    source::{flexible::FlexibleSource, PointSource},
    time::Time,
    KwaversResult,
};
...
impl KWaveReplicationSuite {
    ...
    pub fn photoacoustic_forward_2d(&self) -> KwaversResult<ReplicationResult> {
        ...
        println!(
            "Time: dt={:.2e} s, steps={}, duration={:.1} µs",
            dt,
            num_steps,
            t_end * 1e6
        );
        ...
        println!(
            "Initial pressure: circular absorber, radius={} pixels, max={:.1e} Pa",
            radius,
            initial_pressure.iter().fold(0.0f64, |a, &b| a.max(b))
        );
        ...
        println!(
            "Sensors: {} point sensors in circular array, radius={:.1} mm",
            num_sensors,
            sensor_radius as f64 * dx * 1000.0
        );
        ...
        println!(
            "  Expected wave arrival time: {:.1} µs",
            expected_arrival_time * 1e6
        );
        println!(
            "  Propagation distance: {:.1} mm",
        ...
        println!(
            "  Estimated dispersion error: {:.2}%",
            dispersion_error * 100.0
        );
    ...
    pub fn heterogeneous_medium_2d(&self) -> KwaversResult<ReplicationResult> {
        ...
        println!(
            "Source: point source at ({:.1}, {:.1}, {:.1}) mm, f={:.1} MHz",
            source_pos[0] * 1000.0,
            source_pos[1] * 1000.0,
            source_pos[2] * 1000.0,
            source_freq * 1e-6
        ...
        let sensor_positions = vec![
            ...
            [
                (2 * layer_thickness) as f64 * dx - dx,
                (ny / 2) as f64 * dx,
                0.0,
            ], // Fat-bone interface
        ...
        // Combined RMS error
        let rms_error =
            (dispersion_error.powi(2) + interface_error.powi(2) + cfl_error.powi(2)).sqrt();

        println!("Numerical analysis:");
        println!(
            "  Points per wavelength (bone): {:.1}",
...
fn main() -> KwaversResult<()> {
    ...
    for result in &results {
        ...
        println!(
            "   Max pressure: {:.2e} Pa, RMS error: {:.3}",
