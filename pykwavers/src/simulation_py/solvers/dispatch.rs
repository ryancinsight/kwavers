    ...
    pub(crate) fn run_gpu_pstd_or_cpu_fallback(
        ...
        #[cfg(feature = "gpu")]
        {
            match super::super::gpu::run_gpu_pstd_impl(
                grid, medium, time_steps, dt, alpha_coeff_db, alpha_power,
                &grid_source, sensor, transducer_sensor, pml_size, pml_size_xyz,
                pml_inside, pml_alpha_xyz,
            ) {
                Ok((sensor_data, stats)) => {
                    let sensor_data = Self::trim_initial_recorder_sample(
                        sensor_data, time_steps, record_start_index,
                    );
        ...
        Self::run_pstd_impl(
            ...
            pml_size, pml_size_xyz, pml_inside, pml_alpha_xyz, axisymmetric,
            record_modes, record_start_index,
        )
    }
}
