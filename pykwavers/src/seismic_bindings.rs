//! Python bindings for CT-derived seismic FWI brain imaging.

use kwavers::solver::inverse::seismic::brain_helmet::{
    reconstruct_brain_slice, reconstruct_brain_volume, resample_head_slice, resample_head_volume,
    select_head_slice, AcousticSlice, AcousticVolume, BrainHelmetFwiConfig,
};
use ndarray::Array1;
...
/// Run encoded 1024-element transcranial seismic FWI from a RITK-loaded CT.
///
/// The CT must be a NIfTI file readable by `ritk-io`.  The returned dictionary
/// contains CT HU, target/initial/reconstructed sound-speed arrays, masks,
/// encoded synthetic data, residual history, and scalar metrics.
#[pyfunction]
#[pyo3(signature = (
    ct_nifti_path,
    grid_size = 72,
...
pub fn register(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(run_seismic_helmet_fwi_from_ritk_ct, m)?)?;
