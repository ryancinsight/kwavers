//! Focused Transducer Sources Module
//!
//! This module provides focused transducer geometries, including bowl transducers,
//! arc sources, and multi-element arrays.
//!
//! # Design Principles
//! - **SOLID**: Single responsibility for transducer geometry generation
//! - **DRY**: Reusable geometric primitives and calculations
//! - **Zero-Copy**: Uses iterators and in-place operations
//! - **KISS**: Clear interfaces for complex geometries
//!
//! # Literature References
//! - O'Neil (1949): "Theory of focusing radiators"
//! - Cobbold (2007): "Foundations of Biomedical Ultrasound"
//! - Szabo (2014): "Diagnostic Ultrasound Imaging"

pub mod adapter;
pub mod arc;
pub mod bowl;
pub mod cap;
pub mod constructors;
pub mod multi_bowl;
mod validation;

pub use adapter::FocusedSource;
pub use arc::{ArcConfig, ArcSource};
pub use bowl::{BowlAngularBounds, BowlConfig, BowlTransducer};
pub use cap::{SphericalCapConfig, SphericalCapElement, SphericalCapLayout};
pub use constructors::{make_annular_array, make_bowl};
pub use multi_bowl::{ApodizationType, MultiBowlArray};
