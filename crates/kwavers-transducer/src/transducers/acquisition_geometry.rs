//! Transducer acquisition-geometry abstraction (Cartesian element layout +
//! source→receiver mapping).
//!
//! Domain-level geometry shared by the inverse solvers (linear Born + PCG,
//! CBS/FWI frequency-domain) and their clinical adapters. Kept in `domain` so
//! every consumer depends on it downward rather than reaching up into `solver`.
//!
//! # Why a trait
//!
