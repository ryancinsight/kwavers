...
use aequitas::systems::si::quantities::{Length, MassDensity, Pressure};
use aequitas::systems::si::units::{KilogramPerCubicMeter, Meter, Pascal};
...
impl CmutCell {
    ...
    pub fn vacuum_resonance(&self) -> f64 {
        let (r, h, g, e, rho) = self.raw_dims();
        let _ = g;
        plate::vacuum_resonance(e, h, self.poisson, rho, r)
    }
    ...
    pub fn immersion_resonance(&self, density_fluid: f64) -> f64 {
        let (r, h, g, e, rho) = self.raw_dims();
        let _ = (g, e);
        plate::immersion_resonance(self.vacuum_resonance(), rho, h, density_fluid, r)
    }
    ...
    pub fn modal_stiffness(&self) -> f64 {
        let (r, h, g, e, rho) = self.raw_dims();
        let _ = (g, e);
        let m = plate::modal_mass(rho, h, r);
    ...
    pub fn radiation_q(&self, density_fluid: f64, sound_speed_fluid: f64) -> f64 {
        ...
        let w0 = TAU * f0;
        let m = plate::modal_mass(rho, h, r);
        let ka = w0 * r / sound_speed_fluid;
        // R_rad ≈ ρ_f c_f A (ka)²/2 (baffled small piston)
        let r_rad = density_fluid * sound_speed_fluid * PI * r * r * ka * ka / 2.0;
        if r_rad <= 0.0 {
            return f64::INFINITY;
        }
        w0 * m / r_rad
    ...
    pub fn fluid_loading_beta(&self, density_fluid: f64) -> f64 {
        let (r, h, g, e, rho) = self.raw_dims();
        let _ = (g, e);
        plate::fluid_loading_beta(density_fluid, rho, h, r)
    }
    ...
    pub fn squeeze_film_damping(&self, gas_viscosity: f64) -> f64 {
        let (r, h, g, e, rho) = self.raw_dims();
        let _ = (h, e, rho);
        3.0 * PI * gas_viscosity * r.powi(4) / (2.0 * g.powi(3))
    }
    ...
    pub fn squeeze_number(&self, gas_viscosity: f64, ambient_pressure: f64, freq: f64) -> f64 {
        ...
        12.0 * gas_viscosity * TAU * freq * r * r / (ambient_pressure * g * g)
    }
    ...
    pub fn flex_gap_derating(&self, curvature: f64) -> f64 {
        let (r, h, g, e, rho) = self.raw_dims();
        let _ = (h, e, rho);
        let sag = plate::curvature_sag(curvature, r);
        1.0 / (1.0 + sag / g)
    }
}
...
mod tests {
    ...
    fn capacitance_formula() {
        let c = ivus_cmut();
        let expected = VACUUM_PERMITTIVITY * c.area() / c.gap.in_unit::<Meter>();
    ...
    fn bias_pulldown_satisfies_force_balance_and_pullin_limit() {
        ...
        for frac in [0.3, 0.6, 0.85] {
            let v = frac * vc;
            let x = c.bias_pulldown_fraction(v).unwrap() * gap;
            let spring = k * x;
