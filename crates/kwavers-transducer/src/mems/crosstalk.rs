...
use aequitas::systems::si::{
    quantities::{Area, Frequency, Length, MassDensity, MechanicalImpedance, Velocity},
...
///
/// `area_i`, `area_j` are the element face areas, `separation` the
/// centre-to-centre distance `d > 0`, `angular_frequency` the angular
/// frequency, and `rho`/`c` the fluid density and sound speed. The arguments
/// carry SI units; radians are dimensionless, so angular frequency uses the
/// SI frequency quantity.
///
...
pub fn mutual_radiation_impedance(
    ...
    if separation_m <= 0.0 || sound_speed_m_s <= 0.0 {
        return MechanicalImpedance::from_base(Complex64::new(0.0, 0.0));
    }
    ...
    MechanicalImpedance::from_base(Complex64::from_polar(
        magnitude,
...
pub fn crosstalk_matrix(
    ...
    rho: MassDensity,
    c: Velocity,
) -> Array2<MechanicalImpedance<Complex64>> {
    let n = positions.len();
    if areas.len() != n {
        return Array2::from_elem(
            [0, 0],
            MechanicalImpedance::from_base(Complex64::new(0.0, 0.0)),
        );
    }
