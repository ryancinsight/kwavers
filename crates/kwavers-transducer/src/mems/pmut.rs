...
impl PmutCell {
    ...
    pub fn effective_youngs(&self) -> f64 {
        ...
        (self.film.youngs() * tp + SI_YOUNGS * ts) / (tp + ts)
    }
    ...
    pub fn capacitance(&self) -> f64 {
        ...
        VACUUM_PERMITTIVITY * self.film.rel_permittivity() * PI * r * r / tp
    }
    ...
    pub fn radiation_q(&self, density_fluid: f64, sound_speed_fluid: f64) -> f64 {
        ...
        let w0 = TAU * f0;
        let m = plate::modal_mass(self.effective_density(), tp + ts, r);
    ...
    pub fn fluid_loading_beta(&self, density_fluid: f64) -> f64 {
        let (r, tp, ts) = self.raw_dims();
        plate::fluid_loading_beta(density_fluid, self.effective_density(), tp + ts, r)
    }
    ...
    pub fn deflection_per_volt(&self) -> f64 {
        let (r, tp, ts) = self.raw_dims();
        const ETA: f64 = 0.3;
        ETA * self.film.e31f().abs() * r * r / (self.effective_youngs() * (tp + ts).powi(2))
    }
