...
fn all_elements_lie_on_the_curvature_arc() {
    ...
    for i in 0..geo.num_elements() {
        ...
        assert!(
            (r - R_C).abs() < 1e-12,
            "element {i} off the arc: {r} vs {R_C}"
        );
    }
}
...
fn normals_are_unit_radial_and_tangents_orthogonal() {
    ...
    for i in 0..geo.num_elements() {
        ...
        for k in 0..3 {
            assert!(
                (n[k] - radial[k]).abs() < 1e-12,
                "normal {i} not radial on axis {k}"
            );
        }
        // Tangent ⊥ normal.
        ...
        assert!(
            dot.abs() < 1e-12,
            "tangent {i} not orthogonal to normal: {dot}"
        );
    }
}
...
fn arc_pitch_round_trips_and_layout_is_symmetric() {
    ...
    assert!(
        (geo.arc_pitch() - pitch).abs() < 1e-15,
        "arc pitch should round-trip"
    );
...
fn focusing_at_curvature_center_gives_zero_relative_delays() {
    ...
    let delays = geo
        .focusing_delays(geo.curvature_center(), 1540.0)
        .expect("delays");
    for (i, &d) in delays.iter().enumerate() {
        assert!(
            d.abs() < 1e-15,
            "delay {i} should be 0 focusing at curvature center, got {d}"
        );
    }
}
...
fn focusing_delays_are_nonnegative_symmetric_and_min_zero() {
    ...
    assert!(
        delays.iter().copied().fold(f64::INFINITY, f64::min).abs() < 1e-18,
        "min delay must be 0"
    );
    for i in 0..n {
        assert!(
