...
fn test_max_prf() {
    ...
    assert!(
        (seq.max_prf().into_base() - expected).abs() / expected < 1e-10,
        "PRF_max mismatch: {:.1} Hz expected {:.1} Hz",
        seq.max_prf().into_base(),
        expected
    );
}
...
fn test_prf_exceeds_max_returns_error() {
    let seq = sequencer_40mm();
    let prf_over = Frequency::from_base(seq.max_prf().into_base() * 2.0);
...
fn test_prf_at_max_is_valid() {
    ...
    assert!(
        (stored.into_base() - prf_max.into_base()).abs() < 1e-6,
        "prf_override = {:.6} (expected {:.6})",
        stored.into_base(),
        prf_max.into_base()
    );
}
...
fn test_sequential_schedule_timing() {
    let seq = sequencer_40mm();
    let angles: Vec<Angle> = (-2..=2)
        .map(|i| Angle::from_base(i as f64 * PI / 180.0))
        .collect(); // 5 angles
    let sched = seq.sequential_schedule(&angles);

    assert_eq!(sched.n_events(), 5);
    let pri = 1.0 / seq.max_prf().into_base();
    for (k, ev) in sched.events.iter().enumerate() {
        let expected_t = k as f64 * pri;
        assert!(
            (ev.t_start.into_base() - expected_t).abs() < 1e-15,
            "Event {k} t_start mismatch"
        );
    }
    assert!((sched.frame_rate.into_base() - seq.max_prf().into_base() / 5.0).abs() < 1e-6);
}
...
fn test_flash_schedule() {
    ...
    assert!(sched.events[0].t_start.into_base().abs() < 1e-15);
    assert!(sched.events[0].tilt_angle.into_base().abs() < 1e-15);
    assert!((sched.frame_rate.into_base() - seq.max_prf().into_base()).abs() < 1e-6);
}
...
fn test_interleaved_schedule_all_angles_present() {
    ...
    let mut fired: Vec<i64> = sched
        .events
        .iter()
        .map(|ev| (ev.tilt_angle.into_base() * 180.0 / PI).round() as i64)
        .collect();
    fired.sort();
...
fn test_interleaved_max_separation_first_pair() {
    ...
    let a0 = sched.events[0].tilt_angle.into_base();
    let a1 = sched.events[1].tilt_angle.into_base();
    let sep = (a1 - a0).abs();
...
fn test_sta_schedule_element_ordering() {
    ...
    for (k, ev) in sched.events.iter().enumerate() {
        ...
        assert!(ev.tilt_angle.into_base().abs() < 1e-15);
    }
}
...
fn test_frame_rate_formula() {
    let seq = sequencer_40mm();
    let n = 11usize;
    let expected = seq.max_prf().into_base() / n as f64;
    assert!(
        (seq.frame_rate(n).into_base() - expected).abs() / expected < 1e-10,
        "Frame rate {:.1} Hz ≠ {:.1} Hz",
        seq.frame_rate(n).into_base(),
        expected
    );
}
