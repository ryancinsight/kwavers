...
use aequitas::systems::si::{
    quantities::{Angle, Length, Velocity},
    units::{Hertz, Meter, MeterPerSecond, Radian},
};
...
fn test_frame_rate_formula() {
    ...
    assert!(
        (seq.frame_rate(n).in_unit::<Hertz>() - expected).abs() / expected < 1e-10,
        "Frame rate {:.1} Hz ≠ {:.1} Hz",
        seq.frame_rate(n).in_unit::<Hertz>(),
        expected
    );
}
