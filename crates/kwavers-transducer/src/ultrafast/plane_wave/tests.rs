...
fn test_plane_wave_transmission_delays() {
    let positions = [-0.002, -0.001, 0.0, 0.001, 0.002]
        .into_iter()
    ...
    let delays_0deg = pw.transmission_delays(Angle::from_base(0.0)).unwrap();
    ...
    let delays_5deg = pw.transmission_delays(Angle::from_base(theta)).unwrap();
...
fn test_apodization_weights() {
    let positions: Vec<Length> = (0..128)
        .map(|i| Length::from_base((i as f64 - 63.5) * 0.00011))
        .collect();
    let config = UltrafastPlaneWaveConfig {
        element_positions: positions,
        f_number: Some(1.5),
    ...
    let weights = pw
        .apodization_weights(Length::from_base(0.0), Length::from_base(0.02))
        .unwrap();
...
fn test_functional_ultrasound_config() {
    ...
    let frame_rate = pw.compounded_frame_rate(Frequency::from_base(5500.0));
...
fn test_delay_surface() {
    ...
    let surface = pw
        .delay_surface(&x_pixels, &y_pixels, Angle::from_base(0.0))
        .unwrap();
