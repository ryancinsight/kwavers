...
impl DomainEMSource for PointEMSource {
    ...
    fn electric_field_at_time(&self, time: f64, position: &[f64]) -> [f64; 3] {
        let distance = (position[2] - self.position[2])
            .mul_add(
                position[2] - self.position[2],
                (position[1] - self.position[1]).mul_add(
                    position[1] - self.position[1],
                    (position[0] - self.position[0]).powi(2),
                ),
            )
            .sqrt();
...
impl DomainEMSource for PlaneWaveEMSource {
    ...
    fn electric_field_at_time(&self, time: f64, position: &[f64]) -> [f64; 3] {
        ...
        let k_dot_r = self.direction[2].mul_add(
            position[2],
    ...
    fn directivity(&self, direction: &[f64]) -> f64 {
        ...
        let cos_theta = self.direction[2].mul_add(
            direction[2],
            self.direction[0].mul_add(direction[0], self.direction[1] * direction[1]),
        );
