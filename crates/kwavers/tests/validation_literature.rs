...
fn test_plane_wave_analytical_solution() {
    ...
    let grid = Grid::new(128, 8, 8, c0 / (freq * 10.0), 0.001, 0.001).expect("Grid creation");
    ...
    assert!(
        ...
        "Grid resolution insufficient: {:.1} ppw (need ≥10)",
    ...
    assert!(
        (impedance - expected_impedance).abs() / expected_impedance < 0.01,
        "Impedance mismatch"
    );
}
...
fn test_cfl_stability_criterion() {
    ...
    assert!(
        cfl < fdtd_limit,
        "CFL number {:.3} exceeds FDTD stability limit {:.3}",
        cfl,
        fdtd_limit
    );
...
fn test_power_law_absorption_scaling() {
    ...
    for &freq in &frequencies {
        ...
        assert!(
            alpha_f > 0.0 && alpha_f.is_finite(),
            "Absorption coefficient must be positive and finite at f={:.1} MHz",
            freq / 1e6
        );
    ...
    for power_test in [0.0, 0.5, 1.0, 1.5, 2.0] {
        ...
        assert!(
            ...
            "Power-law absorption must be finite and non-negative for y={}",
            power_test
        );
    }
}
...
fn test_acoustic_impedance_reflection_transmission() {
    ...
    assert!(
        r_percent < 5.0,
        "Water-muscle reflection should be <5%, got {:.2}%",
        r_percent
    );
    ...
    assert!(
        r_air > 0.99,
        "Water-air interface should have >99% reflection, got {:.4}",
        r_air
    );
}
...
fn test_wavelength_frequency_relationship() {
    ...
    for &freq in &frequencies {
        ...
        assert!(
            wavelength > 0.0 && wavelength < 1.0, // < 1m for ultrasound
        ...
        assert!(
            (wavelength_double - wavelength / 2.0).abs() < 1e-10,
...
fn test_nyquist_sampling_criterion() {
    ...
    assert!(
        ppw_inadequate < 2.0,
        "Inadequate sampling should be detectable: {:.2} ppw < 2",
        ppw_inadequate
    );
}
...
fn test_energy_conservation_principle() {
    ...
    assert!(
        (kinetic_energy - potential_energy).abs() / kinetic_energy < 1e-10,
...
fn test_nonlinearity_parameter_biological_range() {
    ...
    for &ba in &biological_values {
        assert!(
            (3.0..=12.0).contains(&ba),
            "B/A = {:.1} outside biological tissue range [3, 12]",
            ba
        );
