...
fn test_fft_peak_scaling() {
    ...
    for i in 0..nx {
        let kx = if i <= nx / 2 {
            i as f64 * (2.0 * std::f64::consts::PI / (nx as f64 * dx))
        } else {
            (i as f64 - nx as f64) * (2.0 * std::f64::consts::PI / (nx as f64 * dx))
        };
        for j in 0..ny {
            let ky = if j <= ny / 2 {
                j as f64 * (2.0 * std::f64::consts::PI / (ny as f64 * dx))
            } else {
                (j as f64 - ny as f64) * (2.0 * std::f64::consts::PI / (ny as f64 * dx))
            };
            for k in 0..nz {
                let kz = if k <= nz / 2 {
                    k as f64 * (2.0 * std::f64::consts::PI / (nz as f64 * dx))
                } else {
                    (k as f64 - nz as f64) * (2.0 * std::f64::consts::PI / (nz as f64 * dx))
                };
