...
def imports(uses_quantity: bool) -> str:
    ...
    # `TypeAlias` is retained deliberately. It is 3.10+ while the package
    # declares `requires-python = ">=3.8"`, but a stub is never executed, so no
    # 3.8 *runtime* reads it, and current mypy refuses to target anything below
    # 3.10 at all -- there is no reachable checker that both supports 3.8 and
    # trips on this. The implicit alias form was tried and is worse: mypy reads
    # `Float64Array = NDArray[np.float64]` as a variable rather than an alias
    # and reports 317 `Variable ... is not valid as a type` errors.
    names = ["ClassVar", "Dict", "List", "Optional"]
    if uses_quantity:
        names.append("Protocol")
    names += ["Sequence", "Tuple", "TypeAlias", "Union"]
    return (
