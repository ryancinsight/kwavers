...
# A dimensioned parameter accepts either a float in canonical SI base units
# -- the historical contract, unchanged -- or any object carrying the
# Aequitas quantity protocol. The stub states that structurally instead of
# importing `pyaequitas`, so a consumer needs no dependency on it to
# type-check, and the protocol written here is exactly what the extractor
# checks at runtime (see `src/quantity_args`).
QUANTITY_TYPE = "Quantity"
QUANTITY_BLOCK = """class AequitasQuantity(Protocol):
    @property
...
class TypeMapper:
    ...
    def __init__(self, class_names: set[str], alias_map: dict[str, str] | None = None,
                 ...
                 quantity_aliases: set[str] | None = None):
        self.class_names = class_names
        # `pub type PyLength = Dimensioned<...>` names, read from the
        # binding source rather than listed here, so a new dimensioned
        # alias needs no edit to this generator.
        self.quantity_aliases = set(quantity_aliases or ())
    ...
    def map(self, ty: str) -> str:
        ...
        # Dimensioned parameters: a base-unit float, or a quantity whose
        # dimension the extractor checks.
        if ty in self.quantity_aliases or re.match(r'^Dimensioned<.+>$', ty):
...
def parse_rust(root: Path) -> dict:
    ...
    return {
        ...
        "quantity_aliases": quantity_aliases(root),
...
def quantity_aliases(root: Path) -> set[str]:
    """Names of the dimensioned parameter aliases, read from the source.

    `src/quantity_args` defines `pub type PyLength = Dimensioned<...>` per
    dimension. Reading them here keeps the two in step: a new alias reaches
    the stub by existing, not by also being listed in this generator.
    """
    module = root / "src" / "quantity_args" / "mod.rs"
...
def imports(uses_quantity: bool) -> str:
    """Stub imports. `Protocol` appears only when a dimensioned parameter
    does, so a build without one emits the header it always did."""
    # `TypeAlias` comes from `typing_extensions`, not `typing`. It is 3.10+ in
    # the standard library while this package declares
    # `requires-python = ">=3.8"`, and a checker targeting 3.8 resolves names
    # against that version's `typing` -- pyright does exactly that, and it is
    # what Pylance runs by default.
    #
    # The annotation cannot simply be dropped either: mypy then reads
    # `Float64Array = NDArray[np.float64]` as a variable rather than an alias
    # and reports 317 `not valid as a type` errors. `typing_extensions` keeps
    # the annotation and works at the declared floor. It is stub-only, and
    # every checker either ships its stub or depends on the package.
    names = ["ClassVar", "Dict", "List", "Optional"]
    if uses_quantity:
        names.append("Protocol")
    names += ["Sequence", "Tuple", "Union"]
    return (
        "from typing import " + ", ".join(names) + "\n"
...
def alias_block(used_aliases: set[str]) -> str:
    ...
    for name in sorted(used_aliases):
        lines.append(f"{name}: TypeAlias = {ARRAY_ALIAS_DEFS[name]}")
    if lines:
        return "\n".join(lines) + "\n\n"
    return ""
...
def build_signatures(model: dict) -> dict:
    ...
    for cls, syms in sorted(by_class.items()):
        ...
        for sym in syms:
            if sym.kind == "new":
                # constructor: __init__ (PEP 484: __init__ must return None)
                sig = emit_fn(sym, True, cls)
                if sig:
                    sig = re.sub(r"\)\s*->\s*[^:]+: \.\.\.$", ") -> None: ...", sig)
                ...
                if sig:
                    if sym.name == "__eq__":
                        # object.__eq__ takes `other: object`; overriding with
                        # a narrower type violates Liskov (mypy override).
                        sig = re.sub(r"other: \"[^\"]+\"", "other: object", sig)
...
def generate_stub(model: dict) -> tuple[str, list[dict]]:
    ...
    uses_quantity = any(
        re.search(r'\b' + QUANTITY_TYPE + r'\b', sig)
        for sig in emitted.values()
    )
    lines = [HEADER, imports(uses_quantity)]
    if uses_quantity:
        lines.append(QUANTITY_BLOCK + "\n")
...
# Pure-Python parity helpers defined in kwave_parity (not the extension).
PARITY_HELPERS = {
    "angular_spectrum_cw",
...
def generate_facade_stub(exported: list[str]) -> str:
    ...
    The facade is a thin re-export of the extension surface plus a fixed set
    of pure-Python parity helpers. Extension names are imported from
    ``._pykwavers``; parity helpers from ``.kwave_parity``; the module
    metadata attributes are declared as strings.
    """
    ext_names = sorted(n for n in exported if n not in PARITY_HELPERS and n not in ("__author__", "__version__"))
    parity_names = sorted(n for n in exported if n in PARITY_HELPERS)
    lines = [
        HEADER.rstrip(),
    ...
    lines.append(")")
    if parity_names:
        lines.extend(["", "from .kwave_parity import ("])
