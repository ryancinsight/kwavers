...
def imports(uses_quantity: bool) -> str:
    ...
    # `TypeAlias` is imported from `typing_extensions` rather than `typing`.
    # At the 3.10 floor `typing` carries it too, but this import is stub-only
    # and keeps resolving under a checker that targets an older version's
    # `typing`; swapping it would rewrite every committed stub for no runtime
    # gain.
    #
    ...
    # and reports 317 `not valid as a type` errors. It is stub-only, and every
    # checker either ships it or depends on the package.
