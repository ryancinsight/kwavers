...
pub fn register_book(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(transducer::multi_focus_field_magnitude_2d, m)?)?;
    m.add_function(wrap_pyfunction!(transducer::linear_array_aperiodic_positions, m)?)?;
    m.add_function(wrap_pyfunction!(transducer::steered_beam_pattern_1d, m)?)?;
    m.add_function(wrap_pyfunction!(transducer::steering_grating_lobe_ratio_1d, m)?)?;
