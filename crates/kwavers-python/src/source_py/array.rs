...
use super::Source;
use crate::array_utils::{leto3_to_pyarray3, pyarray1_to_leto1, pyarray2_to_leto2};
...
impl Source {
    ...
    #[getter]
    fn initial_pressure<'py>(&self, py: Python<'py>) -> PyResult<Option<Py<PyArray3<f64>>>> {
        self.initial_pressure
            .as_ref()
            .map(|arr| leto3_to_pyarray3(py, arr.clone()))
            .transpose()
    }
    ...
    fn from_kwave_array(
        ...
        let signal_length = signal_arr.shape()[0];
        let amplitude = signal_arr.iter().fold(0.0_f64, |acc, v| acc.max(v.abs()));
        let signal_matrix = leto::Array2::from_shape_vec([1, signal_length], signal_arr.into_vec())
            .map_err(|error| {
                PyValueError::new_err(format!("failed to construct source signal: {error}"))
            })?;
    ...
    fn from_kwave_array_per_element(
        ...
        let signal_matrix = pyarray2_to_leto2(&signals)?;
