...
METRICS_PATH = DEFAULT_OUTPUT_DIR / "pr_2D_TR_line_sensor_metrics.txt"
...
def _metrics_pass(metrics: dict[str, float], thresholds: dict[str, float]) -> bool:
    passed = float(metrics["pearson_r"]) >= thresholds["pearson_r"]
    if "rms_ratio_min" in thresholds:
        passed = passed and thresholds["rms_ratio_min"] <= float(metrics["rms_ratio"])
    if "rms_ratio_max" in thresholds:
        passed = passed and float(metrics["rms_ratio"]) <= thresholds["rms_ratio_max"]
    if "psnr_db" in thresholds:
        passed = passed and float(metrics["psnr_db"]) >= thresholds["psnr_db"]
    return passed
...
def main() -> int:
    ...
    tr_checks = {
        "time_reversal": _metrics_pass(result["summary"], thresholds["time_reversal"]),
    }
    fft_checks = {
        "fft": _metrics_pass(result["fft_summary"], thresholds["fft"]),
    }
    reference_checks = {
        name: _metrics_pass(metrics, thresholds["reference"])
        for name, metrics in result["reference_metrics"].items()
    }
    overall_status = (
        "PASS"
        if all(tr_checks.values()) and all(fft_checks.values()) and all(reference_checks.values())
        else "FAIL"
    )
    ...
    print("=" * 80)
    tr_thr = thresholds["time_reversal"]
    fft_thr = thresholds["fft"]
    print(f"Time reversal Pearson r: {tr_r:.6f}  (target >= {tr_thr['pearson_r']})")
    print(
        f"Time reversal RMS ratio: {tr_rms:.6f}  "
        f"(target [{tr_thr['rms_ratio_min']}, {tr_thr['rms_ratio_max']}])"
    )
    print(f"Time reversal PSNR [dB]: {tr_psnr:.6f}  (target >= {tr_thr['psnr_db']})")
    print(f"FFT Pearson r:           {fft_r:.6f}  (target >= {fft_thr['pearson_r']})")
    print(
        f"FFT RMS ratio:           {fft_rms:.6f}  "
        f"(target [{fft_thr['rms_ratio_min']}, {fft_thr['rms_ratio_max']}])"
    )
    print(f"FFT PSNR [dB]:           {fft_psnr:.6f}  (target >= {fft_thr['psnr_db']})")
    ...
    with METRICS_PATH.open("w") as fh:
        ...
        fh.write(f"  pearson_r = {tr_r:.6f}  (target >= {tr_thr['pearson_r']})\n")
        fh.write(
            f"  rms_ratio = {tr_rms:.6f}  "
            f"(target [{tr_thr['rms_ratio_min']}, {tr_thr['rms_ratio_max']}])\n"
        )
        fh.write(f"  psnr_db   = {tr_psnr:.6f}  (target >= {tr_thr['psnr_db']} dB)\n\n")
        fh.write("FFT line reconstruction (kwave vs pykwavers):\n")
        fh.write(f"  pearson_r = {fft_r:.6f}  (target >= {fft_thr['pearson_r']})\n")
        fh.write(
            f"  rms_ratio = {fft_rms:.6f}  "
            f"(target [{fft_thr['rms_ratio_min']}, {fft_thr['rms_ratio_max']}])\n"
        )
        fh.write(f"  psnr_db   = {fft_psnr:.6f}  (target >= {fft_thr['psnr_db']} dB)\n\n")
        fh.write("Reconstruction vs ground-truth p0:\n")
        fh.write(f"  kwave     TR pearson_r = {kw_ref['pearson_r']:.6f}\n")
        fh.write(f"  kwave     TR rms_ratio = {kw_ref['rms_ratio']:.6f}\n")
        fh.write(f"  kwave     TR psnr_db   = {kw_ref['psnr_db']:.6f}\n")
        fh.write(f"  pykwavers TR pearson_r = {py_ref['pearson_r']:.6f}\n")
        fh.write(f"  pykwavers TR rms_ratio = {py_ref['rms_ratio']:.6f}\n")
        fh.write(f"  pykwavers TR psnr_db   = {py_ref['psnr_db']:.6f}\n")
        fh.write(
            f"  kwave     FFT pearson_r = "
            f"{result['reference_metrics']['kwave_fft']['pearson_r']:.6f}\n"
        )
        fh.write(
            f"  pykwavers FFT pearson_r = "
            f"{result['reference_metrics']['pykwavers_fft']['pearson_r']:.6f}\n\n"
        )
