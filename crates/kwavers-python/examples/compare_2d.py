...
def main():
    ...
    source_p_sq = input_signal.reshape(1, -1) # Reshape for k-wave-python
    
    source_mask = np.zeros((Nx, Ny), dtype=float)
    source_mask[Nx//2, Ny//2] = 1

    source = kSource()
    source.p_mask = source_mask
    source.p = source_p_sq
    source.p_mode = 'additive-no-correction'
    
    # 3 sensors at different distances along x axis
    sensor_mask = np.zeros((kgrid.Nx, kgrid.Ny))
    sensor_mask[kgrid.Nx//2 + 4, kgrid.Ny//2] = 1   # Near
    sensor_mask[kgrid.Nx//2 + 10, kgrid.Ny//2] = 1  # Mid
    sensor_mask[kgrid.Nx//2 + 18, kgrid.Ny//2] = 1  # Far
    ...
    py_grid = kw.Grid(Nx, Ny, 1, dx, dy, 1e-3)
    py_medium = kw.Medium.homogeneous(sound_speed, density)
    
    py_source_mask = np.zeros((Nx, Ny, 1), dtype=np.float64)
    py_source_mask[Nx//2, Ny//2, 0] = 1.0
    py_source = kw.Source.from_mask(py_source_mask, input_signal.flatten(), tone_burst_freq, mode="additive_no_correction")
    
    py_sensor_mask = np.zeros((Nx, Ny, 1), dtype=bool)
    py_sensor_mask[Nx//2 + 4, Ny//2, 0] = 1
    py_sensor_mask[Nx//2 + 10, Ny//2, 0] = 1
    py_sensor_mask[Nx//2 + 18, Ny//2, 0] = 1
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig("point_source_comparison_2d.png", dpi=150)
        print("\nSaved point_source_comparison_2d.png")
    except ImportError:
        pass
