"""Parity regression for the vendored `at_focused_bowl_AS` example."""
...
def _run_example() -> Path:
    ...
    script = root / "examples" / "at_focused_bowl_AS_compare.py"
    metrics_path = root / "examples" / "output" / "at_focused_bowl_AS_metrics.txt"
    env = os.environ.copy()
...
def _load_example_module():
    root = Path(__file__).resolve().parents[1]
    module_path = root / "examples" / "at_focused_bowl_AS_compare.py"
    examples_dir = str(root / "examples")
    if examples_dir not in sys.path:
        sys.path.insert(0, examples_dir)
    spec = importlib.util.spec_from_file_location(
        "at_focused_bowl_AS_compare",
        module_path,
    )
...
def _assert_report_contract(module, text: str):
    ...
    assert (
        abs(kwave_vs_analytical - pkwav_vs_analytical)
...
def test_current_at_focused_bowl_as_artifacts_match_thresholds():
...
class TestKWaveExampleParityAtFocusedBowlAS:
    def test_at_focused_bowl_as_metrics(self):
        module = _load_example_module()
