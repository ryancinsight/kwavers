...
def test_scattering_increment_diagnostics_identify_exact_increment_model():
    ...
    assert diagnostics["best_model_scaled_increment_model"] == "exact_increment"
    ...
    assert by_model["exact_increment"]["model_scaled_observed_increment_l2_norm"] > 0.0
    assert by_model["exact_increment"]["model_scaled_increment_residual_l2_norm"] <= 1.0e-14
    assert by_model["exact_increment"]["model_scaled_normalized_increment_residual"] <= 1.0e-14
    assert abs(by_model["exact_increment"]["model_scaled_increment_energy_ratio"] - 1.0) <= 1.0e-12
...
def test_scattering_increment_diagnostics_report_model_scale_drift():
    ...
    assert abs(row["model_scaled_increment_energy_ratio"] - np.sqrt(2.0)) <= 1.0e-14
