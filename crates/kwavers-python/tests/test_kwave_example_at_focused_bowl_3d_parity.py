...
class TestKWaveExampleParityAtFocusedBowl3D:
    ...
    def test_at_focused_bowl_3d_metrics(self):
        ...
        assert summary["pearson_r"] > 0.999
        assert abs(summary["rms_ratio"] - 1.0) < 1e-2
        assert summary["psnr_db"] > 40.0

        assert source_metrics["pearson_r"] > 0.995
        assert abs(source_metrics["rms_ratio"] - 1.0) < 1e-2
        assert abs(source_metrics["peak_ratio"] - 1.0) < 1e-2
        assert source_metrics["rmse"] < 1e-2
