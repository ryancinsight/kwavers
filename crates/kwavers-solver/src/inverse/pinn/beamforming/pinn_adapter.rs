//! Concrete implementation of PinnBeamformingProvider for Coeus-backed PINNs.
//!
//! This module provides the bridge between the solver layer's PINN implementations
...
//!     ↑ implements
//! PinnBeamformingAdapter (this module)
//!     ↓ wraps
//! PINN implementations (Solver Layer 2)
//! ```
...
#[cfg(test)]
use crate::inverse::pinn::ml::LossWeights;
use crate::inverse::pinn::ml::{PinnConfig, PinnTrainer, PinnWave1D};
...
impl<B: coeus_ops::BackendOps<f32> + coeus_ops::CpuBackend + Default> std::fmt::Debug
    for PinnBeamformingAdapter<B>
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PinnBeamformingAdapter")
            .field("config", &self.config)
            .field("is_trained", &self.is_trained)
            .field("metadata", &self.metadata)
            .field("model", &"<Arc<Mutex<Option<PinnWave1D>>>>")
            .finish()
    }
}

pub struct PinnBeamformingAdapter<B: coeus_ops::BackendOps<f32> + coeus_ops::CpuBackend + Default> {
    /// Underlying PINN model
    model: Arc<Mutex<Option<PinnWave1D<B>>>>,
    /// Model configuration
    config: PinnConfig,
    /// Is model trained?
    is_trained: bool,
...
impl<B: coeus_ops::BackendOps<f32> + coeus_ops::CpuBackend + Default> PinnBeamformingAdapter<B>
where
    B::DeviceBuffer<f32>:
        coeus_core::CpuAddressableStorage<f32> + coeus_core::CpuAddressableStorageMut<f32>,
{
    ...
    fn estimate_parameters(config: &PinnConfig) -> usize {
    ...
    fn ensure_model_initialized(&self) -> KwaversResult<()> {
        ...
        if model_lock.is_none() {
            let model = PinnWave1D::new(self.config.clone())?;
...
impl<B> PinnBeamformingProvider for PinnBeamformingAdapter<B>
where
        ...
        coeus_core::CpuAddressableStorage<f32> + coeus_core::CpuAddressableStorageMut<f32>,
{
    ...
    fn train(
        ...
        let mut trainer = PinnTrainer::<B>::new(self.config.clone())?;
        let training_metrics = trainer.train(&x_data, &t_data, &u_data, wave_speed, epochs)?;
        ...
        Ok(BeamformingTrainingMetrics {
            total_loss: training_metrics.total_loss.last().copied().unwrap_or(0.0),
            physics_loss: training_metrics.pde_loss.last().copied().unwrap_or(0.0),
            data_loss: training_metrics.data_loss.last().copied().unwrap_or(0.0),
            iterations: training_metrics.epochs_completed,
            training_time: training_metrics.training_time_secs,
        })
    }
...
pub fn create_pinn_beamforming_provider() -> KwaversResult<Box<dyn PinnBeamformingProvider>> {
    ...
    let adapter = PinnBeamformingAdapter::<Backend>::new(config)?;
    Ok(Box::new(adapter))
}
...
mod tests {
    ...
    fn test_adapter_creation() {
        ...
        let result = PinnBeamformingAdapter::<Backend>::new(config);
    ...
    fn test_parameter_estimation() {
        ...
        let params =
            PinnBeamformingAdapter::<coeus_core::MoiraiBackend>::estimate_parameters(&config);
    ...
    fn test_model_info() {
        ...
        let adapter = PinnBeamformingAdapter::<Backend>::new(config).unwrap();
