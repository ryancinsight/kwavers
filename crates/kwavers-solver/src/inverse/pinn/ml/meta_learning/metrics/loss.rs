...
impl MetaLoss {
    ...
    pub fn new(task_losses: Vec<f64>, physics_loss: f64) -> Self {
        let total_loss = if !task_losses.is_empty() {
            task_losses.iter().sum::<f64>() / (task_losses.shape()[0] * task_losses.shape()[1] * task_losses.shape()[2]) as f64
        } else {
            0.0
        };
    ...
    pub fn task_loss_std_dev(&self) -> f64 {
        ...
        let variance = self
            ...
            .map(|loss| (loss - mean).powi(2))
            .sum::<f64>()
            / (self.task_losses.shape()[0] * self.task_losses.shape()[1] * self.task_losses.shape()[2]) as f64;
