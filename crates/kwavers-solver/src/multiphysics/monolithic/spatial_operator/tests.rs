...
fn test_laplacian_unit_vs_nonunit_spacing() {
    ...
    let lap_1mm = laplacian_3d(&field.view(), (5, 5, 5), 1e-3, 1e-3, 1e-3);
...
fn test_laplacian_quadratic_field_exact() {
    ...
    let lap = laplacian_3d(&field.view(), (n, n, n), dx, dx, dx);
...
fn test_laplacian_view_matches_owned_input() {
    ...
    let owned = block.to_contiguous();

    let lap_view = laplacian_3d(&block, (n, n, n), dx, dx, dx);
    let lap_owned = laplacian_3d(&owned.view(), (n, n, n), dx, dx, dx);
...
fn test_laplacian_zero_field() {
    ...
    for &dx in &[1.0, 1e-3, 1e-6] {
        let lap = laplacian_3d(&field.view(), (6, 6, 6), dx, dx, dx);
        assert!(
