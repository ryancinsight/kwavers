...
fn test_gaussian_beam_paraxial_check() {
    assert!(GaussianBeam::new(1e5, MHZ_TO_HZ, SOUND_SPEED_WATER_SIM, 1e-3, 0.0).is_err());
    assert!(GaussianBeam::new(1e5, MHZ_TO_HZ, SOUND_SPEED_WATER_SIM, 5e-3, 0.0).is_ok());
}

#[test]
fn test_gaussian_beam_rayleigh_range() {
    let beam = GaussianBeam::new(1e5, 2.0 * MHZ_TO_HZ, SOUND_SPEED_WATER_SIM, 5e-3, 0.0).unwrap();
