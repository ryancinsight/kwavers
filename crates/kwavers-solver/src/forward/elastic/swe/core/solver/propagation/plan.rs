...
impl PropagationPlan {
    pub(super) fn history_layout<T>(self) -> KwaversResult<(usize, Layout)> {
        derive_history_layout::<T>(self.steps, self.save_every)
    }
...
fn derive_history_layout<T>(steps: usize, save_every: usize) -> KwaversResult<(usize, Layout)> {
...
mod tests {
    ...
    fn history_layout_covers_complete_snapshot_schedule() {
        for (steps, save_every, expected_capacity) in [
            ...
            let (capacity, full_layout) =
                derive_history_layout::<ElasticWaveField>(steps, save_every)
                    .expect("addressable full-field schedule");
            let (_, displacement_layout) =
                derive_history_layout::<ElasticDisplacementSnapshot>(steps, save_every)
                    .expect("addressable displacement schedule");
            assert_eq!(capacity, expected_capacity);
            assert_eq!(full_layout.size(), capacity * size_of::<ElasticWaveField>());
            assert_eq!(
                displacement_layout.size(),
                capacity * size_of::<ElasticDisplacementSnapshot>()
            );
        }
    }
    ...
    fn history_layout_rejects_count_and_address_space_overflow() {
        let count_error = derive_history_layout::<ElasticWaveField>(usize::MAX, 1)
        ...
        let layout_error =
            derive_history_layout::<ElasticWaveField>(first_unaddressable_capacity - 1, 1)
