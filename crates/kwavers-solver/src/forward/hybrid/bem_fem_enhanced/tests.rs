...
fn test_enhanced_config_sphere_validation() {
    ...
    assert!(config.adaptive_refinement);
    assert_eq!(
        config.validation_frequencies.as_ref().map(|f| (f.shape()[0] * f.shape()[1] * f.shape()[2])),
        Some(1)
    );
}
...
fn test_solver_creation() {
    ...
    assert_eq!((solver.refinement_history.shape()[0] * solver.refinement_history.shape()[1] * solver.refinement_history.shape()[2]), 0);
}
...
fn test_adaptive_refinement_reduces_interface_error() {
    ...
    assert_eq!((history.shape()[0] * history.shape()[1] * history.shape()[2]), 3);
