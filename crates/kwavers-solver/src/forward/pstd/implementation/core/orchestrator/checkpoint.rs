...
fn clone_field(input: &LetoArray3<f64>) -> leto::Array3<f64> {
    input.to_contiguous()
}

fn assign_field(dst: &mut LetoArray3<f64>, src: &leto::Array3<f64>) {
    assert_eq!(
        dst.shape(),
...
impl PSTDSolver {
    ...
    pub fn run_to_checkpoint(
        ...
        use crate::forward::pstd::checkpoint::PSTDCheckpoint;
    ...
    pub fn run_from_checkpoint_loaded(
        &mut self,
        ckpt: crate::forward::pstd::checkpoint::PSTDCheckpoint,
        path: &std::path::Path,
        remaining_steps: usize,
    ) -> KwaversResult<Option<Array2<f64>>> {
        ...
        assign_field(&mut self.rhox, &ckpt.rhox);
        assign_field(&mut self.rhoy, &ckpt.rhoy);
        assign_field(&mut self.rhoz, &ckpt.rhoz);
