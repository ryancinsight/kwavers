...
mod tests {
    ...
    fn test_gaussian_beam_analytical() {
        ...
        test_info!("Beam waist w0 = {:.3} mm", w0 * 1000.0);
        test_info!("Wavelength λ = {:.3} mm", lambda * 1000.0);
        test_info!("Rayleigh distance z_R = {:.3} mm", z_r * 1000.0);

        // Beam radius at various distances
        for factor in &[0.0, 0.5, 1.0, 2.0] {
            let z = factor * z_r;
            let w_z = w0 * (1.0 + (z / z_r).powi(2)).sqrt();
            test_info!("At z = {:.1} z_R: w(z) = {:.3} mm", factor, w_z * 1000.0);
        }
    ...
    fn test_diffraction_strength() {
        ...
        test_info!("k0 = {:.3} rad/m", k0);
        test_info!("k_max ≈ {:.3} rad/m", k_max);
        test_info!("k_max/k0 = {:.3}", k_max / k0);
        test_info!("Maximum phase over z_R: {:.3} rad", phase_max);
        test_info!(
