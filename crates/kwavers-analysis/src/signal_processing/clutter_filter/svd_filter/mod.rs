...
impl SignalSvdClutterFilter {
    ...
    pub fn filter(&self, slow_time_data: &Array2<f64>) -> KwaversResult<Array2<f64>> {
        ...
        // rank-deficient (low-rank tissue clutter is the entire premise);
        // leto's post-collapse SVD (bidiagonal QR) keeps the decomposition at
        // full rank with zero singular values rather than rejecting the input.
        // Returns V (right singular vectors as columns); transpose to Vᵀ.
