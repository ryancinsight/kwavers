...
use iris::color::{LookupTable, NamedColorMap, Normalized};
...
struct TransferFunction {
    color_map: LookupTable<NamedColorMap, 256>,
}

impl TransferFunction {
    /// Create a new transfer function
    fn new(scheme: &ColorScheme) -> Self {
        let named_map = match scheme {
        ...
        Self {
            color_map: LookupTable::from_map(named_map),
        }
    }
    ...
    fn map_value(&self, value: f32) -> [f32; 4] {
        ...
        let color = self.color_map.sample(Normalized::from_u8(quantized));
        *color.channels()
    }
}
