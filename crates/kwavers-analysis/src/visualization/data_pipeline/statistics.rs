...
impl TransferStatistics {
    ...
    pub fn record_transfer(&mut self, bytes: usize, duration: Time) {
        self.total_bytes_transferred += bytes;
        self.transfer_time = Time::from_base(self.transfer_time.into_base() + duration.into_base());
        self.num_transfers += 1;
        self.last_transfer_time = Instant::now();

        // Calculate bandwidth in GB/s
        if duration.into_base() > 0.0 {
            let gb = bytes as f32 / 1_073_741_824.0; // 1024^3
            let seconds = duration.into_base() as f32;
    ...
    pub fn average_transfer_time(&self) -> Time {
        if self.num_transfers > 0 {
            Time::from_base(self.transfer_time.into_base() / self.num_transfers as f64)
        } else {
            Time::from_base(0.0)
        }
    }
