...
use crate::route::grid::{Grid, NodeId, Step, NO_OWNER};
...
/// in `grid.owner`, and the router must simply push the new via off that geometry. Inline test
/// `diagonal_versus_via_clearance` in the `tests` module below pins the threshold.
const DIAGONAL_VIA_CLEARANCE_BUDGET_NM: i64 = 435_000;
...
fn diagonal_crosses_foreign_edge(
    grid: &Grid,
    u: (usize, usize, usize),
    v: (usize, usize, usize),
    net_id: i32,
...
fn diagonal_passes_foreign_via_corner(
    grid: &Grid,
    u: (usize, usize, usize),
    v: (usize, usize, usize),
    net_id: i32,
    ...
    corners.iter().any(|&corner| {
        ...
        owner != crate::route::grid::NO_VIA && owner != net_id
    })
}
...
/// contended.
/// The 4 candidate neighbour-pairs cover the four diagonal orientations (thinker analysis:
/// each pair connects two axial neighbours that differ by `±1` in x and `±1` in y, so they
/// form a 45° segment that "wraps around" `v`):
///
///  - `W ↔ N` `(cx-1, cy) → (cx, cy+1)`
///  - `N ↔ E` `(cx, cy+1) → (cx+1, cy)`
///  - `E ↔ S` `(cx+1, cy) → (cx, cy-1)`
///  - `S ↔ W` `(cx, cy-1) → (cx-1, cy)`
///
/// For each in-bounds pair, if the two cells are owned by the same net and that net is foreign
/// to the routing net, we compute the perpendicular distance from `v`'s column centre to the
/// foreign segment and reject if the closest approach is below the geometric clearance budget
/// ([`DIAGONAL_VIA_CLEARANCE_BUDGET_NM`] = 0.435 mm — derived from the DONE↔PROG residue at
/// [`output/full_driver/fpga_controller_tile.kicad_drc.rpt`]).
///
/// Why only axial-neighbour pairs (not 2-cell-diagonal pairs around `v`): a diagonal A\* step
/// cannot jump 2 cells without routing through an intermediate cell; if it routed through
/// `v`, PathFinder's overuse bitset would already catch the conflict. The only diagonal tracks
/// that can be **near** `v` without registering at `v` are the four pairs above — a diagonal
/// of the 1-cell square around `v`.
pub(crate) fn via_clips_foreign_diagonal_track(
    grid: &Grid,
    v: (usize, usize, usize),
    net_id: i32,
    budget_nm: i64,
...
fn diagonal_passes_foreign_track_corner(
    grid: &Grid,
    u: (usize, usize, usize),
    v: (usize, usize, usize),
    net_id: i32,
