...
impl AcousticHeatingSource {
    ...
    #[must_use]
    pub fn new(absorption_coefficient: ReciprocalLength<f64>, intensity: Intensity<f64>) -> Self {
        Self {
    ...
    pub fn power_at_depth(&self, depth: Length<f64>) -> VolumetricPowerDensity<f64> {
        let power_density: VolumetricPowerDensity<f64> =
            self.absorption_coefficient * self.intensity;
        let optical_depth: Dimensionless<f64> = self.absorption_coefficient * depth;
        power_density * 2.0 * (-2.0 * optical_depth.into_base()).exp()
    }
}
