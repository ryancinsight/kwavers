...
fn test_zeldovich_ros_generation_rates() {
    use crate::chemistry::ros_plasma::ros_species::calculate_ros_generation;
    ...
    assert!(
        (actual_oh_rate - expected_oh_rate).abs() < 1e-8 * expected_oh_rate,
        "OH rate must match Arrhenius prediction exactly"
    );
    assert!(
        (actual_h2o2_rate - expected_h2o2_rate).abs() < 1e-8 * expected_h2o2_rate,
        "H2O2 rate must match second-order formation exactly"
    );
}
...
fn test_ph_update() {
    ...
    let expected_ph = 6.9;
    assert!(
        (final_ph - expected_ph).abs() < 1e-12,
        "pH update must strictly follow derived acidification rate"
    );
}
