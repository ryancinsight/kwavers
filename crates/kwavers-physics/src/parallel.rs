...
/// Apply an unindexed mutation over one mutable and five immutable 3-D views.
#[inline]
pub(crate) fn zip_mut_five_refs<T, U, V, W, X, Y, F>(
    mut values: ArrayViewMut3<'_, T>,
    first: ArrayView3<'_, U>,
    second: ArrayView3<'_, V>,
    third: ArrayView3<'_, W>,
    fourth: ArrayView3<'_, X>,
    fifth: ArrayView3<'_, Y>,
    f: F,
) where
    T: Send,
    U: Sync,
    ...
    F: Fn(&mut T, &U, &V, &W, &X, &Y) + Send + Sync,
{
    ...
    assert_eq!(
        values.dim(),
        fifth.dim(),
        "invariant: physics zip fifth shape mismatch"
    );
    ...
    match (
        ...
        third.as_slice_memory_order(),
        fourth.as_slice_memory_order(),
        fifth.as_slice_memory_order(),
    ) {
        (Some(values), Some(first), Some(second), Some(third), Some(fourth), Some(fifth)) => {
            enumerate_mut_with::<Adaptive, _, _>(values, |idx, value| {
                f(
                    value,
                    &first[idx],
