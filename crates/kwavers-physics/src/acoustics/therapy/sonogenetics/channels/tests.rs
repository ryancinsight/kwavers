...
use aequitas::systems::si::quantities::{
    ElectricConductance, ElectricPotential, Pressure, ThermodynamicTemperature,
};
...
fn test_boltzmann_half_activation() {
    ...
    let tension = Array3::from_elem((2, 2, 2), bp.half_tension.into_base());
    let p_open = boltzmann_p_open(
        &tension,
        bp,
...
fn test_boltzmann_deep_open() {
    ...
    let tension = Array3::from_elem((2, 2, 2), 10.0 * bp.half_tension.into_base());
    let p_open = boltzmann_p_open(
        &tension,
        bp,
        ThermodynamicTemperature::from_base(BODY_TEMPERATURE_K),
...
fn test_boltzmann_monotone() {
    ...
    let tensions: Vec<f64> = (0..10)
        .map(|i| i as f64 * bp.half_tension.into_base() / 4.0)
        .collect();
    let mut prev = 0.0_f64;
    for &t in &tensions {
        let arr = Array3::from_elem((1, 1, 1), t);
        let p = boltzmann_p_open(
            &arr,
            bp,
            ThermodynamicTemperature::from_base(BODY_TEMPERATURE_K),
        )
...
fn test_boltzmann_zero_temperature_is_error() {
    ...
    let tension = Array3::zeros([2, 2, 2]);
    assert!(boltzmann_p_open(&tension, bp, ThermodynamicTemperature::from_base(0.0),).is_err());
    assert!(boltzmann_p_open(&tension, bp, ThermodynamicTemperature::from_base(-1.0),).is_err());
}
...
fn test_pressure_threshold_half_activation() {
    ...
    let p_rad = Array3::from_elem((2, 2, 2), pp.half_pressure.into_base());
...
fn test_pressure_threshold_zero_steepness_is_error() {
    let mut pp = PressureThresholdParams {
        ...
        steepness: Pressure::from_base(0.0),
    ...
    pp.steepness = Pressure::from_base(-1.0);
    assert!(pressure_threshold_p_open(&p_rad, &pp).is_err());
}
...
fn test_ion_current_analytical() {
    ...
    let current = ion_current(
        &p_open,
        ElectricConductance::from_base(3.0e-9),
        1000.0,
        ElectricPotential::from_base(-60.0e-3),
        ElectricPotential::from_base(0.0),
    );
...
fn test_ion_current_zero_at_reversal() {
    ...
    let e_rev = ElectricPotential::from_base(-10.0e-3_f64);
    let current = ion_current(
        &p_open,
        ElectricConductance::from_base(35.0e-12),
        5000.0,
        e_rev,
        e_rev,
    );
    ...
    for ch in [
        ...
        match ch.canonical_params() {
            GatingModel::Boltzmann(p) => {
                assert!(
                    p.gating_area.into_base() > 0.0,
                    "{ch:?}: gating_area must be > 0"
                );
                assert!(
                    p.half_tension.into_base() > 0.0,
                    "{ch:?}: half_tension must be > 0"
                );
                assert!(
                    p.single_channel_conductance.into_base() > 0.0,
                    "{ch:?}: conductance must be > 0"
                );
            }
            GatingModel::PressureThreshold(p) => {
                assert!(
                    p.half_pressure.into_base() > 0.0,
                    "{ch:?}: half_pressure must be > 0"
                );
                assert!(
                    p.steepness.into_base() > 0.0,
                    "{ch:?}: steepness must be > 0"
                );
                assert!(
                    p.single_channel_conductance.into_base() > 0.0,
                    "{ch:?}: conductance must be > 0"
...
fn test_bacterial_channel_variant_ordering() {
    ...
    assert!(
        g22n.half_tension.into_base() < g22s.half_tension.into_base(),
        "G22N must encode lower mechanical threshold than G22S"
    );
    assert!(
        mscs.single_channel_conductance.into_base() < g22s.single_channel_conductance.into_base(),
...
fn test_canonical_boltzmann_values_match_documented_table() {
    ...
    for (ch, a_gate, t_half, g, e_rev) in table {
        ...
        assert_relative_eq!(p.gating_area.into_base(), a_gate, max_relative = 1e-12);
        assert_relative_eq!(p.half_tension.into_base(), t_half, max_relative = 1e-12);
        assert_relative_eq!(
            p.single_channel_conductance.into_base(),
            g,
            max_relative = 1e-12
        );
        assert_relative_eq!(
            p.reversal_potential.into_base(),
            e_rev,
            max_relative = 1e-12
        );
    }
