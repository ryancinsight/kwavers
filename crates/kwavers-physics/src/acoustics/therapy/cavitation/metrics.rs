...
impl TherapyCavitationDetector {
    ...
    #[must_use]
    pub fn minnaert_frequency(&self, r0: Length<f64>) -> Frequency<f64> {
        let pressure_term =
            3.0 * AIR_POLYTROPIC_INDEX * ATMOSPHERIC_PRESSURE / DENSITY_WATER_NOMINAL;
        Frequency::from_base(pressure_term.sqrt() / (TWO_PI * r0.into_base()))
    }
    ...
    #[must_use]
    pub fn cavitation_index(&self, peak_negative_pressure: Pressure<f64>) -> f64 {
        peak_negative_pressure.into_base().abs() / self.blake_threshold.into_base()
    }
    ...
    #[must_use]
    pub fn cavitation_probability(&self, peak_negative_pressure: Pressure<f64>) -> f64 {
    ...
    #[must_use]
    pub fn is_stable_cavitation(&self, peak_negative_pressure: Pressure<f64>) -> bool {
        let ci = self.cavitation_index(peak_negative_pressure);
        ci > 0.5 && ci < 1.0
    }
    ...
    #[must_use]
    pub fn is_inertial_cavitation(&self, peak_negative_pressure: Pressure<f64>) -> bool {
