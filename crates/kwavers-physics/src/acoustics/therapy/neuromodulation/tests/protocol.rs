...
use aequitas::systems::si::quantities::{Frequency, Pressure, TemperatureDifference, Time};
...
fn protocol_derived_quantities_match_blackmore_definitions() {
    ...
    let p = PulseTrainProtocol {
        ...
        pulse_length: Time::from_base(0.5e-3),
        pulse_repetition_frequency: Frequency::from_base(1500.0),
        burst_duration: Time::from_base(0.2),
        burst_interval: Time::from_base(0.3),
        num_bursts: 10,
    };
    assert!(p.is_valid());
    ...
    assert!(
        (p.burst_repetition_frequency().into_base() - 2.0).abs() < 1e-12,
        "BRF = 1/0.5"
    );
    assert!(
        (p.total_time().into_base() - 5.0).abs() < 1e-12,
        "TT = 10·0.5"
    );
}
...
fn protocol_rejects_pulse_longer_than_period() {
    ...
    let p = PulseTrainProtocol {
        carrier_frequency: Frequency::from_base(500.0e3),
        pulse_length: Time::from_base(1.0e-3),
        pulse_repetition_frequency: Frequency::from_base(2000.0),
        burst_duration: Time::from_base(0.1),
...
fn protocol_pulse_envelope_gates_correctly() {
    ...
    let p = PulseTrainProtocol {
        carrier_frequency: Frequency::from_base(500.0e3),
        pulse_length: Time::from_base(1.0e-3),
        pulse_repetition_frequency: Frequency::from_base(100.0),
        burst_duration: Time::from_base(0.05),
        burst_interval: Time::from_base(0.0),
        num_bursts: 1,
    };
    assert!(
        p.pulse_active(Time::from_base(0.0005)),
        "inside first pulse"
    );
    assert!(!p.pulse_active(Time::from_base(0.005)), "in pulse-off gap");
    assert!(
        p.pulse_active(Time::from_base(0.0105)),
        "inside second pulse"
    );
    assert!(
        !p.pulse_active(Time::from_base(0.06)),
        "after the burst ends"
    );
    assert!(!p.pulse_active(Time::from_base(-1.0)), "before onset");
}
...
fn theta_burst_atkinson_dosimetry_matches_paper() {
    ...
    assert!(
        (p.total_time().into_base() - 80.0).abs() < 1e-9,
        "TT = 80 s"
    );
...
fn dosimetry_fda_limit_screening() {
    ...
    let safe = tissue_dosimetry(&p, Pressure::from_base(50.0e3));
    assert!(
    ...
    // (MI = 5/√0.5 ≈ 7.07).
    let unsafe_dose = tissue_dosimetry(&p, Pressure::from_base(5.0e6));
    assert!(
