...
mod tests {
    ...
    fn vdw_pressure_approaches_ideal_gas_at_large_volume() {
        ...
        // Ideal gas reference
        let volume = 4.0 / 3.0 * std::f64::consts::PI * state.radius.powi(3);
        let n_moles = (state.n_gas + state.n_vapor) / kwavers_core::constants::AVOGADRO;
        let p_ideal = n_moles * kwavers_core::constants::GAS_CONSTANT * state.temperature / volume;
    ...
    fn vdw_pressure_errors_when_volume_below_excluded() {
        ...
        calculate_vdw_pressure(&state)
            .expect_err("must return Err when volume <= excluded volume");
    }
}
