...
use aequitas::systems::si::quantities::{Frequency, MassDensity, Pressure};
...
fn ambient_pressure() -> Pressure<f64> {
    Pressure::from_base(ATMOSPHERIC_PRESSURE)
}

fn water_density() -> MassDensity<f64> {
...
fn test_scattering_cross_section_positive() {
    ...
    for &f_mhz in &[1.0f64, 2.0, 3.0, 5.0, 10.0] {
        let sigma = mb
            .scattering_cross_section(Frequency::from_base(f_mhz * MHZ_TO_HZ))
            .into_base();
        assert!(
...
fn test_scattering_peak_near_resonance() {
    ...
    let f_r = mb.resonance_frequency(ambient_pressure(), water_density());
    let sigma_res = mb.scattering_cross_section(f_r).into_base();
    let sigma_far = mb
        .scattering_cross_section(Frequency::from_base(f_r.into_base() * 10.0))
        .into_base();
    assert!(
        sigma_res > sigma_far,
...
fn test_scattering_uses_water_sound_speed() {
    ...
    let f_r = mb.resonance_frequency(ambient_pressure(), water_density());
    let sigma_res = mb.scattering_cross_section(f_r).into_base();
    let radius = mb.radius_eq.into_base();
    let geo = FOUR_PI * radius * radius;
...
fn test_scattering_decreases_above_resonance() {
    ...
    let f_r = mb.resonance_frequency(ambient_pressure(), water_density());
    let s1 = mb
        .scattering_cross_section(Frequency::from_base(f_r.into_base() * 1.5))
        .into_base();
    let s2 = mb
        .scattering_cross_section(Frequency::from_base(f_r.into_base() * 3.0))
        .into_base();
    assert!(
        s1 > s2,
...
fn test_scattering_resonance_matches_closed_form() {
    ...
    let mb = Microbubble::sono_vue();
    let r = mb.radius_eq.into_base();
    let c_l = SOUND_SPEED_WATER;
    let rho_l = DENSITY_WATER_NOMINAL;
    let mu_l = VISCOSITY_WATER;
    let omega0 = 2.0
        * PI
        * mb.resonance_frequency(ambient_pressure(), water_density())
            .into_base();
    ...
    let delta_sh = 4.0 * mb.shell_thickness.into_base() * mb.shell_viscosity.into_base()
        / (omega0 * rho_l * r * r * r);
    ...
    let f_r = mb.resonance_frequency(ambient_pressure(), water_density());
    let actual = mb.scattering_cross_section(f_r).into_base();
    assert!(
...
fn test_scattering_low_frequency_omega_squared_scaling() {
    ...
    let f = f_r.into_base() / 1000.0;
    let s1 = mb
        .scattering_cross_section(Frequency::from_base(f))
        .into_base();
    let s2 = mb
        .scattering_cross_section(Frequency::from_base(2.0 * f))
        .into_base();
    assert!(
        (s2 / s1 - 4.0).abs() < 0.01,
