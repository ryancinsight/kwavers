...
pub struct ClinicalSafetyMonitor {
    ...
    /// `None` until the first [`ClinicalSafetyMonitor::check_safety`] call, so "never checked"
    /// is representable without back-dating an [`Instant`] past the process clock origin.
