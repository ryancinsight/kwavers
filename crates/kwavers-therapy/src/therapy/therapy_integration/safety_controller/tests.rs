...
fn create_test_controller() -> SafetyController {
    let limits = TherapyIntegrationSafetyLimits {
        ...
        max_treatment_time: Time::from_base(600.0),
    };
...
fn test_thermal_index_violation() {
    ...
    controller.start_monitoring(Time::from_base(0.0));
    ...
    let action = controller
        .evaluate_safety(metrics, Time::from_base(1.0))
        .unwrap();
...
fn test_treatment_time_limit() {
    ...
    let action = controller
        .evaluate_safety(metrics, Time::from_base(600.0))
        .unwrap();
    assert_eq!(action, TherapyAction::Stop);
}
