...
fn focused_cap_count_and_radius() {
    ...
    assert_eq!(positions.shape(), [64, 3]);
    for row in positions.rows().unwrap() {
...
fn skull_adaptive_benchmark_selects_ct_conditioned_aperture_and_evaluates() {
    ...
    assert_eq!(result.reference_pressure_pa.shape(), shape);
    assert_eq!(result.baseline_pressure_pa.shape(), shape);
}
