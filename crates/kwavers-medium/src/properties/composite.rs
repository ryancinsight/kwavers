...
use kwavers_core::constants::acoustic_parameters::{BONE_DENSITY, BONE_SOUND_SPEED};
use kwavers_core::constants::tissue_acoustics::{ACOUSTIC_ABSORPTION_CORTICAL_BONE, B_OVER_A_BONE};
use std::fmt;
...
mod tests {
    ...
    fn test_material_builder() {
        ...
        assert!(props.thermal.as_ref().unwrap().conductivity().into_base() > 0.0);
    ...
    fn test_material_presets() {
        ...
        assert!(water.thermal.as_ref().unwrap().conductivity().into_base() > 0.0);
