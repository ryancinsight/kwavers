...
fn validate_steering_vector_computation() {
    ...
    for i in 1..sensor_positions.len() {
        ...
        let expected_phase =
            (2.0 * std::f64::consts::PI * frequency * distance / speed_of_sound).cos();
        assert!(
    ...
    for i in 0..sensor_positions.len() {
        let distance = ((sensor_positions[i][0] - source_position[0]).powi(2)
            ...
            + (sensor_positions[i][2] - source_position[2]).powi(2))
        .sqrt();
...
fn validate_blackbody_radiation_planck_law() {
    ...
    use kwavers::physics::optics::sonoluminescence::blackbody::{
    ...
    let radiance_field =
        calculate_blackbody_emission(&temperature_field, &bubble_radius_field, &model);

    // Test that all radiance values are positive and reasonable
    assert!(
        radiance_field.iter().all(|&r| r > 0.0),
        "Blackbody radiance should be positive"
    );
    assert!(
        radiance_field.iter().all(|&r| r < 1e20),
        "Blackbody radiance should be physically reasonable"
    );
    ...
    let hot_radiance_field =
        calculate_blackbody_emission(&hot_temperature_field, &bubble_radius_field, &model);

    assert!(
        hot_radiance_field.iter().sum::<f64>() > radiance_field.iter().sum::<f64>(),
...
fn validate_bremsstrahlung_kramers_law() {
    ...
    use kwavers::physics::optics::sonoluminescence::bremsstrahlung::{
    ...
    assert!(
        power_field.iter().all(|&p| p >= 0.0),
        "Bremsstrahlung power should be non-negative"
    );
    ...
    assert!(
        high_power_field.iter().sum::<f64>() > power_field.iter().sum::<f64>(),
...
fn validate_photoacoustic_wave_equation() {
    ...
    assert!(
        mua_blood > mua_tissue,
        "Blood should absorb more light than tissue"
    );
    assert!(
        mua_blood > 0.0 && mua_tissue > 0.0,
...
fn validate_sonoluminescence_bubble_temperature() {
    ...
    assert!(
        temperature > 5000.0,
        "Extreme collapse should produce high temperature, got {} K",
        temperature
    );
    assert!(
        (temperature - expected_temp).abs() / expected_temp < 0.1,
        "Temperature should match adiabatic heating theory: expected {}, got {}",
        expected_temp,
        temperature
    );
    ...
    assert!(
        ...
        "More extreme collapse should produce higher temperature"
    );
}
...
fn validate_sonoluminescence_spectral_analysis() {
    ...
    assert!(
        total_emission > 0.0,
        "Should produce light emission from hot plasma"
    );
    ...
    assert_eq!(
        ...
        "Spectrum should have matching wavelength/intensity arrays"
    );
    ...
    assert!(
        total_intensity > 0.0,
        "Spectrum should have positive total intensity"
    );

    let peak_wavelength = spectrum.peak_wavelength();
    assert!(
        peak_wavelength > 200e-9 && peak_wavelength < 800e-9,
        "Peak wavelength should be in visible/near-IR range, got {} m",
        peak_wavelength
    );
}
...
fn validate_acoustic_to_optic_energy_conversion() {
    ...
    assert!(
        params.r0 > 1e-6,
        "Bubbles should be micrometer-sized for sonoluminescence"
    );
    assert!(
        params.gamma > 1.0,
        "Gas content should provide compressibility for oscillation"
    );
    ...
    assert!(
        total_light > 0.0,
        "Acoustic energy should convert to optical emission"
    );
    ...
    assert!(
        peak_wavelength > 200e-9 && peak_wavelength < 800e-9,
        "Sonoluminescence should peak in UV-visible range, got {} m",
        peak_wavelength
    );
}
...
fn validate_interdisciplinary_coupling_efficiency() {
    ...
    assert!(
        hotter_optical_energy > optical_energy,
...
fn validate_mie_scattering_theory() {
    ...
    assert!(
        m.re > 1.0,
        "Relative refractive index should be greater than 1"
    );
...
fn validate_multi_modal_fusion_ultrasound_optical() {
    ...
    // Verify registration
    assert_eq!(
        fusion.num_registered_modalities(),
        2,
        "Should have two modalities registered"
    );
    assert!(
        fusion.is_modality_registered("ultrasound"),
        "Should contain ultrasound data"
    );
    assert!(
        fusion.is_modality_registered("optical_500nm"),
        "Should contain optical data"
    );
    ...
    assert!(
        fused_mean >= us_mean.min(opt_mean) && fused_mean <= us_mean.max(opt_mean),
    ...
    assert!(
        !fused_result.modality_quality.is_empty(),
...
fn validate_fusion_registration_validation() {
    ...
    assert_eq!(
        fusion.num_registered_modalities(),
        2,
        "Should have two modalities"
    );

    // Now fusion should succeed
    let fused_result = fusion.fuse().unwrap();
    assert!(
        fused_result.intensity_image.iter().all(|&x| x >= 0.0),
        "Fused values should be non-negative"
    );
    ...
    for method in methods {
        ...
        assert!(
            test_result
                .intensity_image
                .iter()
                .all(|&x: &f64| x.is_finite()),
            "All fusion methods should produce finite values"
        );
    }
}
...
fn validate_interdisciplinary_fusion_quality() {
    ...
    let max_fused = fused_result
        .intensity_image
        .iter()
        .fold(0.0_f64, |a: f64, &b: &f64| a.max(b));
    let min_fused = fused_result
        .intensity_image
        .iter()
    ...
    assert!(
        max_fused > 0.5,
        "Fusion should preserve high-contrast features, got max={}",
        max_fused
    );
    assert!(
        min_fused < 0.3,
        "Fusion should preserve low-contrast regions"
    );

    // Quality should be reasonable for complementary modalities
    let avg_quality: f64 = fused_result.modality_quality.values().sum::<f64>()
        / fused_result.modality_quality.len() as f64;
    assert!(
        avg_quality > 0.5,
        "Complementary modalities should yield good fusion quality"
    );
    ...
    for &wl in &wavelengths {
        ...
        let test_avg_quality: f64 = test_result.modality_quality.values().sum::<f64>()
            / test_result.modality_quality.len() as f64;
        assert!(
            test_avg_quality > 0.0,
            "All wavelengths should produce valid quality scores"
        );
    }
}
