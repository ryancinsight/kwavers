...
fn test_plane_wave_propagation() {
    ...
    let max_diff = (&fdtd_result - &pstd_result).mapv(f64::abs).fold(0.0f64, |a, &b| a.max(b));
    let max_val = fdtd_result.mapv(f64::abs).fold(0.0f64, |a, &b| a.max(b)).max(
        pstd_result.mapv(f64::abs).fold(0.0f64, |a, &b| a.max(b))
    );
    
    // Allow 5% relative difference due to numerical dispersion
    assert!(max_diff / max_val < 0.05, 
...
fn test_standing_wave_analytical() {
    ...
    assert!(fdtd_error < 0.02, "FDTD error too large: {:.2}%", 100.0 * fdtd_error);
    
    // Check PSTD accuracy - should be more accurate
    let pstd_error = calculate_relative_error(&pstd_result, &analytical);
    assert!(pstd_error < 0.01, "PSTD error too large: {:.2}%", 100.0 * pstd_error);
    assert!(pstd_error < fdtd_error, "PSTD should be more accurate than FDTD");
}

/// Helper function to run FDTD simulation
fn run_fdtd_simulation(grid: &Grid, medium: &HomogeneousMedium, initial_pressure: &Array3<f64>) -> Array3<f64> {
...
fn run_pstd_simulation(grid: &Grid, medium: &HomogeneousMedium, initial_pressure: &Array3<f64>) -> Array3<f64> {
...
fn test_dispersion_characteristics() {
    ...
    assert!(fdtd_error < 5, "FDTD dispersion error too large: {} grid points", fdtd_error);
    assert!(pstd_error < 2, "PSTD dispersion error too large: {} grid points", pstd_error);
}
