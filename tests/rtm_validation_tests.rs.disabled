...
use kwavers::{
    ...
    solver::reconstruction::seismic::{
        ReverseTimeMigration, RtmImagingCondition, SeismicImagingConfig,
    },
    Grid, KwaversResult,
};
...
fn test_rtm_horizontal_reflector() -> KwaversResult<()> {
    ...
    assert!(
        depth_error <= REFLECTOR_POSITION_TOLERANCE,
    ...
    println!(
        "Horizontal reflector test: detected at depth {} (expected: {})",
...
fn test_rtm_imaging_conditions() -> KwaversResult<()> {
    ...
    for (i, condition) in imaging_conditions.iter().enumerate() {
        ...
        assert!(
            error1 <= REFLECTOR_POSITION_TOLERANCE || error2 <= REFLECTOR_POSITION_TOLERANCE,
        ...
        println!(
            "Imaging condition {:?}: max reflectivity at depth {}",
            condition, max_depth
        );
    }
...
fn test_rtm_dipping_reflector() -> KwaversResult<()> {
    ...
    assert!(
        right_side_depth > left_side_depth,
    ...
    println!(
        "Dipping reflector test: left depth {}, right depth {} (dip detected)",
...
fn test_rtm_cfl_validation() -> KwaversResult<()> {
    ...
    assert!(
        result.is_ok(),
        "RTM should handle high velocities with proper CFL validation"
    );
...
fn test_rtm_point_scatterer() -> KwaversResult<()> {
    ...
    assert!(
        position_error <= 3 * REFLECTOR_POSITION_TOLERANCE,
    ...
    println!(
        "Point scatterer test: detected at ({}, {}, {}) vs expected ({}, {}, {})",
...
fn test_rtm_memory_efficiency() -> KwaversResult<()> {
    ...
    assert!(
        result.is_ok(),
        "RTM should handle larger models efficiently"
    );

    println!(
        "Memory efficiency test: RTM processed {}^3 grid successfully",
        grid.nx
    );
...
fn generate_rtm_synthetic_data(
