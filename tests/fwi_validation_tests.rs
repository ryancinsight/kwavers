...
use kwavers::{
    ...
    solver::reconstruction::seismic::{
        FullWaveformInversion, SeismicImagingConfig, ReverseTimeMigration
    },
...
fn test_fwi_two_layer_model() -> KwaversResult<()> {
    ...
    let upper_recovered = recovered_velocity.slice(s![.., .., 0..grid.nz/2]).mean().unwrap();
    let lower_recovered = recovered_velocity.slice(s![.., .., grid.nz/2..]).mean().unwrap();
    
    assert_relative_eq!(upper_recovered, 2000.0, epsilon = 2000.0 * VELOCITY_TOLERANCE);
    assert_relative_eq!(lower_recovered, 3000.0, epsilon = 3000.0 * VELOCITY_TOLERANCE);
    
    println!("Two-layer test: Upper layer {:.1} m/s (true: 2000), Lower layer {:.1} m/s (true: 3000)", 
...
fn test_fwi_gradient_accuracy() -> KwaversResult<()> {
    ...
    fwi.compute_gradient(&observed_data, &source_positions, &receiver_positions, &grid, &medium)?;
    ...
    for &(i, j, k) in &test_points {
        if i < grid.nx && j < grid.ny && k < grid.nz {
            ...
            let misfit_plus = fwi_plus.compute_data_misfit(&observed_data, &source_positions, &receiver_positions, &grid, &medium)?;
            
            // Backward perturbation
            let mut perturbed_model = velocity_model.clone();
            perturbed_model[[i, j, k]] -= perturbation;
            let mut fwi_minus = FullWaveformInversion::new(config.clone(), perturbed_model);
            let misfit_minus = fwi_minus.compute_data_misfit(&observed_data, &source_positions, &receiver_positions, &grid, &medium)?;
    ...
    for &(i, j, k) in &test_points {
        if i < grid.nx && j < grid.ny && k < grid.nz {
            ...
            if numerical.abs() > 1e-12 { // Avoid division by very small numbers
    ...
    println!("Gradient accuracy test passed for {} test points", test_points.len());
...
fn test_cfl_condition_validation() -> KwaversResult<()> {
    ...
    assert!(stable_dt <= expected_max_dt, 
        "Computed timestep {:.6e} violates CFL condition (max: {:.6e})", 
        stable_dt, expected_max_dt);
    
    println!("CFL test: stable dt = {:.6e} s (max allowed: {:.6e} s)", stable_dt, expected_max_dt);
...
fn test_fwi_convergence() -> KwaversResult<()> {
    ...
    for iteration in 0..10 {
        ...
        let current_misfit = fwi.compute_data_misfit(&observed_data, &source_positions, &receiver_positions, &grid, &medium)?;
        ...
        if iteration > 0 {
            ...
            println!("Iteration {}: misfit = {:.6e}, improvement = {:.2}%", 
                     iteration, current_misfit, improvement * 100.0);
        }
        ...
        let step_size = fwi.line_search(&observed_data, &source_positions, &receiver_positions, &grid, &medium)?;
        fwi.update_velocity_model(step_size)?;
    }
    
    // Verify convergence: misfit should decrease overall
    assert!(misfits.last().unwrap() < &misfits[0], 
        "FWI did not converge: initial misfit {:.6e}, final misfit {:.6e}", 
        misfits[0], misfits.last().unwrap());
...
fn test_rtm_imaging() -> KwaversResult<()> {
    ...
    assert!(depth_error <= 3, 
        "RTM reflector position error: found at depth {}, expected near {}", 
        max_depth, interface_depth);
    
    println!("RTM test: reflector detected at depth index {} (expected: {})", max_depth, interface_depth);
...
fn generate_synthetic_seismic_data(velocity_model: &Array3<f64>, grid: &Grid) -> KwaversResult<Array2<f64>> {
...
fn create_line_receiver_array(grid: &Grid, num_receivers: usize) -> Vec<[f64; 3]> {
    ...
    for i in 0..num_receivers {
        ...
        receivers.push([x, grid.ny as f64 * grid.dy / 2.0, grid.nz as f64 * grid.dz * 0.1]);
    }
    receivers
}
...
mod integration_tests {
    ...
    fn test_fwi_rtm_workflow() -> KwaversResult<()> {
        ...
        let anomaly_velocity = estimated_velocity.slice(s![8..16, 8..16, 8..16]).mean().unwrap();
        let background_velocity = estimated_velocity.slice(s![0..8, 0..8, 0..8]).mean().unwrap();
        
        assert!(anomaly_velocity > background_velocity, 
            "FWI failed to detect velocity anomaly: anomaly {:.1} vs background {:.1}", 
            anomaly_velocity, background_velocity);
        
        println!("Integrated test: anomaly velocity {:.1} m/s, background {:.1} m/s", 
                 anomaly_velocity, background_velocity);
