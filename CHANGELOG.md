...
- [minor] Add `BowlTransducer::with_element_count` and read-only element
  layout accessors so fixed-count focused-bowl arrays, including 1024-element
  hemispherical transcranial layouts, can be generated by the source-domain
  bowl transducer without introducing anatomy-specific source APIs.
